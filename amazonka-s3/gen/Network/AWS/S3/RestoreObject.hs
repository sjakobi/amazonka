...
data RestoreObjectResponse = RestoreObjectResponse'
  { _rorrsRequestCharged ::
      !(Maybe RequestCharged),
    _rorrsRestoreOutputPath ::
      !(Maybe Text),
    _rorrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rorrsRequestCharged' - Undocumented member.
--
-- * 'rorrsRestoreOutputPath' - Indicates the path in the provided S3 output location where Select results will be restored to.
--
-- * 'rorrsResponseStatus' - -- | The response status code.
restoreObjectResponse ::
  -- | 'rorrsResponseStatus'
  Int ->
  RestoreObjectResponse
restoreObjectResponse pResponseStatus_ =
  RestoreObjectResponse'
    { _rorrsRequestCharged =
        Nothing,
      _rorrsRestoreOutputPath = Nothing,
      _rorrsResponseStatus = pResponseStatus_
    }

-- | Undocumented member.
rorrsRequestCharged :: Lens' RestoreObjectResponse (Maybe RequestCharged)
rorrsRequestCharged = lens _rorrsRequestCharged (\s a -> s {_rorrsRequestCharged = a})

-- | Indicates the path in the provided S3 output location where Select results will be restored to.
rorrsRestoreOutputPath :: Lens' RestoreObjectResponse (Maybe Text)
rorrsRestoreOutputPath = lens _rorrsRestoreOutputPath (\s a -> s {_rorrsRestoreOutputPath = a})

-- | -- | The response status code.
rorrsResponseStatus :: Lens' RestoreObjectResponse Int
rorrsResponseStatus = lens _rorrsResponseStatus (\s a -> s {_rorrsResponseStatus = a})
