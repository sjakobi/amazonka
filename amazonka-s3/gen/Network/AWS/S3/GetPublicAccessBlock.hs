...
data GetPublicAccessBlockResponse = GetPublicAccessBlockResponse'
  { _gpabrrsPublicAccessBlockConfiguration ::
      !( Maybe
           PublicAccessBlockConfiguration
       ),
    _gpabrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gpabrrsPublicAccessBlockConfiguration' - The @PublicAccessBlock@ configuration currently in effect for this Amazon S3 bucket.
--
-- * 'gpabrrsResponseStatus' - -- | The response status code.
getPublicAccessBlockResponse ::
  -- | 'gpabrrsResponseStatus'
  Int ->
  GetPublicAccessBlockResponse
getPublicAccessBlockResponse pResponseStatus_ =
  GetPublicAccessBlockResponse'
    { _gpabrrsPublicAccessBlockConfiguration =
        Nothing,
      _gpabrrsResponseStatus = pResponseStatus_
    }

-- | The @PublicAccessBlock@ configuration currently in effect for this Amazon S3 bucket.
gpabrrsPublicAccessBlockConfiguration :: Lens' GetPublicAccessBlockResponse (Maybe PublicAccessBlockConfiguration)
gpabrrsPublicAccessBlockConfiguration = lens _gpabrrsPublicAccessBlockConfiguration (\s a -> s {_gpabrrsPublicAccessBlockConfiguration = a})

-- | -- | The response status code.
gpabrrsResponseStatus :: Lens' GetPublicAccessBlockResponse Int
gpabrrsResponseStatus = lens _gpabrrsResponseStatus (\s a -> s {_gpabrrsResponseStatus = a})
