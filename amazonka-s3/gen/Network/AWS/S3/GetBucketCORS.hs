...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gbcrrsCORSRules' - A set of origins and methods (cross-origin access that you want to allow). You can add up to 100 rules to the configuration.
--
...
getBucketCORSResponse pResponseStatus_ =
  GetBucketCORSResponse'
    { _gbcrrsCORSRules = Nothing,
      _gbcrrsResponseStatus = pResponseStatus_
    }

-- | A set of origins and methods (cross-origin access that you want to allow). You can add up to 100 rules to the configuration.
gbcrrsCORSRules :: Lens' GetBucketCORSResponse [CORSRule]
gbcrrsCORSRules = lens _gbcrrsCORSRules (\s a -> s {_gbcrrsCORSRules = a}) . _Default . _Coerce

-- | -- | The response status code.
gbcrrsResponseStatus :: Lens' GetBucketCORSResponse Int
