...
module Network.AWS.Rekognition.DescribeProjectVersions
  ( -- * Creating a Request
    ...
    -- * Response Lenses
    dpvrprsNextToken,
    dpvrprsProjectVersionDescriptions,
    dpvrprsResponseStatus,
  )
where
...
instance AWSPager DescribeProjectVersions where
  page rq rs
    | stop (rs ^. dpvrprsNextToken) = Nothing
    | stop (rs ^. dpvrprsProjectVersionDescriptions) =
      Nothing
    | otherwise =
      Just $ rq & dpvNextToken .~ rs ^. dpvrprsNextToken
...
data DescribeProjectVersionsResponse = DescribeProjectVersionsResponse'
  { _dpvrprsNextToken ::
    ...
    _dpvrprsProjectVersionDescriptions ::
      !( Maybe
           [ProjectVersionDescription]
       ),
    _dpvrprsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dpvrprsNextToken' - If the previous response was incomplete (because there is more results to retrieve), Amazon Rekognition Custom Labels returns a pagination token in the response. You can use this pagination token to retrieve the next set of results.
--
-- * 'dpvrprsProjectVersionDescriptions' - A list of model descriptions. The list is sorted by the creation date and time of the model versions, latest to earliest.
--
-- * 'dpvrprsResponseStatus' - -- | The response status code.
describeProjectVersionsResponse ::
  -- | 'dpvrprsResponseStatus'
  Int ->
  DescribeProjectVersionsResponse
describeProjectVersionsResponse pResponseStatus_ =
  DescribeProjectVersionsResponse'
    { _dpvrprsNextToken =
      ...
      _dpvrprsProjectVersionDescriptions =
        Nothing,
      _dpvrprsResponseStatus = pResponseStatus_
    }

-- | If the previous response was incomplete (because there is more results to retrieve), Amazon Rekognition Custom Labels returns a pagination token in the response. You can use this pagination token to retrieve the next set of results.
dpvrprsNextToken :: Lens' DescribeProjectVersionsResponse (Maybe Text)
dpvrprsNextToken = lens _dpvrprsNextToken (\s a -> s {_dpvrprsNextToken = a})

-- | A list of model descriptions. The list is sorted by the creation date and time of the model versions, latest to earliest.
dpvrprsProjectVersionDescriptions :: Lens' DescribeProjectVersionsResponse [ProjectVersionDescription]
dpvrprsProjectVersionDescriptions = lens _dpvrprsProjectVersionDescriptions (\s a -> s {_dpvrprsProjectVersionDescriptions = a}) . _Default . _Coerce

-- | -- | The response status code.
dpvrprsResponseStatus :: Lens' DescribeProjectVersionsResponse Int
dpvrprsResponseStatus = lens _dpvrprsResponseStatus (\s a -> s {_dpvrprsResponseStatus = a})
