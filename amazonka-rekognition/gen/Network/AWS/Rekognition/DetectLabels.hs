...
data DetectLabels = DetectLabels'
  { -- | Maximum number of labels you want the service to return in the response.
    ...
    maxLabels :: Prelude.Maybe Prelude.Nat,
...
detectLabels_maxLabels = Lens.lens (\DetectLabels' {maxLabels} -> maxLabels) (\s@DetectLabels' {} a -> s {maxLabels = a} :: DetectLabels) Prelude.. Lens.mapping Prelude._Nat
