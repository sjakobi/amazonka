...
data StartTextDetection = StartTextDetection'
  { _stdNotificationChannel ::
    ...
    _stdClientRequestToken ::
      !(Maybe Text),
    _stdJobTag :: !(Maybe Text),
    _stdVideo :: !Video
  }
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'stdrrsJobId' - Identifier for the text detection job. Use @JobId@ to identify the job in a subsequent call to @GetTextDetection@ .
--
-- * 'stdrrsResponseStatus' - -- | The response status code.
startTextDetectionResponse ::
  -- | 'stdrrsResponseStatus'
  Int ->
  StartTextDetectionResponse
startTextDetectionResponse pResponseStatus_ =
  StartTextDetectionResponse'
    { _stdrrsJobId = Nothing,
      _stdrrsResponseStatus = pResponseStatus_
    }

-- | Identifier for the text detection job. Use @JobId@ to identify the job in a subsequent call to @GetTextDetection@ .
stdrrsJobId :: Lens' StartTextDetectionResponse (Maybe Text)
stdrrsJobId = lens _stdrrsJobId (\s a -> s {_stdrrsJobId = a})

-- | -- | The response status code.
stdrrsResponseStatus :: Lens' StartTextDetectionResponse Int
stdrrsResponseStatus = lens _stdrrsResponseStatus (\s a -> s {_stdrrsResponseStatus = a})
