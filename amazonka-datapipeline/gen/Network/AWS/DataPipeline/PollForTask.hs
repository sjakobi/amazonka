...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'pftrrsTaskObject' - The information needed to complete the task that is being assigned to the task runner. One of the fields returned in this object is @taskId@ , which contains an identifier for the task being assigned. The calling task runner uses @taskId@ in subsequent calls to 'ReportTaskProgress' and 'SetTaskStatus' .
--
-- * 'pftrrsResponseStatus' - -- | The response status code.
pollForTaskResponse ::
  -- | 'pftrrsResponseStatus'
  Int ->
  PollForTaskResponse
pollForTaskResponse pResponseStatus_ =
  PollForTaskResponse'
    { _pftrrsTaskObject = Nothing,
      _pftrrsResponseStatus = pResponseStatus_
    }

-- | The information needed to complete the task that is being assigned to the task runner. One of the fields returned in this object is @taskId@ , which contains an identifier for the task being assigned. The calling task runner uses @taskId@ in subsequent calls to 'ReportTaskProgress' and 'SetTaskStatus' .
pftrrsTaskObject :: Lens' PollForTaskResponse (Maybe TaskObject)
pftrrsTaskObject = lens _pftrrsTaskObject (\s a -> s {_pftrrsTaskObject = a})

-- | -- | The response status code.
pftrrsResponseStatus :: Lens' PollForTaskResponse Int
pftrrsResponseStatus = lens _pftrrsResponseStatus (\s a -> s {_pftrrsResponseStatus = a})
