...
data GetLogGroupFieldsResponse = GetLogGroupFieldsResponse'
  { _glgfrrsLogGroupFields ::
      !( Maybe
           [LogGroupField]
       ),
    _glgfrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'glgfrrsLogGroupFields' - The array of fields found in the query. Each object in the array contains the name of the field, along with the percentage of time it appeared in the log events that were queried.
--
-- * 'glgfrrsResponseStatus' - -- | The response status code.
getLogGroupFieldsResponse ::
  -- | 'glgfrrsResponseStatus'
  Int ->
  GetLogGroupFieldsResponse
getLogGroupFieldsResponse pResponseStatus_ =
  GetLogGroupFieldsResponse'
    { _glgfrrsLogGroupFields =
        Nothing,
      _glgfrrsResponseStatus = pResponseStatus_
    }

-- | The array of fields found in the query. Each object in the array contains the name of the field, along with the percentage of time it appeared in the log events that were queried.
glgfrrsLogGroupFields :: Lens' GetLogGroupFieldsResponse [LogGroupField]
glgfrrsLogGroupFields = lens _glgfrrsLogGroupFields (\s a -> s {_glgfrrsLogGroupFields = a}) . _Default . _Coerce

-- | -- | The response status code.
glgfrrsResponseStatus :: Lens' GetLogGroupFieldsResponse Int
glgfrrsResponseStatus = lens _glgfrrsResponseStatus (\s a -> s {_glgfrrsResponseStatus = a})
