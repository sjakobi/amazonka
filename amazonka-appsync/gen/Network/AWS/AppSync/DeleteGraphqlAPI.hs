...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dgarrsResponseStatus' - -- | The response status code.
deleteGraphqlAPIResponse ::
  -- | 'dgarrsResponseStatus'
  Int ->
  DeleteGraphqlAPIResponse
...
dgarrsResponseStatus :: Lens' DeleteGraphqlAPIResponse Int
dgarrsResponseStatus = lens _dgarrsResponseStatus (\s a -> s {_dgarrsResponseStatus = a})
