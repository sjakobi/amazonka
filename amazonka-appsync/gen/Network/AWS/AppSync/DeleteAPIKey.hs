...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dakrrsResponseStatus' - -- | The response status code.
deleteAPIKeyResponse ::
  -- | 'dakrrsResponseStatus'
  Int ->
  DeleteAPIKeyResponse
...
dakrrsResponseStatus :: Lens' DeleteAPIKeyResponse Int
dakrrsResponseStatus = lens _dakrrsResponseStatus (\s a -> s {_dakrrsResponseStatus = a})
