...
instance AWSRequest FlushAPICache where
  ...
  response =
    receiveEmpty
      ( \s h x ->
          FlushAPICacheResponse' <$> (pure (fromEnum s))
      )
...
instance ToPath FlushAPICache where
  toPath FlushAPICache' {..} =
    mconcat
      ["/v1/apis/", toBS _facApiId, "/FlushCache"]
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'facrrsResponseStatus' - -- | The response status code.
flushAPICacheResponse ::
  -- | 'facrrsResponseStatus'
  Int ->
  FlushAPICacheResponse
...
facrrsResponseStatus :: Lens' FlushAPICacheResponse Int
facrrsResponseStatus = lens _facrrsResponseStatus (\s a -> s {_facrrsResponseStatus = a})
