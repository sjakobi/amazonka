...
instance
  ...
  toJSON RejectDomainTransferFromAnotherAWSAccount' {..} =
    object
      ( catMaybes
          [Just ("DomainName" .= _rdtfaaaDomainName)]
      )
...
data RejectDomainTransferFromAnotherAWSAccountResponse = RejectDomainTransferFromAnotherAWSAccountResponse'
  { _rdtfaaarrsOperationId ::
      !( Maybe
           Text
       ),
    _rdtfaaarrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rdtfaaarrsOperationId' - The identifier that @TransferDomainToAnotherAwsAccount@ returned to track the progress of the request. Because the transfer request was rejected, the value is no longer valid, and you can't use @GetOperationDetail@ to query the operation status.
--
-- * 'rdtfaaarrsResponseStatus' - -- | The response status code.
rejectDomainTransferFromAnotherAWSAccountResponse ::
  -- | 'rdtfaaarrsResponseStatus'
  Int ->
  RejectDomainTransferFromAnotherAWSAccountResponse
...
rdtfaaarrsOperationId :: Lens' RejectDomainTransferFromAnotherAWSAccountResponse (Maybe Text)
rdtfaaarrsOperationId = lens _rdtfaaarrsOperationId (\s a -> s {_rdtfaaarrsOperationId = a})

-- | -- | The response status code.
rdtfaaarrsResponseStatus :: Lens' RejectDomainTransferFromAnotherAWSAccountResponse Int
rdtfaaarrsResponseStatus = lens _rdtfaaarrsResponseStatus (\s a -> s {_rdtfaaarrsResponseStatus = a})
