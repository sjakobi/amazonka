...
instance
  ...
  toJSON CancelDomainTransferToAnotherAWSAccount' {..} =
    object
      ( catMaybes
          [Just ("DomainName" .= _cdttaaaDomainName)]
      )
...
data CancelDomainTransferToAnotherAWSAccountResponse = CancelDomainTransferToAnotherAWSAccountResponse'
  { _cdttaaarrsOperationId ::
      !( Maybe
           Text
       ),
    _cdttaaarrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'cdttaaarrsOperationId' - The identifier that @TransferDomainToAnotherAwsAccount@ returned to track the progress of the request. Because the transfer request was canceled, the value is no longer valid, and you can't use @GetOperationDetail@ to query the operation status.
--
-- * 'cdttaaarrsResponseStatus' - -- | The response status code.
cancelDomainTransferToAnotherAWSAccountResponse ::
  -- | 'cdttaaarrsResponseStatus'
  Int ->
  CancelDomainTransferToAnotherAWSAccountResponse
...
cdttaaarrsOperationId :: Lens' CancelDomainTransferToAnotherAWSAccountResponse (Maybe Text)
cdttaaarrsOperationId = lens _cdttaaarrsOperationId (\s a -> s {_cdttaaarrsOperationId = a})

-- | -- | The response status code.
cdttaaarrsResponseStatus :: Lens' CancelDomainTransferToAnotherAWSAccountResponse Int
cdttaaarrsResponseStatus = lens _cdttaaarrsResponseStatus (\s a -> s {_cdttaaarrsResponseStatus = a})
