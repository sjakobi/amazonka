...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dwaclrrsResponseStatus' - -- | The response status code.
disassociateWebACLResponse ::
  -- | 'dwaclrrsResponseStatus'
  Int ->
  DisassociateWebACLResponse
...
dwaclrrsResponseStatus :: Lens' DisassociateWebACLResponse Int
dwaclrrsResponseStatus = lens _dwaclrrsResponseStatus (\s a -> s {_dwaclrrsResponseStatus = a})
