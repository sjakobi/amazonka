...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gisrrsIPSet' - Information about the 'IPSet' that you specified in the @GetIPSet@ request. For more information, see the following topics:     * 'IPSet' : Contains @IPSetDescriptors@ , @IPSetId@ , and @Name@      * @IPSetDescriptors@ : Contains an array of 'IPSetDescriptor' objects. Each @IPSetDescriptor@ object contains @Type@ and @Value@
--
...
gisrrsIPSet :: Lens' GetIPSetResponse (Maybe IPSet)
gisrrsIPSet = lens _gisrrsIPSet (\s a -> s {_gisrrsIPSet = a})
