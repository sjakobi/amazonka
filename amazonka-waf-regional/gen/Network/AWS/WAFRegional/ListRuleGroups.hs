...
listRuleGroups_limit = Lens.lens (\ListRuleGroups' {limit} -> limit) (\s@ListRuleGroups' {} a -> s {limit = a} :: ListRuleGroups) Prelude.. Lens.mapping Prelude._Nat
...
instance Prelude.ToHeaders ListRuleGroups where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "X-Amz-Target"
              Prelude.=# ( "AWSWAF_Regional_20161128.ListRuleGroups" ::
                             Prelude.ByteString
                         ),
            "Content-Type"
