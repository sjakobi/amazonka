...
data CreateWebACLMigrationStackResponse = CreateWebACLMigrationStackResponse'
  { _cwamsrrsResponseStatus ::
      !Int,
    _cwamsrrsS3ObjectURL ::
      !Text
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'cwamsrrsResponseStatus' - -- | The response status code.
--
-- * 'cwamsrrsS3ObjectURL' - The URL of the template created in Amazon S3.
createWebACLMigrationStackResponse ::
  -- | 'cwamsrrsResponseStatus'
  Int ->
  -- | 'cwamsrrsS3ObjectURL'
...
createWebACLMigrationStackResponse
  ...
  pS3ObjectURL_ =
    CreateWebACLMigrationStackResponse'
      { _cwamsrrsResponseStatus =
          pResponseStatus_,
        _cwamsrrsS3ObjectURL = pS3ObjectURL_
      }

-- | -- | The response status code.
cwamsrrsResponseStatus :: Lens' CreateWebACLMigrationStackResponse Int
cwamsrrsResponseStatus = lens _cwamsrrsResponseStatus (\s a -> s {_cwamsrrsResponseStatus = a})

-- | The URL of the template created in Amazon S3.
cwamsrrsS3ObjectURL :: Lens' CreateWebACLMigrationStackResponse Text
cwamsrrsS3ObjectURL = lens _cwamsrrsS3ObjectURL (\s a -> s {_cwamsrrsS3ObjectURL = a})
