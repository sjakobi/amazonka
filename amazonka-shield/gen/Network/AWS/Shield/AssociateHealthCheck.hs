...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ahcrrsResponseStatus' - -- | The response status code.
associateHealthCheckResponse ::
  -- | 'ahcrrsResponseStatus'
  Int ->
  AssociateHealthCheckResponse
...
ahcrrsResponseStatus :: Lens' AssociateHealthCheckResponse Int
ahcrrsResponseStatus = lens _ahcrrsResponseStatus (\s a -> s {_ahcrrsResponseStatus = a})
