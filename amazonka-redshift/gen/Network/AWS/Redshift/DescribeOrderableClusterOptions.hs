...
instance AWSPager DescribeOrderableClusterOptions where
  page rq rs
    | stop (rs ^. docorrsMarker) = Nothing
    | stop (rs ^. docorrsOrderableClusterOptions) =
      Nothing
    | otherwise =
      Just $ rq & docoMarker .~ rs ^. docorrsMarker
...
data DescribeOrderableClusterOptionsResponse = DescribeOrderableClusterOptionsResponse'
  { _docorrsOrderableClusterOptions ::
      !( Maybe
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'docorrsOrderableClusterOptions' - An @OrderableClusterOption@ structure containing information about orderable options for the cluster.
--
-- * 'docorrsMarker' - A value that indicates the starting point for the next set of response records in a subsequent request. If a value is returned in a response, you can retrieve the next set of records by providing this returned marker value in the @Marker@ parameter and retrying the command. If the @Marker@ field is empty, all response records have been retrieved for the request.
--
-- * 'docorrsResponseStatus' - -- | The response status code.
describeOrderableClusterOptionsResponse ::
  -- | 'docorrsResponseStatus'
  Int ->
  DescribeOrderableClusterOptionsResponse
describeOrderableClusterOptionsResponse
  pResponseStatus_ =
    DescribeOrderableClusterOptionsResponse'
      { _docorrsOrderableClusterOptions =
          Nothing,
        _docorrsMarker = Nothing,
        _docorrsResponseStatus =
          pResponseStatus_
      }

-- | An @OrderableClusterOption@ structure containing information about orderable options for the cluster.
docorrsOrderableClusterOptions :: Lens' DescribeOrderableClusterOptionsResponse [OrderableClusterOption]
docorrsOrderableClusterOptions = lens _docorrsOrderableClusterOptions (\s a -> s {_docorrsOrderableClusterOptions = a}) . _Default . _Coerce

-- | A value that indicates the starting point for the next set of response records in a subsequent request. If a value is returned in a response, you can retrieve the next set of records by providing this returned marker value in the @Marker@ parameter and retrying the command. If the @Marker@ field is empty, all response records have been retrieved for the request.
docorrsMarker :: Lens' DescribeOrderableClusterOptionsResponse (Maybe Text)
docorrsMarker = lens _docorrsMarker (\s a -> s {_docorrsMarker = a})

-- | -- | The response status code.
docorrsResponseStatus :: Lens' DescribeOrderableClusterOptionsResponse Int
docorrsResponseStatus = lens _docorrsResponseStatus (\s a -> s {_docorrsResponseStatus = a})
