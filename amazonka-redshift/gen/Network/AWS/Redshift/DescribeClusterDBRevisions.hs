...
instance AWSPager DescribeClusterDBRevisions where
  page rq rs
    | stop (rs ^. dcdrrrsMarker) = Nothing
    | stop (rs ^. dcdrrrsClusterDBRevisions) = Nothing
    | otherwise =
      Just $ rq & dcdrMarker .~ rs ^. dcdrrrsMarker
...
instance ToQuery DescribeClusterDBRevisions where
  toQuery DescribeClusterDBRevisions' {..} =
    mconcat
      [ "Action"
          =: ("DescribeClusterDbRevisions" :: ByteString),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dcdrrrsClusterDBRevisions' - A list of revisions.
--
-- * 'dcdrrrsMarker' - A string representing the starting point for the next set of revisions. If a value is returned in a response, you can retrieve the next set of revisions by providing the value in the @marker@ parameter and retrying the command. If the @marker@ field is empty, all revisions have already been returned.
--
-- * 'dcdrrrsResponseStatus' - -- | The response status code.
describeClusterDBRevisionsResponse ::
  -- | 'dcdrrrsResponseStatus'
  Int ->
  DescribeClusterDBRevisionsResponse
describeClusterDBRevisionsResponse pResponseStatus_ =
  DescribeClusterDBRevisionsResponse'
    { _dcdrrrsClusterDBRevisions =
        Nothing,
      _dcdrrrsMarker = Nothing,
      _dcdrrrsResponseStatus =
        pResponseStatus_
    }

-- | A list of revisions.
dcdrrrsClusterDBRevisions :: Lens' DescribeClusterDBRevisionsResponse [ClusterDBRevision]
dcdrrrsClusterDBRevisions = lens _dcdrrrsClusterDBRevisions (\s a -> s {_dcdrrrsClusterDBRevisions = a}) . _Default . _Coerce

-- | A string representing the starting point for the next set of revisions. If a value is returned in a response, you can retrieve the next set of revisions by providing the value in the @marker@ parameter and retrying the command. If the @marker@ field is empty, all revisions have already been returned.
dcdrrrsMarker :: Lens' DescribeClusterDBRevisionsResponse (Maybe Text)
dcdrrrsMarker = lens _dcdrrrsMarker (\s a -> s {_dcdrrrsMarker = a})

-- | -- | The response status code.
dcdrrrsResponseStatus :: Lens' DescribeClusterDBRevisionsResponse Int
dcdrrrsResponseStatus = lens _dcdrrrsResponseStatus (\s a -> s {_dcdrrrsResponseStatus = a})
