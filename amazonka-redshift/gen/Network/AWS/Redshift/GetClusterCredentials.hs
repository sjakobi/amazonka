...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gccrrsDBUser' - A database user name that is authorized to log on to the database @DbName@ using the password @DbPassword@ . If the specified DbUser exists in the database, the new user name has the same database privileges as the the user named in DbUser. By default, the user is added to PUBLIC. If the @DbGroups@ parameter is specifed, @DbUser@ is added to the listed groups for any sessions created using these credentials.
--
-- * 'gccrrsExpiration' - The date and time the password in @DbPassword@ expires.
--
-- * 'gccrrsDBPassword' - A temporary password that authorizes the user name returned by @DbUser@ to log on to the database @DbName@ .
--
...
getClusterCredentialsResponse pResponseStatus_ =
  GetClusterCredentialsResponse'
    { _gccrrsDBUser =
        Nothing,
      _gccrrsExpiration = Nothing,
      _gccrrsDBPassword = Nothing,
      _gccrrsResponseStatus = pResponseStatus_
    }

-- | A database user name that is authorized to log on to the database @DbName@ using the password @DbPassword@ . If the specified DbUser exists in the database, the new user name has the same database privileges as the the user named in DbUser. By default, the user is added to PUBLIC. If the @DbGroups@ parameter is specifed, @DbUser@ is added to the listed groups for any sessions created using these credentials.
gccrrsDBUser :: Lens' GetClusterCredentialsResponse (Maybe Text)
gccrrsDBUser = lens _gccrrsDBUser (\s a -> s {_gccrrsDBUser = a})

-- | The date and time the password in @DbPassword@ expires.
gccrrsExpiration :: Lens' GetClusterCredentialsResponse (Maybe UTCTime)
gccrrsExpiration = lens _gccrrsExpiration (\s a -> s {_gccrrsExpiration = a}) . mapping _Time

-- | A temporary password that authorizes the user name returned by @DbUser@ to log on to the database @DbName@ .
gccrrsDBPassword :: Lens' GetClusterCredentialsResponse (Maybe Text)
gccrrsDBPassword = lens _gccrrsDBPassword (\s a -> s {_gccrrsDBPassword = a}) . mapping _Sensitive

-- | -- | The response status code.
gccrrsResponseStatus :: Lens' GetClusterCredentialsResponse Int
