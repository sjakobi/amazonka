...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'psvrrsVersionStages' - The list of staging labels that are currently attached to this version of the secret. Staging labels are used to track a version as it progresses through the secret rotation process.
--
-- * 'psvrrsARN' - The Amazon Resource Name (ARN) for the secret for which you just created a version.
--
-- * 'psvrrsVersionId' - The unique identifier of the version of the secret you just created or updated.
--
-- * 'psvrrsName' - The friendly name of the secret for which you just created or updated a version.
--
-- * 'psvrrsResponseStatus' - -- | The response status code.
putSecretValueResponse ::
  -- | 'psvrrsResponseStatus'
  Int ->
  PutSecretValueResponse
putSecretValueResponse pResponseStatus_ =
  PutSecretValueResponse'
    { _psvrrsVersionStages =
        Nothing,
      _psvrrsARN = Nothing,
      _psvrrsVersionId = Nothing,
      _psvrrsName = Nothing,
      _psvrrsResponseStatus = pResponseStatus_
    }
...
psvrrsVersionStages :: Lens' PutSecretValueResponse (Maybe (NonEmpty Text))
psvrrsVersionStages = lens _psvrrsVersionStages (\s a -> s {_psvrrsVersionStages = a}) . mapping _List1

-- | The Amazon Resource Name (ARN) for the secret for which you just created a version.
psvrrsARN :: Lens' PutSecretValueResponse (Maybe Text)
psvrrsARN = lens _psvrrsARN (\s a -> s {_psvrrsARN = a})

-- | The unique identifier of the version of the secret you just created or updated.
psvrrsVersionId :: Lens' PutSecretValueResponse (Maybe Text)
psvrrsVersionId = lens _psvrrsVersionId (\s a -> s {_psvrrsVersionId = a})

-- | The friendly name of the secret for which you just created or updated a version.
psvrrsName :: Lens' PutSecretValueResponse (Maybe Text)
psvrrsName = lens _psvrrsName (\s a -> s {_psvrrsName = a})

-- | -- | The response status code.
psvrrsResponseStatus :: Lens' PutSecretValueResponse Int
psvrrsResponseStatus = lens _psvrrsResponseStatus (\s a -> s {_psvrrsResponseStatus = a})
