...
--
-- __Required permissions__ : <https://docs.aws.amazon.com/kms/latest/developerguide/kms-api-permissions-reference.html kms:ImportKeyMaterial> (key policy)
--
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ikmrrsResponseStatus' - -- | The response status code.
importKeyMaterialResponse ::
  -- | 'ikmrrsResponseStatus'
  Int ->
  ImportKeyMaterialResponse
...
ikmrrsResponseStatus :: Lens' ImportKeyMaterialResponse Int
ikmrrsResponseStatus = lens _ikmrrsResponseStatus (\s a -> s {_ikmrrsResponseStatus = a})
