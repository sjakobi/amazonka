...
--
-- __Required permissions__ : <https://docs.aws.amazon.com/kms/latest/developerguide/kms-api-permissions-reference.html kms:UpdateCustomKeyStore> (IAM policy)
--
...
data UpdateCustomKeyStore = UpdateCustomKeyStore'
  { _ucksKeyStorePassword ::
    ...
    _ucksNewCustomKeyStoreName ::
      !(Maybe Text),
    _ucksCloudHSMClusterId ::
      !(Maybe Text),
    _ucksCustomKeyStoreId ::
      !Text
  }
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ucksrrsResponseStatus' - -- | The response status code.
updateCustomKeyStoreResponse ::
  -- | 'ucksrrsResponseStatus'
  Int ->
  UpdateCustomKeyStoreResponse
...
ucksrrsResponseStatus :: Lens' UpdateCustomKeyStoreResponse Int
ucksrrsResponseStatus = lens _ucksrrsResponseStatus (\s a -> s {_ucksrrsResponseStatus = a})
