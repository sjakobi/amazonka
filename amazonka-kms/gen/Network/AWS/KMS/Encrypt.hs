...
--
-- __Required permissions__ : <https://docs.aws.amazon.com/kms/latest/developerguide/kms-api-permissions-reference.html kms:Encrypt> (key policy)
--
...
data EncryptResponse = EncryptResponse'
  { _errsEncryptionAlgorithm ::
      !(Maybe EncryptionAlgorithmSpec),
    _errsCiphertextBlob :: !(Maybe Base64),
    _errsKeyId :: !(Maybe Text),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'errsEncryptionAlgorithm' - The encryption algorithm that was used to encrypt the plaintext.
--
-- * 'errsCiphertextBlob' - The encrypted plaintext. When you use the HTTP API or the AWS CLI, the value is Base64-encoded. Otherwise, it is not Base64-encoded.-- /Note:/ This 'Lens' automatically encodes and decodes Base64 data. The underlying isomorphism will encode to Base64 representation during serialisation, and decode from Base64 representation during deserialisation. This 'Lens' accepts and returns only raw unencoded data.
--
-- * 'errsKeyId' - The Amazon Resource Name (<https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-key-ARN key ARN> ) of the CMK that was used to encrypt the plaintext.
--
...
encryptResponse pResponseStatus_ =
  EncryptResponse'
    { _errsEncryptionAlgorithm =
        Nothing,
      _errsCiphertextBlob = Nothing,
      _errsKeyId = Nothing,
      _errsResponseStatus = pResponseStatus_
    }

-- | The encryption algorithm that was used to encrypt the plaintext.
errsEncryptionAlgorithm :: Lens' EncryptResponse (Maybe EncryptionAlgorithmSpec)
errsEncryptionAlgorithm = lens _errsEncryptionAlgorithm (\s a -> s {_errsEncryptionAlgorithm = a})

-- | The encrypted plaintext. When you use the HTTP API or the AWS CLI, the value is Base64-encoded. Otherwise, it is not Base64-encoded.-- /Note:/ This 'Lens' automatically encodes and decodes Base64 data. The underlying isomorphism will encode to Base64 representation during serialisation, and decode from Base64 representation during deserialisation. This 'Lens' accepts and returns only raw unencoded data.
errsCiphertextBlob :: Lens' EncryptResponse (Maybe ByteString)
errsCiphertextBlob = lens _errsCiphertextBlob (\s a -> s {_errsCiphertextBlob = a}) . mapping _Base64
...
errsKeyId :: Lens' EncryptResponse (Maybe Text)
errsKeyId = lens _errsKeyId (\s a -> s {_errsKeyId = a})

-- | -- | The response status code.
errsResponseStatus :: Lens' EncryptResponse Int
