...
--
-- __Required permissions__ : <https://docs.aws.amazon.com/kms/latest/developerguide/kms-api-permissions-reference.html kms:DescribeKey> (key policy)
--
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dkrrsKeyMetadata' - Metadata associated with the key.
--
-- * 'dkrrsResponseStatus' - -- | The response status code.
describeKeyResponse ::
  -- | 'dkrrsResponseStatus'
  Int ->
  DescribeKeyResponse
describeKeyResponse pResponseStatus_ =
  DescribeKeyResponse'
    { _dkrrsKeyMetadata = Nothing,
      _dkrrsResponseStatus = pResponseStatus_
    }

-- | Metadata associated with the key.
dkrrsKeyMetadata :: Lens' DescribeKeyResponse (Maybe KeyMetadata)
dkrrsKeyMetadata = lens _dkrrsKeyMetadata (\s a -> s {_dkrrsKeyMetadata = a})

-- | -- | The response status code.
dkrrsResponseStatus :: Lens' DescribeKeyResponse Int
dkrrsResponseStatus = lens _dkrrsResponseStatus (\s a -> s {_dkrrsResponseStatus = a})
