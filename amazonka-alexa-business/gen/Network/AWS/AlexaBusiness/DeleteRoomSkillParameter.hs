...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'drsprrsResponseStatus' - -- | The response status code.
deleteRoomSkillParameterResponse ::
  -- | 'drsprrsResponseStatus'
  Int ->
  DeleteRoomSkillParameterResponse
...
drsprrsResponseStatus :: Lens' DeleteRoomSkillParameterResponse Int
drsprrsResponseStatus = lens _drsprrsResponseStatus (\s a -> s {_drsprrsResponseStatus = a})
