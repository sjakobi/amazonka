...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'uabrrsResponseStatus' - -- | The response status code.
updateAddressBookResponse ::
  -- | 'uabrrsResponseStatus'
  Int ->
  UpdateAddressBookResponse
...
uabrrsResponseStatus :: Lens' UpdateAddressBookResponse Int
uabrrsResponseStatus = lens _uabrrsResponseStatus (\s a -> s {_uabrrsResponseStatus = a})
