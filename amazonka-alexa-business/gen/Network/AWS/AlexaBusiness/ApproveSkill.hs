...
approveSkill ::
  ...
  Text ->
  ApproveSkill
approveSkill pSkillId_ =
  ApproveSkill' {_asSkillId = pSkillId_}
...
instance AWSRequest ApproveSkill where
  ...
  response =
    receiveEmpty
      ( \s h x ->
          ApproveSkillResponse' <$> (pure (fromEnum s))
      )
...
instance ToHeaders ApproveSkill where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AlexaForBusiness.ApproveSkill" :: ByteString),
            "Content-Type"
...
asrrsResponseStatus :: Lens' ApproveSkillResponse Int
