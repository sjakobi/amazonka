...
--
-- * 'ussCallAs' - [Service-managed permissions] Specifies whether you are acting as an account administrator in the organization's management account or as a delegated administrator in a member account. By default, @SELF@ is specified. Use @SELF@ for stack sets with self-managed permissions.     * If you are signed in to the management account, specify @SELF@ .     * If you are signed in to a delegated administrator account, specify @DELEGATED_ADMIN@ . Your AWS account must be registered as a delegated administrator in the management account. For more information, see <https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/stacksets-orgs-delegated-admin.html Register a delegated administrator> in the /AWS CloudFormation User Guide/ .
--
...
ussCallAs :: Lens' UpdateStackSet (Maybe CallAs)
ussCallAs = lens _ussCallAs (\s a -> s {_ussCallAs = a})
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ussrrsOperationId' - The unique ID for this stack set operation.
--
...
ussrrsOperationId :: Lens' UpdateStackSetResponse (Maybe Text)
ussrrsOperationId = lens _ussrrsOperationId (\s a -> s {_ussrrsOperationId = a})

-- | -- | The response status code.
ussrrsResponseStatus :: Lens' UpdateStackSetResponse Int
