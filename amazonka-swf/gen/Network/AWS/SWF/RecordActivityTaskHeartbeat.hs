...
data RecordActivityTaskHeartbeatResponse = RecordActivityTaskHeartbeatResponse'
  { _rathrrsResponseStatus ::
      !Int,
    _rathrrsCancelRequested ::
      !Bool
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rathrrsResponseStatus' - -- | The response status code.
--
-- * 'rathrrsCancelRequested' - Set to @true@ if cancellation of the task is requested.
recordActivityTaskHeartbeatResponse ::
  -- | 'rathrrsResponseStatus'
  Int ->
  -- | 'rathrrsCancelRequested'
...
rathrrsResponseStatus :: Lens' RecordActivityTaskHeartbeatResponse Int
rathrrsResponseStatus = lens _rathrrsResponseStatus (\s a -> s {_rathrrsResponseStatus = a})

-- | Set to @true@ if cancellation of the task is requested.
rathrrsCancelRequested :: Lens' RecordActivityTaskHeartbeatResponse Bool
rathrrsCancelRequested = lens _rathrrsCancelRequested (\s a -> s {_rathrrsCancelRequested = a})
