...
instance AWSPager GetWorkflowExecutionHistory where
  page rq rs
    | stop (rs ^. gwehrrsNextPageToken) = Nothing
    | stop (rs ^. gwehrrsEvents) = Nothing
    | otherwise =
      Just $
        rq
          & gwehNextPageToken .~ rs ^. gwehrrsNextPageToken
...
data GetWorkflowExecutionHistoryResponse = GetWorkflowExecutionHistoryResponse'
  { _gwehrrsNextPageToken ::
      !( Maybe
           Text
       ),
    _gwehrrsResponseStatus ::
      !Int,
    _gwehrrsEvents ::
      ![HistoryEvent]
  }
...
getWorkflowExecutionHistoryResponse pResponseStatus_ =
  GetWorkflowExecutionHistoryResponse'
    { _gwehrrsNextPageToken =
      ...
      _gwehrrsResponseStatus =
        pResponseStatus_,
      _gwehrrsEvents = mempty
    }

-- | If a @NextPageToken@ was returned by a previous call, there are more results available. To retrieve the next page of results, make the call again using the returned token in @nextPageToken@ . Keep all other arguments unchanged. The configured @maximumPageSize@ determines how many results can be returned in a single call.
gwehrrsNextPageToken :: Lens' GetWorkflowExecutionHistoryResponse (Maybe Text)
gwehrrsNextPageToken = lens _gwehrrsNextPageToken (\s a -> s {_gwehrrsNextPageToken = a})

-- | -- | The response status code.
gwehrrsResponseStatus :: Lens' GetWorkflowExecutionHistoryResponse Int
gwehrrsResponseStatus = lens _gwehrrsResponseStatus (\s a -> s {_gwehrrsResponseStatus = a})

-- | The list of history events.
gwehrrsEvents :: Lens' GetWorkflowExecutionHistoryResponse [HistoryEvent]
gwehrrsEvents = lens _gwehrrsEvents (\s a -> s {_gwehrrsEvents = a}) . _Coerce
