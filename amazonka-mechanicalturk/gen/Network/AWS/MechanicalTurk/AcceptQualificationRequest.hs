...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'aqrrrsResponseStatus' - -- | The response status code.
acceptQualificationRequestResponse ::
  -- | 'aqrrrsResponseStatus'
  Int ->
  AcceptQualificationRequestResponse
...
aqrrrsResponseStatus :: Lens' AcceptQualificationRequestResponse Int
aqrrrsResponseStatus = lens _aqrrrsResponseStatus (\s a -> s {_aqrrrsResponseStatus = a})
