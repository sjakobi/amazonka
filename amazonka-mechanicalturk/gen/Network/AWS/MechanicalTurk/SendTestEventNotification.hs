...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'stenrrsResponseStatus' - -- | The response status code.
sendTestEventNotificationResponse ::
  -- | 'stenrrsResponseStatus'
  Int ->
  SendTestEventNotificationResponse
...
stenrrsResponseStatus :: Lens' SendTestEventNotificationResponse Int
stenrrsResponseStatus = lens _stenrrsResponseStatus (\s a -> s {_stenrrsResponseStatus = a})
