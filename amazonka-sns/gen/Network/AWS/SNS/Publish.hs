...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'prrsSequenceNumber' - This response element applies only to FIFO (first-in-first-out) topics.  The sequence number is a large, non-consecutive number that Amazon SNS assigns to each message. The length of @SequenceNumber@ is 128 bits. @SequenceNumber@ continues to increase for each @MessageGroupId@ .
--
-- * 'prrsMessageId' - Unique identifier assigned to the published message. Length Constraint: Maximum 100 characters
--
...
publishResponse pResponseStatus_ =
  PublishResponse'
    { _prrsSequenceNumber = Nothing,
      _prrsMessageId = Nothing,
...
prrsSequenceNumber :: Lens' PublishResponse (Maybe Text)
...
prrsMessageId :: Lens' PublishResponse (Maybe Text)
prrsMessageId = lens _prrsMessageId (\s a -> s {_prrsMessageId = a})

-- | -- | The response status code.
prrsResponseStatus :: Lens' PublishResponse Int
