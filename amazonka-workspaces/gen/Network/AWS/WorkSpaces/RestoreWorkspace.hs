...
instance AWSRequest RestoreWorkspace where
  ...
  response =
    receiveEmpty
      ( \s h x ->
          RestoreWorkspaceResponse' <$> (pure (fromEnum s))
      )
...
instance ToJSON RestoreWorkspace where
  toJSON RestoreWorkspace' {..} =
    object
      (catMaybes [Just ("WorkspaceId" .= _rwWorkspaceId)])
...
rwrrsResponseStatus :: Lens' RestoreWorkspaceResponse Int
