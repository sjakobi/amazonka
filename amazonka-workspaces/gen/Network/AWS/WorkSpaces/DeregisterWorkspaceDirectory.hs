...
instance ToJSON DeregisterWorkspaceDirectory where
  toJSON DeregisterWorkspaceDirectory' {..} =
    object
      (catMaybes [Just ("DirectoryId" .= _dwdDirectoryId)])
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dwdrrsResponseStatus' - -- | The response status code.
deregisterWorkspaceDirectoryResponse ::
  -- | 'dwdrrsResponseStatus'
  Int ->
  DeregisterWorkspaceDirectoryResponse
...
dwdrrsResponseStatus :: Lens' DeregisterWorkspaceDirectoryResponse Int
dwdrrsResponseStatus = lens _dwdrrsResponseStatus (\s a -> s {_dwdrrsResponseStatus = a})
