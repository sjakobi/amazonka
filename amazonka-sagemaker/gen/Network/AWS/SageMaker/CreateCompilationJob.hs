...
--
-- * 'ccjTags' - An array of key-value pairs. You can use tags to categorize your AWS resources in different ways, for example, by purpose, owner, or environment. For more information, see <https://docs.aws.amazon.com/general/latest/gr/aws_tagging.html Tagging AWS Resources> .
--
...
--
-- * 'ccjStoppingCondition' - Specifies a limit to how long a model compilation job can run. When the job reaches the time limit, Amazon SageMaker ends the compilation job. Use this API to cap model training costs.
createCompilationJob ::
  -- | 'ccjCompilationJobName'
  Text ->
  -- | 'ccjRoleARN'
...
ccjTags :: Lens' CreateCompilationJob [Tag]
ccjTags = lens _ccjTags (\s a -> s {_ccjTags = a}) . _Default . _Coerce
...
instance Hashable CreateCompilationJob

instance NFData CreateCompilationJob
...
data CreateCompilationJobResponse = CreateCompilationJobResponse'
  { _ccjrrsResponseStatus ::
      !Int,
    _ccjrrsCompilationJobARN ::
      !Text
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ccjrrsResponseStatus' - -- | The response status code.
--
-- * 'ccjrrsCompilationJobARN' - If the action is successful, the service sends back an HTTP 200 response. Amazon SageMaker returns the following data in JSON format:     * @CompilationJobArn@ : The Amazon Resource Name (ARN) of the compiled job.
createCompilationJobResponse ::
  -- | 'ccjrrsResponseStatus'
  Int ->
  -- | 'ccjrrsCompilationJobARN'
...
ccjrrsResponseStatus :: Lens' CreateCompilationJobResponse Int
ccjrrsResponseStatus = lens _ccjrrsResponseStatus (\s a -> s {_ccjrrsResponseStatus = a})
...
ccjrrsCompilationJobARN :: Lens' CreateCompilationJobResponse Text
ccjrrsCompilationJobARN = lens _ccjrrsCompilationJobARN (\s a -> s {_ccjrrsCompilationJobARN = a})

instance NFData CreateCompilationJobResponse
