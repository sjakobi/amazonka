...
module Network.AWS.Route53AutoNaming.CreateHTTPNamespace
  ( -- * Creating a Request
    ...
    -- * Response Lenses
    chttpnrrsOperationId,
    chttpnrrsResponseStatus,
  )
where
...
data CreateHTTPNamespaceResponse = CreateHTTPNamespaceResponse'
  { _chttpnrrsOperationId ::
      !(Maybe Text),
    _chttpnrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'chttpnrrsOperationId' - A value that you can use to determine whether the request completed successfully. To get the status of the operation, see <https://docs.aws.amazon.com/cloud-map/latest/api/API_GetOperation.html GetOperation> .
--
-- * 'chttpnrrsResponseStatus' - -- | The response status code.
createHTTPNamespaceResponse ::
  -- | 'chttpnrrsResponseStatus'
  Int ->
  CreateHTTPNamespaceResponse
createHTTPNamespaceResponse pResponseStatus_ =
  CreateHTTPNamespaceResponse'
    { _chttpnrrsOperationId =
        Nothing,
      _chttpnrrsResponseStatus = pResponseStatus_
    }
...
chttpnrrsOperationId :: Lens' CreateHTTPNamespaceResponse (Maybe Text)
chttpnrrsOperationId = lens _chttpnrrsOperationId (\s a -> s {_chttpnrrsOperationId = a})

-- | -- | The response status code.
chttpnrrsResponseStatus :: Lens' CreateHTTPNamespaceResponse Int
chttpnrrsResponseStatus = lens _chttpnrrsResponseStatus (\s a -> s {_chttpnrrsResponseStatus = a})
