...
instance AWSPager DescribePullRequestEvents where
  page rq rs
    | stop (rs ^. dprerrsNextToken) = Nothing
    | stop (rs ^. dprerrsPullRequestEvents) = Nothing
    | otherwise =
      Just $ rq & dpreNextToken .~ rs ^. dprerrsNextToken
...
instance Hashable DescribePullRequestEvents

instance NFData DescribePullRequestEvents
...
data DescribePullRequestEventsResponse = DescribePullRequestEventsResponse'
  { _dprerrsNextToken ::
    ...
    _dprerrsResponseStatus ::
      !Int,
    _dprerrsPullRequestEvents ::
      ![PullRequestEvent]
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dprerrsNextToken' - An enumeration token that can be used in a request to return the next batch of the results.
--
-- * 'dprerrsResponseStatus' - -- | The response status code.
--
-- * 'dprerrsPullRequestEvents' - Information about the pull request events.
describePullRequestEventsResponse ::
  -- | 'dprerrsResponseStatus'
  Int ->
  DescribePullRequestEventsResponse
describePullRequestEventsResponse pResponseStatus_ =
  DescribePullRequestEventsResponse'
    { _dprerrsNextToken =
      ...
      _dprerrsResponseStatus =
        pResponseStatus_,
      _dprerrsPullRequestEvents = mempty
    }

-- | An enumeration token that can be used in a request to return the next batch of the results.
dprerrsNextToken :: Lens' DescribePullRequestEventsResponse (Maybe Text)
dprerrsNextToken = lens _dprerrsNextToken (\s a -> s {_dprerrsNextToken = a})

-- | -- | The response status code.
dprerrsResponseStatus :: Lens' DescribePullRequestEventsResponse Int
dprerrsResponseStatus = lens _dprerrsResponseStatus (\s a -> s {_dprerrsResponseStatus = a})

-- | Information about the pull request events.
dprerrsPullRequestEvents :: Lens' DescribePullRequestEventsResponse [PullRequestEvent]
dprerrsPullRequestEvents = lens _dprerrsPullRequestEvents (\s a -> s {_dprerrsPullRequestEvents = a}) . _Coerce
