...
--
-- * 'gvnAccountId' - The @AccountId@ value is the AWS account ID of the account that owns the vault. You can either specify an AWS account ID or optionally a single '@-@ ' (hyphen), in which case Amazon S3 Glacier uses the AWS account ID associated with the credentials used to sign the request. If you use an account ID, do not include any hyphens ('-') in the ID.
--
...
data GetVaultNotificationsResponse = GetVaultNotificationsResponse'
  { _gvnrrsVaultNotificationConfig ::
      !( Maybe
           VaultNotificationConfig
       ),
    _gvnrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gvnrrsVaultNotificationConfig' - Returns the notification configuration set on the vault.
--
-- * 'gvnrrsResponseStatus' - -- | The response status code.
getVaultNotificationsResponse ::
  -- | 'gvnrrsResponseStatus'
  Int ->
  GetVaultNotificationsResponse
getVaultNotificationsResponse pResponseStatus_ =
  GetVaultNotificationsResponse'
    { _gvnrrsVaultNotificationConfig =
        Nothing,
      _gvnrrsResponseStatus = pResponseStatus_
    }

-- | Returns the notification configuration set on the vault.
gvnrrsVaultNotificationConfig :: Lens' GetVaultNotificationsResponse (Maybe VaultNotificationConfig)
gvnrrsVaultNotificationConfig = lens _gvnrrsVaultNotificationConfig (\s a -> s {_gvnrrsVaultNotificationConfig = a})

-- | -- | The response status code.
gvnrrsResponseStatus :: Lens' GetVaultNotificationsResponse Int
gvnrrsResponseStatus = lens _gvnrrsResponseStatus (\s a -> s {_gvnrrsResponseStatus = a})
