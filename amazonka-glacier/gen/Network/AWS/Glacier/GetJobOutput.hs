...
--
-- * 'gjoAccountId' - The @AccountId@ value is the AWS account ID of the account that owns the vault. You can either specify an AWS account ID or optionally a single '@-@ ' (hyphen), in which case Amazon S3 Glacier uses the AWS account ID associated with the credentials used to sign the request. If you use an account ID, do not include any hyphens ('-') in the ID.
--
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gjorrsContentType' - The Content-Type depends on whether the job output is an archive or a vault inventory. For archive data, the Content-Type is application/octet-stream. For vault inventory, if you requested CSV format when you initiated the job, the Content-Type is text/csv. Otherwise, by default, vault inventory is returned as JSON, and the Content-Type is application/json.
--
-- * 'gjorrsContentRange' - The range of bytes returned by Amazon S3 Glacier. If only partial output is downloaded, the response provides the range of bytes Amazon S3 Glacier returned. For example, bytes 0-1048575/8388608 returns the first 1 MB from 8 MB.
--
-- * 'gjorrsArchiveDescription' - The description of an archive.
--
-- * 'gjorrsAcceptRanges' - Indicates the range units accepted. For more information, see <http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html RFC2616> .
--
-- * 'gjorrsChecksum' - The checksum of the data in the response. This header is returned only when retrieving the output for an archive retrieval job. Furthermore, this header appears only under the following conditions:     * You get the entire range of the archive.     * You request a range to return of the archive that starts and ends on a multiple of 1 MB. For example, if you have an 3.1 MB archive and you specify a range to return that starts at 1 MB and ends at 2 MB, then the x-amz-sha256-tree-hash is returned as a response header.     * You request a range of the archive to return that starts on a multiple of 1 MB and goes to the end of the archive. For example, if you have a 3.1 MB archive and you specify a range that starts at 2 MB and ends at 3.1 MB (the end of the archive), then the x-amz-sha256-tree-hash is returned as a response header.
--
-- * 'gjorrsStatus' - The HTTP response code for a job output request. The value depends on whether a range was specified in the request.
--
-- * 'gjorrsBody' - The job data, either archive data or inventory data.
getJobOutputResponse ::
  -- | 'gjorrsStatus'
  Int ->
  -- | 'gjorrsBody'
...
getJobOutputResponse pStatus_ pBody_ =
  GetJobOutputResponse'
    { _gjorrsContentType = Nothing,
      _gjorrsContentRange = Nothing,
      _gjorrsArchiveDescription = Nothing,
      _gjorrsAcceptRanges = Nothing,
      _gjorrsChecksum = Nothing,
      _gjorrsStatus = pStatus_,
      _gjorrsBody = pBody_
    }

-- | The Content-Type depends on whether the job output is an archive or a vault inventory. For archive data, the Content-Type is application/octet-stream. For vault inventory, if you requested CSV format when you initiated the job, the Content-Type is text/csv. Otherwise, by default, vault inventory is returned as JSON, and the Content-Type is application/json.
gjorrsContentType :: Lens' GetJobOutputResponse (Maybe Text)
gjorrsContentType = lens _gjorrsContentType (\s a -> s {_gjorrsContentType = a})
...
gjorrsContentRange :: Lens' GetJobOutputResponse (Maybe Text)
gjorrsContentRange = lens _gjorrsContentRange (\s a -> s {_gjorrsContentRange = a})

-- | The description of an archive.
gjorrsArchiveDescription :: Lens' GetJobOutputResponse (Maybe Text)
gjorrsArchiveDescription = lens _gjorrsArchiveDescription (\s a -> s {_gjorrsArchiveDescription = a})

-- | Indicates the range units accepted. For more information, see <http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html RFC2616> .
gjorrsAcceptRanges :: Lens' GetJobOutputResponse (Maybe Text)
gjorrsAcceptRanges = lens _gjorrsAcceptRanges (\s a -> s {_gjorrsAcceptRanges = a})

-- | The checksum of the data in the response. This header is returned only when retrieving the output for an archive retrieval job. Furthermore, this header appears only under the following conditions:     * You get the entire range of the archive.     * You request a range to return of the archive that starts and ends on a multiple of 1 MB. For example, if you have an 3.1 MB archive and you specify a range to return that starts at 1 MB and ends at 2 MB, then the x-amz-sha256-tree-hash is returned as a response header.     * You request a range of the archive to return that starts on a multiple of 1 MB and goes to the end of the archive. For example, if you have a 3.1 MB archive and you specify a range that starts at 2 MB and ends at 3.1 MB (the end of the archive), then the x-amz-sha256-tree-hash is returned as a response header.
gjorrsChecksum :: Lens' GetJobOutputResponse (Maybe Text)
gjorrsChecksum = lens _gjorrsChecksum (\s a -> s {_gjorrsChecksum = a})

-- | The HTTP response code for a job output request. The value depends on whether a range was specified in the request.
gjorrsStatus :: Lens' GetJobOutputResponse Int
gjorrsStatus = lens _gjorrsStatus (\s a -> s {_gjorrsStatus = a})

-- | The job data, either archive data or inventory data.
gjorrsBody :: Lens' GetJobOutputResponse RsBody
gjorrsBody = lens _gjorrsBody (\s a -> s {_gjorrsBody = a})
