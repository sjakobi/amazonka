...
instance AWSPager ListFindings where
  page rq rs
    ...
    | stop (rs ^. lfrrsFindingARNs) = Nothing
    | otherwise =
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lfrrsNextToken' - When a response is generated, if there is more data to be listed, this parameter is present in the response and contains the value to use for the __nextToken__ parameter in a subsequent pagination request. If there is no more data to be listed, this parameter is set to null.
--
-- * 'lfrrsResponseStatus' - -- | The response status code.
--
-- * 'lfrrsFindingARNs' - A list of ARNs that specifies the findings returned by the action.
listFindingsResponse ::
  -- | 'lfrrsResponseStatus'
  Int ->
  ListFindingsResponse
...
lfrrsFindingARNs :: Lens' ListFindingsResponse [Text]
lfrrsFindingARNs = lens _lfrrsFindingARNs (\s a -> s {_lfrrsFindingARNs = a}) . _Coerce
