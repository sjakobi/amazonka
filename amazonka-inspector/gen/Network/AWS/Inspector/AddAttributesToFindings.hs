...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'aatfrrsResponseStatus' - -- | The response status code.
--
-- * 'aatfrrsFailedItems' - Attribute details that cannot be described. An error code is provided for each failed item.
addAttributesToFindingsResponse ::
  -- | 'aatfrrsResponseStatus'
  Int ->
  AddAttributesToFindingsResponse
addAttributesToFindingsResponse pResponseStatus_ =
  AddAttributesToFindingsResponse'
    { _aatfrrsResponseStatus =
        pResponseStatus_,
      _aatfrrsFailedItems = mempty
    }

-- | -- | The response status code.
aatfrrsResponseStatus :: Lens' AddAttributesToFindingsResponse Int
aatfrrsResponseStatus = lens _aatfrrsResponseStatus (\s a -> s {_aatfrrsResponseStatus = a})

-- | Attribute details that cannot be described. An error code is provided for each failed item.
aatfrrsFailedItems :: Lens' AddAttributesToFindingsResponse (HashMap Text FailedItemDetails)
aatfrrsFailedItems = lens _aatfrrsFailedItems (\s a -> s {_aatfrrsFailedItems = a}) . _Map
