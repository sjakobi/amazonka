...
data NotifyWhenUploadedResponse = NotifyWhenUploadedResponse'
  { _nwurrsFileShareARN ::
      !(Maybe Text),
    _nwurrsNotificationId ::
      !(Maybe Text),
    _nwurrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'nwurrsFileShareARN' - Undocumented member.
--
-- * 'nwurrsNotificationId' - Undocumented member.
--
-- * 'nwurrsResponseStatus' - -- | The response status code.
notifyWhenUploadedResponse ::
  -- | 'nwurrsResponseStatus'
  Int ->
  NotifyWhenUploadedResponse
notifyWhenUploadedResponse pResponseStatus_ =
  NotifyWhenUploadedResponse'
    { _nwurrsFileShareARN =
        Nothing,
      _nwurrsNotificationId = Nothing,
      _nwurrsResponseStatus = pResponseStatus_
    }

-- | Undocumented member.
nwurrsFileShareARN :: Lens' NotifyWhenUploadedResponse (Maybe Text)
nwurrsFileShareARN = lens _nwurrsFileShareARN (\s a -> s {_nwurrsFileShareARN = a})

-- | Undocumented member.
nwurrsNotificationId :: Lens' NotifyWhenUploadedResponse (Maybe Text)
nwurrsNotificationId = lens _nwurrsNotificationId (\s a -> s {_nwurrsNotificationId = a})

-- | -- | The response status code.
nwurrsResponseStatus :: Lens' NotifyWhenUploadedResponse Int
nwurrsResponseStatus = lens _nwurrsResponseStatus (\s a -> s {_nwurrsResponseStatus = a})
