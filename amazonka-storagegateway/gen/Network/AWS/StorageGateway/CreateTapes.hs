...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ctrrsTapeARNs' - A list of unique Amazon Resource Names (ARNs) that represents the virtual tapes that were created.
--
...
createTapesResponse pResponseStatus_ =
  CreateTapesResponse'
    { _ctrrsTapeARNs = Nothing,
      _ctrrsResponseStatus = pResponseStatus_
    }

-- | A list of unique Amazon Resource Names (ARNs) that represents the virtual tapes that were created.
ctrrsTapeARNs :: Lens' CreateTapesResponse [Text]
ctrrsTapeARNs = lens _ctrrsTapeARNs (\s a -> s {_ctrrsTapeARNs = a}) . _Default . _Coerce

-- | -- | The response status code.
ctrrsResponseStatus :: Lens' CreateTapesResponse Int
