...
instance AWSPager GetGatewayResponses where
  page rq rs
    | stop (rs ^. ggrrrsPosition) = Nothing
    | stop (rs ^. ggrrrsItems) = Nothing
    | otherwise =
      Just $ rq & ggrPosition .~ rs ^. ggrrrsPosition
...
instance ToQuery GetGatewayResponses where
  toQuery GetGatewayResponses' {..} =
    mconcat
      ["position" =: _ggrPosition, "limit" =: _ggrLimit]
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ggrrrsItems' - Returns the entire collection, because of no pagination support.
--
-- * 'ggrrrsPosition' - Undocumented member.
--
-- * 'ggrrrsResponseStatus' - -- | The response status code.
getGatewayResponsesResponse ::
  -- | 'ggrrrsResponseStatus'
  Int ->
  GetGatewayResponsesResponse
getGatewayResponsesResponse pResponseStatus_ =
  GetGatewayResponsesResponse'
    { _ggrrrsItems =
        Nothing,
      _ggrrrsPosition = Nothing,
      _ggrrrsResponseStatus = pResponseStatus_
    }

-- | Returns the entire collection, because of no pagination support.
ggrrrsItems :: Lens' GetGatewayResponsesResponse [GatewayResponse]
ggrrrsItems = lens _ggrrrsItems (\s a -> s {_ggrrrsItems = a}) . _Default . _Coerce

-- | Undocumented member.
ggrrrsPosition :: Lens' GetGatewayResponsesResponse (Maybe Text)
ggrrrsPosition = lens _ggrrrsPosition (\s a -> s {_ggrrrsPosition = a})

-- | -- | The response status code.
ggrrrsResponseStatus :: Lens' GetGatewayResponsesResponse Int
ggrrrsResponseStatus = lens _ggrrrsResponseStatus (\s a -> s {_ggrrrsResponseStatus = a})
