...
data PutDashboardResponse = PutDashboardResponse'
  { _pdrrsDashboardValidationMessages ::
      !( Maybe
           [DashboardValidationMessage]
       ),
    _pdrrsResponseStatus :: !Int
  }
  deriving (Eq, Read, Show, Data, Typeable, Generic)
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'pdrrsDashboardValidationMessages' - If the input for @PutDashboard@ was correct and the dashboard was successfully created or modified, this result is empty. If this result includes only warning messages, then the input was valid enough for the dashboard to be created or modified, but some elements of the dashboard might not render. If this result includes error messages, the input was not valid and the operation failed.
--
-- * 'pdrrsResponseStatus' - -- | The response status code.
putDashboardResponse ::
  -- | 'pdrrsResponseStatus'
  Int ->
  PutDashboardResponse
putDashboardResponse pResponseStatus_ =
  PutDashboardResponse'
    { _pdrrsDashboardValidationMessages =
        Nothing,
      _pdrrsResponseStatus = pResponseStatus_
    }
...
pdrrsDashboardValidationMessages :: Lens' PutDashboardResponse [DashboardValidationMessage]
pdrrsDashboardValidationMessages = lens _pdrrsDashboardValidationMessages (\s a -> s {_pdrrsDashboardValidationMessages = a}) . _Default . _Coerce

-- | -- | The response status code.
pdrrsResponseStatus :: Lens' PutDashboardResponse Int
pdrrsResponseStatus = lens _pdrrsResponseStatus (\s a -> s {_pdrrsResponseStatus = a})
