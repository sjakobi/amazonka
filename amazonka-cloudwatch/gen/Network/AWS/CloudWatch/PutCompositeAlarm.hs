...
--
-- * 'pcaAlarmActions' - The actions to execute when this alarm transitions to the @ALARM@ state from any other state. Each action is specified as an Amazon Resource Name (ARN). Valid Values: @arn:aws:sns:/region/ :/account-id/ :/sns-topic-name/ @ | @arn:aws:ssm:/region/ :/account-id/ :opsitem:/severity/ @
--
  ...
  toQuery PutCompositeAlarm' {..} =
    mconcat
      [ "Action" =: ("PutCompositeAlarm" :: ByteString),
        ...
        "Tags"
          =: toQuery (toQueryList "member" <$> _pcaTags),
        "OKActions"
          =: toQuery (toQueryList "member" <$> _pcaOKActions),
