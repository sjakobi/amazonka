...
--
-- * 'samatTarget' - Specifies the audit findings to which the mitigation actions are applied. You can apply them to a type of audit check, to all findings from an audit, or to a specific set of findings.
--
    ...
    object
      ( catMaybes
          [ Just ("target" .= _samatTarget),
            ...
            Just
              ("clientRequestToken" .= _samatClientRequestToken)
          ]
      )
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'samatrrsTaskId' - The unique identifier for the audit mitigation task. This matches the @taskId@ that you specified in the request.
--
-- * 'samatrrsResponseStatus' - -- | The response status code.
startAuditMitigationActionsTaskResponse ::
  -- | 'samatrrsResponseStatus'
  Int ->
  StartAuditMitigationActionsTaskResponse
...
samatrrsTaskId :: Lens' StartAuditMitigationActionsTaskResponse (Maybe Text)
samatrrsTaskId = lens _samatrrsTaskId (\s a -> s {_samatrrsTaskId = a})

-- | -- | The response status code.
samatrrsResponseStatus :: Lens' StartAuditMitigationActionsTaskResponse Int
samatrrsResponseStatus = lens _samatrrsResponseStatus (\s a -> s {_samatrrsResponseStatus = a})
