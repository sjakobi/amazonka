...
--
-- * 'uaacRoleARN' - The Amazon Resource Name (ARN) of the role that grants permission to AWS IoT to access information about your devices, policies, certificates, and other items as required when performing an audit.
--
...
updateAccountAuditConfiguration =
  UpdateAccountAuditConfiguration'
    { _uaacRoleARN =
        Nothing,
      _uaacAuditCheckConfigurations = Nothing,
      _uaacAuditNotificationTargetConfigurations =
        Nothing
    }
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'uaacrrsResponseStatus' - -- | The response status code.
updateAccountAuditConfigurationResponse ::
  -- | 'uaacrrsResponseStatus'
  Int ->
  UpdateAccountAuditConfigurationResponse
...
uaacrrsResponseStatus :: Lens' UpdateAccountAuditConfigurationResponse Int
uaacrrsResponseStatus = lens _uaacrrsResponseStatus (\s a -> s {_uaacrrsResponseStatus = a})
