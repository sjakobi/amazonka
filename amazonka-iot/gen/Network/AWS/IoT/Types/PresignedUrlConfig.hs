...
data PresignedUrlConfig = PresignedUrlConfig'
  { -- | The ARN of an IAM role that grants grants permission to download files
    ...
    expiresInSec :: Prelude.Maybe Prelude.Nat
  }
...
presignedUrlConfig_expiresInSec = Lens.lens (\PresignedUrlConfig' {expiresInSec} -> expiresInSec) (\s@PresignedUrlConfig' {} a -> s {expiresInSec = a} :: PresignedUrlConfig) Prelude.. Lens.mapping Prelude._Nat
