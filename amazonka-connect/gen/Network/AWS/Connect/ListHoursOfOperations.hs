...
--
-- * 'lhooMaxResults' - The maximum number of results to return per page.
--
...
instance AWSPager ListHoursOfOperations where
  page rq rs
    | stop (rs ^. lhoorrsNextToken) = Nothing
    | stop (rs ^. lhoorrsHoursOfOperationSummaryList) =
      Nothing
    | otherwise =
      Just $ rq & lhooNextToken .~ rs ^. lhoorrsNextToken

instance AWSRequest ListHoursOfOperations where
  type
    Rs ListHoursOfOperations =
      ListHoursOfOperationsResponse
  request = get connect
  response =
    receiveJSON
...
data ListHoursOfOperationsResponse = ListHoursOfOperationsResponse'
  { _lhoorrsNextToken ::
    ...
    _lhoorrsHoursOfOperationSummaryList ::
      !( Maybe
           [HoursOfOperationSummary]
       ),
    _lhoorrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lhoorrsNextToken' - If there are additional results, this is the token for the next set of results.
--
-- * 'lhoorrsHoursOfOperationSummaryList' - Information about the hours of operation.
--
-- * 'lhoorrsResponseStatus' - -- | The response status code.
listHoursOfOperationsResponse ::
  -- | 'lhoorrsResponseStatus'
  Int ->
  ListHoursOfOperationsResponse
listHoursOfOperationsResponse pResponseStatus_ =
  ListHoursOfOperationsResponse'
    { _lhoorrsNextToken =
      ...
      _lhoorrsHoursOfOperationSummaryList =
        Nothing,
      _lhoorrsResponseStatus = pResponseStatus_
    }

-- | If there are additional results, this is the token for the next set of results.
lhoorrsNextToken :: Lens' ListHoursOfOperationsResponse (Maybe Text)
lhoorrsNextToken = lens _lhoorrsNextToken (\s a -> s {_lhoorrsNextToken = a})

-- | Information about the hours of operation.
lhoorrsHoursOfOperationSummaryList :: Lens' ListHoursOfOperationsResponse [HoursOfOperationSummary]
lhoorrsHoursOfOperationSummaryList = lens _lhoorrsHoursOfOperationSummaryList (\s a -> s {_lhoorrsHoursOfOperationSummaryList = a}) . _Default . _Coerce

-- | -- | The response status code.
lhoorrsResponseStatus :: Lens' ListHoursOfOperationsResponse Int
lhoorrsResponseStatus = lens _lhoorrsResponseStatus (\s a -> s {_lhoorrsResponseStatus = a})
