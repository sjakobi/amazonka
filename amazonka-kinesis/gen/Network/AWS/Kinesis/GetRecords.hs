...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'grrrsMillisBehindLatest' - The number of milliseconds the 'GetRecords' response is from the tip of the stream, indicating how far behind current time the consumer is. A value of zero indicates that record processing is caught up, and there are no new records to process at this moment.
--
-- * 'grrrsNextShardIterator' - The next position in the shard from which to start sequentially reading data records. If set to @null@ , the shard has been closed and the requested iterator does not return any more data.
--
-- * 'grrrsChildShards' - Undocumented member.
--
-- * 'grrrsResponseStatus' - -- | The response status code.
--
-- * 'grrrsRecords' - The data records retrieved from the shard.
getRecordsResponse ::
  -- | 'grrrsResponseStatus'
  Int ->
  GetRecordsResponse
getRecordsResponse pResponseStatus_ =
  GetRecordsResponse'
    { _grrrsMillisBehindLatest =
      ...
      _grrrsChildShards = Nothing,
      _grrrsResponseStatus = pResponseStatus_,
      _grrrsRecords = mempty
    }

-- | The number of milliseconds the 'GetRecords' response is from the tip of the stream, indicating how far behind current time the consumer is. A value of zero indicates that record processing is caught up, and there are no new records to process at this moment.
grrrsMillisBehindLatest :: Lens' GetRecordsResponse (Maybe Natural)
grrrsMillisBehindLatest = lens _grrrsMillisBehindLatest (\s a -> s {_grrrsMillisBehindLatest = a}) . mapping _Nat
...
-- | Undocumented member.
grrrsChildShards :: Lens' GetRecordsResponse [ChildShard]
grrrsChildShards = lens _grrrsChildShards (\s a -> s {_grrrsChildShards = a}) . _Default . _Coerce
...
grrrsRecords = lens _grrrsRecords (\s a -> s {_grrrsRecords = a}) . _Coerce
