...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'bgprrsProjects' - Information about the requested build projects.
--
-- * 'bgprrsProjectsNotFound' - The names of build projects for which information could not be found.
--
...
batchGetProjectsResponse pResponseStatus_ =
  BatchGetProjectsResponse'
    { _bgprrsProjects =
        Nothing,
      _bgprrsProjectsNotFound = Nothing,
      _bgprrsResponseStatus = pResponseStatus_
    }

-- | Information about the requested build projects.
bgprrsProjects :: Lens' BatchGetProjectsResponse [Project]
bgprrsProjects = lens _bgprrsProjects (\s a -> s {_bgprrsProjects = a}) . _Default . _Coerce

-- | The names of build projects for which information could not be found.
bgprrsProjectsNotFound :: Lens' BatchGetProjectsResponse (Maybe (NonEmpty Text))
bgprrsProjectsNotFound = lens _bgprrsProjectsNotFound (\s a -> s {_bgprrsProjectsNotFound = a}) . mapping _List1

-- | -- | The response status code.
bgprrsResponseStatus :: Lens' BatchGetProjectsResponse Int
