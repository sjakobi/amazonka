...
data GetTrafficPolicyResponse = GetTrafficPolicyResponse'
  { _gtprrsResponseStatus ::
      !Int,
    _gtprrsTrafficPolicy ::
      !TrafficPolicy
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gtprrsResponseStatus' - -- | The response status code.
--
-- * 'gtprrsTrafficPolicy' - A complex type that contains settings for the specified traffic policy.
getTrafficPolicyResponse ::
  -- | 'gtprrsResponseStatus'
  Int ->
  -- | 'gtprrsTrafficPolicy'
...
getTrafficPolicyResponse
  ...
  pTrafficPolicy_ =
    GetTrafficPolicyResponse'
      { _gtprrsResponseStatus =
          pResponseStatus_,
        _gtprrsTrafficPolicy = pTrafficPolicy_
      }

-- | -- | The response status code.
gtprrsResponseStatus :: Lens' GetTrafficPolicyResponse Int
gtprrsResponseStatus = lens _gtprrsResponseStatus (\s a -> s {_gtprrsResponseStatus = a})

-- | A complex type that contains settings for the specified traffic policy.
gtprrsTrafficPolicy :: Lens' GetTrafficPolicyResponse TrafficPolicy
gtprrsTrafficPolicy = lens _gtprrsTrafficPolicy (\s a -> s {_gtprrsTrafficPolicy = a})
