...
--
-- * 'ltfrResourceType' - The type of the resources.     * The resource type for health checks is @healthcheck@ .     * The resource type for hosted zones is @hostedzone@ .
--
-- * 'ltfrResourceIds' - A complex type that contains the ResourceId element for each resource for which you want to get a list of tags.
listTagsForResources ::
  -- | 'ltfrResourceType'
  TagResourceType ->
  -- | 'ltfrResourceIds'
...
listTagsForResources pResourceType_ pResourceIds_ =
  ListTagsForResources'
    { _ltfrResourceType =
        pResourceType_,
      _ltfrResourceIds = _List1 # pResourceIds_
    }
...
ltfrResourceType :: Lens' ListTagsForResources TagResourceType
...
ltfrResourceIds = lens _ltfrResourceIds (\s a -> s {_ltfrResourceIds = a}) . _List1
...
instance ToXML ListTagsForResources where
  toXML ListTagsForResources' {..} =
    mconcat
      [ "ResourceIds"
          @= toXMLList "ResourceId" _ltfrResourceIds
      ]
