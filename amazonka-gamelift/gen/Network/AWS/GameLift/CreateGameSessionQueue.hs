...
data CreateGameSessionQueueResponse = CreateGameSessionQueueResponse'
  { _cgsqrrsGameSessionQueue ::
      !( Maybe
           GameSessionQueue
       ),
    _cgsqrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'cgsqrrsGameSessionQueue' - An object that describes the newly created game session queue.
--
-- * 'cgsqrrsResponseStatus' - -- | The response status code.
createGameSessionQueueResponse ::
  -- | 'cgsqrrsResponseStatus'
  Int ->
  CreateGameSessionQueueResponse
createGameSessionQueueResponse pResponseStatus_ =
  CreateGameSessionQueueResponse'
    { _cgsqrrsGameSessionQueue =
        Nothing,
      _cgsqrrsResponseStatus = pResponseStatus_
    }

-- | An object that describes the newly created game session queue.
cgsqrrsGameSessionQueue :: Lens' CreateGameSessionQueueResponse (Maybe GameSessionQueue)
cgsqrrsGameSessionQueue = lens _cgsqrrsGameSessionQueue (\s a -> s {_cgsqrrsGameSessionQueue = a})

-- | -- | The response status code.
cgsqrrsResponseStatus :: Lens' CreateGameSessionQueueResponse Int
cgsqrrsResponseStatus = lens _cgsqrrsResponseStatus (\s a -> s {_cgsqrrsResponseStatus = a})
