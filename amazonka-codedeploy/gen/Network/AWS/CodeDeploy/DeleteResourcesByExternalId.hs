...
instance ToJSON DeleteResourcesByExternalId where
  toJSON DeleteResourcesByExternalId' {..} =
    object
      (catMaybes [("externalId" .=) <$> _drbeiExternalId])
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'drbeirrsResponseStatus' - -- | The response status code.
deleteResourcesByExternalIdResponse ::
  -- | 'drbeirrsResponseStatus'
  Int ->
  DeleteResourcesByExternalIdResponse
...
drbeirrsResponseStatus :: Lens' DeleteResourcesByExternalIdResponse Int
drbeirrsResponseStatus = lens _drbeirrsResponseStatus (\s a -> s {_drbeirrsResponseStatus = a})
