...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ruprrsResponseStatus' - -- | The response status code.
resetUserPasswordResponse ::
  -- | 'ruprrsResponseStatus'
  Int ->
  ResetUserPasswordResponse
...
ruprrsResponseStatus :: Lens' ResetUserPasswordResponse Int
ruprrsResponseStatus = lens _ruprrsResponseStatus (\s a -> s {_ruprrsResponseStatus = a})
