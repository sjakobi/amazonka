...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ltfcrrsTags' - The key-value pairs that define the applied tags.
--
-- * 'ltfcrrsResponseStatus' - -- | The response status code.
listTagsForCertificateResponse ::
  -- | 'ltfcrrsResponseStatus'
  Int ->
  ListTagsForCertificateResponse
listTagsForCertificateResponse pResponseStatus_ =
  ListTagsForCertificateResponse'
    { _ltfcrrsTags =
        Nothing,
      _ltfcrrsResponseStatus = pResponseStatus_
    }

-- | The key-value pairs that define the applied tags.
ltfcrrsTags :: Lens' ListTagsForCertificateResponse (Maybe (NonEmpty Tag))
ltfcrrsTags = lens _ltfcrrsTags (\s a -> s {_ltfcrrsTags = a}) . mapping _List1

-- | -- | The response status code.
ltfcrrsResponseStatus :: Lens' ListTagsForCertificateResponse Int
ltfcrrsResponseStatus = lens _ltfcrrsResponseStatus (\s a -> s {_ltfcrrsResponseStatus = a})
