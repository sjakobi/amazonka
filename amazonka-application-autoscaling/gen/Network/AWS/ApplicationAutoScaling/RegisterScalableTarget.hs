...
--
-- * 'rstMaxCapacity' - The maximum value that you plan to scale out to. When a scaling policy is in effect, Application Auto Scaling can scale out (expand) as needed to the maximum capacity limit in response to changing demand. This property is required when registering a new scalable target. Although you can specify a large maximum capacity, note that service quotas may impose lower limits. Each service has its own default quotas for the maximum capacity of the resource. If you want to specify a higher limit, you can request an increase. For more information, consult the documentation for that service. For information about the default quotas for each service, see <https://docs.aws.amazon.com/general/latest/gr/aws-service-information.html Service Endpoints and Quotas> in the /Amazon Web Services General Reference/ .
--
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rstrrsResponseStatus' - -- | The response status code.
registerScalableTargetResponse ::
  -- | 'rstrrsResponseStatus'
  Int ->
  RegisterScalableTargetResponse
...
rstrrsResponseStatus :: Lens' RegisterScalableTargetResponse Int
rstrrsResponseStatus = lens _rstrrsResponseStatus (\s a -> s {_rstrrsResponseStatus = a})
