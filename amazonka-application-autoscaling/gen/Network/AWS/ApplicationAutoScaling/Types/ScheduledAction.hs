...
data ScheduledAction = ScheduledAction'
  { _saStartTime ::
    ...
    _saTimezone :: !(Maybe Text),
    _saScalableTargetAction ::
...
--
-- * 'saStartTime' - The date and time that the action is scheduled to begin, in UTC.
--
-- * 'saEndTime' - The date and time that the action is scheduled to end, in UTC.
--
...
--
-- * 'saTimezone' - The time zone used when referring to the date and time of a scheduled action, when the scheduled action uses an at or cron expression.
--
...
saTimezone :: Lens' ScheduledAction (Maybe Text)
saTimezone = lens _saTimezone (\s a -> s {_saTimezone = a})
