...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'clarrsResponseStatus' - -- | The response status code.
completeLifecycleActionResponse ::
  -- | 'clarrsResponseStatus'
  Int ->
  CompleteLifecycleActionResponse
...
clarrsResponseStatus :: Lens' CompleteLifecycleActionResponse Int
clarrsResponseStatus = lens _clarrsResponseStatus (\s a -> s {_clarrsResponseStatus = a})
