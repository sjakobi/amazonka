...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rlahrrsResponseStatus' - -- | The response status code.
recordLifecycleActionHeartbeatResponse ::
  -- | 'rlahrrsResponseStatus'
  Int ->
  RecordLifecycleActionHeartbeatResponse
...
rlahrrsResponseStatus :: Lens' RecordLifecycleActionHeartbeatResponse Int
rlahrrsResponseStatus = lens _rlahrrsResponseStatus (\s a -> s {_rlahrrsResponseStatus = a})
