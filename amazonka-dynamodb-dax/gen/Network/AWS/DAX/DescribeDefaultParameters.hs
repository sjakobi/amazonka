...
instance AWSPager DescribeDefaultParameters where
  page rq rs
    | stop (rs ^. ddprrsNextToken) = Nothing
    | stop (rs ^. ddprrsParameters) = Nothing
    | otherwise =
      Just $ rq & ddpNextToken .~ rs ^. ddprrsNextToken
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ddprrsNextToken' - Provides an identifier to allow retrieval of paginated results.
--
-- * 'ddprrsParameters' - A list of parameters. Each element in the list represents one parameter.
--
...
describeDefaultParametersResponse pResponseStatus_ =
  DescribeDefaultParametersResponse'
    { _ddprrsNextToken =
        Nothing,
      _ddprrsParameters = Nothing,
      _ddprrsResponseStatus = pResponseStatus_
    }

-- | Provides an identifier to allow retrieval of paginated results.
ddprrsNextToken :: Lens' DescribeDefaultParametersResponse (Maybe Text)
ddprrsNextToken = lens _ddprrsNextToken (\s a -> s {_ddprrsNextToken = a})

-- | A list of parameters. Each element in the list represents one parameter.
ddprrsParameters :: Lens' DescribeDefaultParametersResponse [Parameter]
ddprrsParameters = lens _ddprrsParameters (\s a -> s {_ddprrsParameters = a}) . _Default . _Coerce

-- | -- | The response status code.
ddprrsResponseStatus :: Lens' DescribeDefaultParametersResponse Int
