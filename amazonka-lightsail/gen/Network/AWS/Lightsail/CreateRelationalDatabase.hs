...
data CreateRelationalDatabase = CreateRelationalDatabase'
  { _crdPreferredBackupWindow ::
    ...
    _crdPubliclyAccessible ::
      !(Maybe Bool),
    _crdAvailabilityZone ::
      !(Maybe Text),
    _crdPreferredMaintenanceWindow ::
      !(Maybe Text),
    _crdTags ::
      !(Maybe [Tag]),
    _crdRelationalDatabaseName ::
      !Text,
    _crdRelationalDatabaseBlueprintId ::
      !Text,
    _crdRelationalDatabaseBundleId ::
      !Text,
    _crdMasterDatabaseName ::
      !Text,
    _crdMasterUsername ::
      !Text
  }
...
--
-- * 'crdRelationalDatabaseName' - The name to use for your new Lightsail database resource. Constraints:     * Must contain from 2 to 255 alphanumeric characters, or hyphens.     * The first and last character must be a letter or number.
--
    ...
    object
      ( catMaybes
          [ ("preferredBackupWindow" .=)
            ...
            Just
              ("masterDatabaseName" .= _crdMasterDatabaseName),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'crdrrsOperations' - An array of objects that describe the result of the action, such as the status of the request, the timestamp of the request, and the resources affected by the request.
--
...
crdrrsOperations :: Lens' CreateRelationalDatabaseResponse [Operation]
crdrrsOperations = lens _crdrrsOperations (\s a -> s {_crdrrsOperations = a}) . _Default . _Coerce

-- | -- | The response status code.
crdrrsResponseStatus :: Lens' CreateRelationalDatabaseResponse Int
