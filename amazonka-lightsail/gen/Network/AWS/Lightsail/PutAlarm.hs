...
instance ToHeaders PutAlarm where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("Lightsail_20161128.PutAlarm" :: ByteString),
            "Content-Type"
      ...
      ( catMaybes
          [ ("datapointsToAlarm" .=) <$> _paDatapointsToAlarm,
            ("notificationTriggers" .=)
              <$> _paNotificationTriggers,
            ("notificationEnabled" .=)
              <$> _paNotificationEnabled,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'parrsOperations' - An array of objects that describe the result of the action, such as the status of the request, the timestamp of the request, and the resources affected by the request.
--
...
putAlarmResponse pResponseStatus_ =
  PutAlarmResponse'
    { _parrsOperations = Nothing,
      _parrsResponseStatus = pResponseStatus_
    }

-- | An array of objects that describe the result of the action, such as the status of the request, the timestamp of the request, and the resources affected by the request.
parrsOperations :: Lens' PutAlarmResponse [Operation]
parrsOperations = lens _parrsOperations (\s a -> s {_parrsOperations = a}) . _Default . _Coerce

-- | -- | The response status code.
parrsResponseStatus :: Lens' PutAlarmResponse Int
