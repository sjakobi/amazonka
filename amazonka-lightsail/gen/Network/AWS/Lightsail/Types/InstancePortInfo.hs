...
--
-- * 'ipiFromPort' - The first port in a range of open ports on an instance. Allowed ports:     * TCP and UDP - @0@ to @65535@      * ICMP - The ICMP type for IPv4 addresses. For example, specify @8@ as the @fromPort@ (ICMP type), and @-1@ as the @toPort@ (ICMP code), to enable ICMP Ping. For more information, see <https://en.wikipedia.org/wiki/Internet_Control_Message_Protocol#Control_messages Control Messages> on /Wikipedia/ .     * ICMPv6 - The ICMP type for IPv6 addresses. For example, specify @128@ as the @fromPort@ (ICMPv6 type), and @0@ as @toPort@ (ICMPv6 code). For more information, see <https://en.wikipedia.org/wiki/Internet_Control_Message_Protocol_for_IPv6 Internet Control Message Protocol for IPv6> .
--
...
ipiIpv6Cidrs :: Lens' InstancePortInfo [Text]
ipiIpv6Cidrs = lens _ipiIpv6Cidrs (\s a -> s {_ipiIpv6Cidrs = a}) . _Default . _Coerce
