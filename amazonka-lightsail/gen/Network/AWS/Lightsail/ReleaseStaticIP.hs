...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rsirrsOperations' - An array of objects that describe the result of the action, such as the status of the request, the timestamp of the request, and the resources affected by the request.
--
...
rsirrsOperations :: Lens' ReleaseStaticIPResponse [Operation]
rsirrsOperations = lens _rsirrsOperations (\s a -> s {_rsirrsOperations = a}) . _Default . _Coerce

-- | -- | The response status code.
rsirrsResponseStatus :: Lens' ReleaseStaticIPResponse Int
