  ...
  pTaskType_ =
    RegisterTaskWithMaintenanceWindow'
      { _rtwmwMaxErrors =
        ...
        _rtwmwTargets = Nothing,
        _rtwmwTaskInvocationParameters = Nothing,
        _rtwmwName = Nothing,
        _rtwmwMaxConcurrency = Nothing,
...
rtwmwMaxErrors :: Lens' RegisterTaskWithMaintenanceWindow (Maybe Text)
...
rtwmwTargets = lens _rtwmwTargets (\s a -> s {_rtwmwTargets = a}) . _Default . _Coerce
...
rtwmwMaxConcurrency :: Lens' RegisterTaskWithMaintenanceWindow (Maybe Text)
...
instance AWSRequest RegisterTaskWithMaintenanceWindow where
  type
...
instance ToHeaders RegisterTaskWithMaintenanceWindow where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
...
instance ToPath RegisterTaskWithMaintenanceWindow where
  toPath = const "/"

instance ToQuery RegisterTaskWithMaintenanceWindow where
...
data RegisterTaskWithMaintenanceWindowResponse = RegisterTaskWithMaintenanceWindowResponse'
  { _rtwmwrrsWindowTaskId ::
      !( Maybe
           Text
       ),
    _rtwmwrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rtwmwrrsWindowTaskId' - The ID of the task in the maintenance window.
--
-- * 'rtwmwrrsResponseStatus' - -- | The response status code.
registerTaskWithMaintenanceWindowResponse ::
  -- | 'rtwmwrrsResponseStatus'
  Int ->
  RegisterTaskWithMaintenanceWindowResponse
...
rtwmwrrsWindowTaskId :: Lens' RegisterTaskWithMaintenanceWindowResponse (Maybe Text)
rtwmwrrsWindowTaskId = lens _rtwmwrrsWindowTaskId (\s a -> s {_rtwmwrrsWindowTaskId = a})

-- | -- | The response status code.
rtwmwrrsResponseStatus :: Lens' RegisterTaskWithMaintenanceWindowResponse Int
rtwmwrrsResponseStatus = lens _rtwmwrrsResponseStatus (\s a -> s {_rtwmwrrsResponseStatus = a})
