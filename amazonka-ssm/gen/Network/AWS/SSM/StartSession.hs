...
--
-- * 'ssDocumentName' - The name of the SSM document to define the parameters and plugin settings for the session. For example, @SSM-SessionManagerRunShell@ . You can call the 'GetDocument' API to verify the document exists before attempting to start a session. If no document name is provided, a shell to the instance is launched by default.
--
...
instance Hashable StartSession

instance NFData StartSession
...
startSessionResponse pResponseStatus_ =
  StartSessionResponse'
    { _ssrrsSessionId = Nothing,
      _ssrrsStreamURL = Nothing,
      _ssrrsTokenValue = Nothing,
      _ssrrsResponseStatus = pResponseStatus_
    }

-- | The ID of the session.
ssrrsSessionId :: Lens' StartSessionResponse (Maybe Text)
ssrrsSessionId = lens _ssrrsSessionId (\s a -> s {_ssrrsSessionId = a})

-- | A URL back to SSM Agent on the instance that the Session Manager client uses to send commands and receive output from the instance. Format: @wss://ssmmessages.__region__ .amazonaws.com/v1/data-channel/__session-id__ ?stream=(input|output)@  __region__ represents the Region identifier for an AWS Region supported by AWS Systems Manager, such as @us-east-2@ for the US East (Ohio) Region. For a list of supported __region__ values, see the __Region__ column in <http://docs.aws.amazon.com/general/latest/gr/ssm.html#ssm_region Systems Manager service endpoints> in the /AWS General Reference/ . __session-id__ represents the ID of a Session Manager session, such as @1a2b3c4dEXAMPLE@ .
ssrrsStreamURL :: Lens' StartSessionResponse (Maybe Text)
ssrrsStreamURL = lens _ssrrsStreamURL (\s a -> s {_ssrrsStreamURL = a})

-- | An encrypted token value containing session and caller information. Used to authenticate the connection to the instance.
ssrrsTokenValue :: Lens' StartSessionResponse (Maybe Text)
ssrrsTokenValue = lens _ssrrsTokenValue (\s a -> s {_ssrrsTokenValue = a})

-- | -- | The response status code.
ssrrsResponseStatus :: Lens' StartSessionResponse Int
ssrrsResponseStatus = lens _ssrrsResponseStatus (\s a -> s {_ssrrsResponseStatus = a})

instance NFData StartSessionResponse
