...
instance AWSPager ListDocuments where
  page rq rs
    ...
    | stop (rs ^. ldrrsDocumentIdentifiers) = Nothing
    | otherwise =
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ldrrsNextToken' - The token to use when requesting the next set of items. If there are no additional items to return, the string is empty.
--
-- * 'ldrrsDocumentIdentifiers' - The names of the Systems Manager documents.
--
...
listDocumentsResponse pResponseStatus_ =
  ListDocumentsResponse'
    { _ldrrsNextToken = Nothing,
      _ldrrsDocumentIdentifiers = Nothing,
      _ldrrsResponseStatus = pResponseStatus_
    }

-- | The token to use when requesting the next set of items. If there are no additional items to return, the string is empty.
ldrrsNextToken :: Lens' ListDocumentsResponse (Maybe Text)
ldrrsNextToken = lens _ldrrsNextToken (\s a -> s {_ldrrsNextToken = a})

-- | The names of the Systems Manager documents.
ldrrsDocumentIdentifiers :: Lens' ListDocumentsResponse [DocumentIdentifier]
ldrrsDocumentIdentifiers = lens _ldrrsDocumentIdentifiers (\s a -> s {_ldrrsDocumentIdentifiers = a}) . _Default . _Coerce

-- | -- | The response status code.
ldrrsResponseStatus :: Lens' ListDocumentsResponse Int
