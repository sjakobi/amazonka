...
data PatchRule = PatchRule'
  { -- | The number of days after the release date of each patch matched by the
    ...
    approveAfterDays :: Prelude.Maybe Prelude.Nat,
...
patchRule_approveAfterDays = Lens.lens (\PatchRule' {approveAfterDays} -> approveAfterDays) (\s@PatchRule' {} a -> s {approveAfterDays = a} :: PatchRule) Prelude.. Lens.mapping Prelude._Nat
