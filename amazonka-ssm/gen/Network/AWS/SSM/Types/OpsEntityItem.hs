...
opsEntityItem_content = Lens.lens (\OpsEntityItem' {content} -> content) (\s@OpsEntityItem' {} a -> s {content = a} :: OpsEntityItem) Prelude.. Lens.mapping Prelude._Coerce

instance Prelude.FromJSON OpsEntityItem where
  parseJSON =
    Prelude.withObject
      "OpsEntityItem"
      ( \x ->
