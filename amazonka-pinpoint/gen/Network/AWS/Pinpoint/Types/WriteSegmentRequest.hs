...
writeSegmentRequest_tags = Lens.lens (\WriteSegmentRequest' {tags} -> tags) (\s@WriteSegmentRequest' {} a -> s {tags = a} :: WriteSegmentRequest) Prelude.. Lens.mapping Prelude._Map
...
instance Prelude.ToJSON WriteSegmentRequest where
  toJSON WriteSegmentRequest' {..} =
    Prelude.object
      ( Prelude.catMaybes
          [ ("SegmentGroups" Prelude..=)
              Prelude.<$> segmentGroups,
