...
--
-- * 'crChannels' - A map that contains a multipart response for each channel. For each item in this object, the ChannelType is the key and the Channel is the value.
channelsResponse ::
  ChannelsResponse
channelsResponse =
  ChannelsResponse' {_crChannels = mempty}

-- | A map that contains a multipart response for each channel. For each item in this object, the ChannelType is the key and the Channel is the value.
crChannels :: Lens' ChannelsResponse (HashMap Text ChannelResponse)
crChannels = lens _crChannels (\s a -> s {_crChannels = a}) . _Map
...
instance FromJSON ChannelsResponse where
  parseJSON =
    withObject
      "ChannelsResponse"
      ( \x ->
          ChannelsResponse' <$> (x .:? "Channels" .!= mempty)
      )
