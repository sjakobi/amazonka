...
instance AWSRequest PutMetricPolicy where
  ...
  response =
    receiveEmpty
      ( \s h x ->
          PutMetricPolicyResponse' <$> (pure (fromEnum s))
      )
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'pmprrsResponseStatus' - -- | The response status code.
putMetricPolicyResponse ::
  -- | 'pmprrsResponseStatus'
  Int ->
  PutMetricPolicyResponse
...
pmprrsResponseStatus :: Lens' PutMetricPolicyResponse Int
pmprrsResponseStatus = lens _pmprrsResponseStatus (\s a -> s {_pmprrsResponseStatus = a})
