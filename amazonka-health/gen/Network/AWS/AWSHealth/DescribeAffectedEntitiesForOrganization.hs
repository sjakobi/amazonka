...
data DescribeAffectedEntitiesForOrganization = DescribeAffectedEntitiesForOrganization'
  { -- | If the results of a search are large, only a portion of the results are
    ...
    organizationEntityFilters :: Prelude.List1 EventAccountFilter
  }
...
describeAffectedEntitiesForOrganization_maxResults = Lens.lens (\DescribeAffectedEntitiesForOrganization' {maxResults} -> maxResults) (\s@DescribeAffectedEntitiesForOrganization' {} a -> s {maxResults = a} :: DescribeAffectedEntitiesForOrganization) Prelude.. Lens.mapping Prelude._Nat
