...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'sdldjrdrsJobStatus' - Either @STOP_REQUESTED@ if the job is currently running, or @STOPPED@ if the job was previously stopped with the @StopDominantLanguageDetectionJob@ operation.
--
-- * 'sdldjrdrsJobId' - The identifier of the dominant language detection job to stop.
--
-- * 'sdldjrdrsResponseStatus' - -- | The response status code.
stopDominantLanguageDetectionJobResponse ::
  -- | 'sdldjrdrsResponseStatus'
  Int ->
  StopDominantLanguageDetectionJobResponse
stopDominantLanguageDetectionJobResponse
  pResponseStatus_ =
    StopDominantLanguageDetectionJobResponse'
      { _sdldjrdrsJobStatus =
          Nothing,
        _sdldjrdrsJobId = Nothing,
        _sdldjrdrsResponseStatus =
          pResponseStatus_
      }

-- | Either @STOP_REQUESTED@ if the job is currently running, or @STOPPED@ if the job was previously stopped with the @StopDominantLanguageDetectionJob@ operation.
sdldjrdrsJobStatus :: Lens' StopDominantLanguageDetectionJobResponse (Maybe JobStatus)
sdldjrdrsJobStatus = lens _sdldjrdrsJobStatus (\s a -> s {_sdldjrdrsJobStatus = a})

-- | The identifier of the dominant language detection job to stop.
sdldjrdrsJobId :: Lens' StopDominantLanguageDetectionJobResponse (Maybe Text)
sdldjrdrsJobId = lens _sdldjrdrsJobId (\s a -> s {_sdldjrdrsJobId = a})

-- | -- | The response status code.
sdldjrdrsResponseStatus :: Lens' StopDominantLanguageDetectionJobResponse Int
sdldjrdrsResponseStatus = lens _sdldjrdrsResponseStatus (\s a -> s {_sdldjrdrsResponseStatus = a})
