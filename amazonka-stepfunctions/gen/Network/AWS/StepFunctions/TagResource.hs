...
--
-- * 'trResourceARN' - The Amazon Resource Name (ARN) for the Step Functions state machine or activity.
--
-- * 'trTags' - The list of tags to add to a resource. Tags may only contain Unicode letters, digits, white space, or these symbols: @_ . : / = + - @@ .
tagResource ::
  -- | 'trResourceARN'
  Text ->
  TagResource
...
instance ToHeaders TagResource where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AWSStepFunctions.TagResource" :: ByteString),
            "Content-Type"
