...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'sthrrsResponseStatus' - -- | The response status code.
sendTaskHeartbeatResponse ::
  -- | 'sthrrsResponseStatus'
  Int ->
  SendTaskHeartbeatResponse
...
sthrrsResponseStatus :: Lens' SendTaskHeartbeatResponse Int
sthrrsResponseStatus = lens _sthrrsResponseStatus (\s a -> s {_sthrrsResponseStatus = a})
