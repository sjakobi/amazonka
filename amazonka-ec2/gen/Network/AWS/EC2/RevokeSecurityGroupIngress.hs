...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rsgirrsUnknownIPPermissions' - The inbound rules that were unknown to the service. In some cases, @unknownIpPermissionSet@ might be in a different format from the request parameter.
--
-- * 'rsgirrsReturn' - Returns @true@ if the request succeeds; otherwise, returns an error.
--
-- * 'rsgirrsResponseStatus' - -- | The response status code.
revokeSecurityGroupIngressResponse ::
  -- | 'rsgirrsResponseStatus'
  Int ->
  RevokeSecurityGroupIngressResponse
revokeSecurityGroupIngressResponse pResponseStatus_ =
  RevokeSecurityGroupIngressResponse'
    { _rsgirrsUnknownIPPermissions =
        Nothing,
      _rsgirrsReturn = Nothing,
      _rsgirrsResponseStatus =
        pResponseStatus_
    }

-- | The inbound rules that were unknown to the service. In some cases, @unknownIpPermissionSet@ might be in a different format from the request parameter.
rsgirrsUnknownIPPermissions :: Lens' RevokeSecurityGroupIngressResponse [IPPermission]
rsgirrsUnknownIPPermissions = lens _rsgirrsUnknownIPPermissions (\s a -> s {_rsgirrsUnknownIPPermissions = a}) . _Default . _Coerce

-- | Returns @true@ if the request succeeds; otherwise, returns an error.
rsgirrsReturn :: Lens' RevokeSecurityGroupIngressResponse (Maybe Bool)
rsgirrsReturn = lens _rsgirrsReturn (\s a -> s {_rsgirrsReturn = a})

-- | -- | The response status code.
rsgirrsResponseStatus :: Lens' RevokeSecurityGroupIngressResponse Int
rsgirrsResponseStatus = lens _rsgirrsResponseStatus (\s a -> s {_rsgirrsResponseStatus = a})
