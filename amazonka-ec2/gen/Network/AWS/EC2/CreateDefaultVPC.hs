...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'cdvrrsVPC' - Information about the VPC.
--
-- * 'cdvrrsResponseStatus' - -- | The response status code.
createDefaultVPCResponse ::
  -- | 'cdvrrsResponseStatus'
  Int ->
  CreateDefaultVPCResponse
createDefaultVPCResponse pResponseStatus_ =
  CreateDefaultVPCResponse'
    { _cdvrrsVPC = Nothing,
      _cdvrrsResponseStatus = pResponseStatus_
    }

-- | Information about the VPC.
cdvrrsVPC :: Lens' CreateDefaultVPCResponse (Maybe VPC)
cdvrrsVPC = lens _cdvrrsVPC (\s a -> s {_cdvrrsVPC = a})

-- | -- | The response status code.
cdvrrsResponseStatus :: Lens' CreateDefaultVPCResponse Int
cdvrrsResponseStatus = lens _cdvrrsResponseStatus (\s a -> s {_cdvrrsResponseStatus = a})
