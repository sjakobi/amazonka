...
data RejectVPCEndpointConnectionsResponse = RejectVPCEndpointConnectionsResponse'
  { _rvecrrsUnsuccessful ::
      !( Maybe
           [UnsuccessfulItem]
       ),
    _rvecrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rvecrrsUnsuccessful' - Information about the endpoints that were not rejected, if applicable.
--
-- * 'rvecrrsResponseStatus' - -- | The response status code.
rejectVPCEndpointConnectionsResponse ::
  -- | 'rvecrrsResponseStatus'
  Int ->
  RejectVPCEndpointConnectionsResponse
...
rvecrrsUnsuccessful :: Lens' RejectVPCEndpointConnectionsResponse [UnsuccessfulItem]
rvecrrsUnsuccessful = lens _rvecrrsUnsuccessful (\s a -> s {_rvecrrsUnsuccessful = a}) . _Default . _Coerce

-- | -- | The response status code.
rvecrrsResponseStatus :: Lens' RejectVPCEndpointConnectionsResponse Int
rvecrrsResponseStatus = lens _rvecrrsResponseStatus (\s a -> s {_rvecrrsResponseStatus = a})
