...
data DeleteVPCEndpointConnectionNotificationsResponse = DeleteVPCEndpointConnectionNotificationsResponse'
  { _dvecnrrsUnsuccessful ::
      !( Maybe
           [UnsuccessfulItem]
       ),
    _dvecnrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dvecnrrsUnsuccessful' - Information about the notifications that could not be deleted successfully.
--
-- * 'dvecnrrsResponseStatus' - -- | The response status code.
deleteVPCEndpointConnectionNotificationsResponse ::
  -- | 'dvecnrrsResponseStatus'
  Int ->
  DeleteVPCEndpointConnectionNotificationsResponse
...
dvecnrrsUnsuccessful :: Lens' DeleteVPCEndpointConnectionNotificationsResponse [UnsuccessfulItem]
dvecnrrsUnsuccessful = lens _dvecnrrsUnsuccessful (\s a -> s {_dvecnrrsUnsuccessful = a}) . _Default . _Coerce

-- | -- | The response status code.
dvecnrrsResponseStatus :: Lens' DeleteVPCEndpointConnectionNotificationsResponse Int
dvecnrrsResponseStatus = lens _dvecnrrsResponseStatus (\s a -> s {_dvecnrrsResponseStatus = a})
