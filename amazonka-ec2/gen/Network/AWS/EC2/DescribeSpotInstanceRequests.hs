...
instance AWSPager DescribeSpotInstanceRequests where
  page rq rs
    | stop (rs ^. dsirrrsNextToken) = Nothing
    | stop (rs ^. dsirrrsSpotInstanceRequests) = Nothing
    | otherwise =
      Just $ rq & dsirNextToken .~ rs ^. dsirrrsNextToken
...
data DescribeSpotInstanceRequestsResponse = DescribeSpotInstanceRequestsResponse'
  { _dsirrrsNextToken ::
    ...
    _dsirrrsSpotInstanceRequests ::
      !( Maybe
           [SpotInstanceRequest]
       ),
    _dsirrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dsirrrsNextToken' - The token to use to retrieve the next set of results. This value is @null@ when there are no more results to return.
--
-- * 'dsirrrsSpotInstanceRequests' - One or more Spot Instance requests.
--
-- * 'dsirrrsResponseStatus' - -- | The response status code.
describeSpotInstanceRequestsResponse ::
  -- | 'dsirrrsResponseStatus'
  Int ->
  DescribeSpotInstanceRequestsResponse
...
dsirrrsNextToken :: Lens' DescribeSpotInstanceRequestsResponse (Maybe Text)
dsirrrsNextToken = lens _dsirrrsNextToken (\s a -> s {_dsirrrsNextToken = a})

-- | One or more Spot Instance requests.
dsirrrsSpotInstanceRequests :: Lens' DescribeSpotInstanceRequestsResponse [SpotInstanceRequest]
dsirrrsSpotInstanceRequests = lens _dsirrrsSpotInstanceRequests (\s a -> s {_dsirrrsSpotInstanceRequests = a}) . _Default . _Coerce

-- | -- | The response status code.
dsirrrsResponseStatus :: Lens' DescribeSpotInstanceRequestsResponse Int
dsirrrsResponseStatus = lens _dsirrrsResponseStatus (\s a -> s {_dsirrrsResponseStatus = a})
