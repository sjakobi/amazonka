...
instance ToQuery CreateTrafficMirrorSession where
  toQuery CreateTrafficMirrorSession' {..} =
    mconcat
      [ "Action"
          =: ("CreateTrafficMirrorSession" :: ByteString),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ctmsrrsTrafficMirrorSession' - Information about the Traffic Mirror session.
--
-- * 'ctmsrrsClientToken' - Unique, case-sensitive identifier that you provide to ensure the idempotency of the request. For more information, see <https://docs.aws.amazon.com/AWSEC2/latest/APIReference/Run_Instance_Idempotency.html How to Ensure Idempotency> .
--
-- * 'ctmsrrsResponseStatus' - -- | The response status code.
createTrafficMirrorSessionResponse ::
  -- | 'ctmsrrsResponseStatus'
  Int ->
  CreateTrafficMirrorSessionResponse
createTrafficMirrorSessionResponse pResponseStatus_ =
  CreateTrafficMirrorSessionResponse'
    { _ctmsrrsTrafficMirrorSession =
        Nothing,
      _ctmsrrsClientToken = Nothing,
      _ctmsrrsResponseStatus =
        pResponseStatus_
    }

-- | Information about the Traffic Mirror session.
ctmsrrsTrafficMirrorSession :: Lens' CreateTrafficMirrorSessionResponse (Maybe TrafficMirrorSession)
ctmsrrsTrafficMirrorSession = lens _ctmsrrsTrafficMirrorSession (\s a -> s {_ctmsrrsTrafficMirrorSession = a})

-- | Unique, case-sensitive identifier that you provide to ensure the idempotency of the request. For more information, see <https://docs.aws.amazon.com/AWSEC2/latest/APIReference/Run_Instance_Idempotency.html How to Ensure Idempotency> .
ctmsrrsClientToken :: Lens' CreateTrafficMirrorSessionResponse (Maybe Text)
ctmsrrsClientToken = lens _ctmsrrsClientToken (\s a -> s {_ctmsrrsClientToken = a})

-- | -- | The response status code.
ctmsrrsResponseStatus :: Lens' CreateTrafficMirrorSessionResponse Int
ctmsrrsResponseStatus = lens _ctmsrrsResponseStatus (\s a -> s {_ctmsrrsResponseStatus = a})
