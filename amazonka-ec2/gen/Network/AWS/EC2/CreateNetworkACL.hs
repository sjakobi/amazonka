...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'cnarrsNetworkACL' - Information about the network ACL.
--
-- * 'cnarrsResponseStatus' - -- | The response status code.
createNetworkACLResponse ::
  -- | 'cnarrsResponseStatus'
  Int ->
  CreateNetworkACLResponse
createNetworkACLResponse pResponseStatus_ =
  CreateNetworkACLResponse'
    { _cnarrsNetworkACL =
        Nothing,
      _cnarrsResponseStatus = pResponseStatus_
    }

-- | Information about the network ACL.
cnarrsNetworkACL :: Lens' CreateNetworkACLResponse (Maybe NetworkACL)
cnarrsNetworkACL = lens _cnarrsNetworkACL (\s a -> s {_cnarrsNetworkACL = a})

-- | -- | The response status code.
cnarrsResponseStatus :: Lens' CreateNetworkACLResponse Int
cnarrsResponseStatus = lens _cnarrsResponseStatus (\s a -> s {_cnarrsResponseStatus = a})
