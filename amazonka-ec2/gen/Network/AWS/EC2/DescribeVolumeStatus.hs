...
instance AWSPager DescribeVolumeStatus where
  page rq rs
    | stop (rs ^. dvsrrsNextToken) = Nothing
    | stop (rs ^. dvsrrsVolumeStatuses) = Nothing
    | otherwise =
      Just $ rq & dvssNextToken .~ rs ^. dvsrrsNextToken
...
data DescribeVolumeStatusResponse = DescribeVolumeStatusResponse'
  { _dvsrrsNextToken ::
      !(Maybe Text),
    _dvsrrsVolumeStatuses ::
      !( Maybe
           [VolumeStatusItem]
       ),
    _dvsrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dvsrrsNextToken' - The token to use to retrieve the next page of results. This value is @null@ when there are no more results to return.
--
-- * 'dvsrrsVolumeStatuses' - Information about the status of the volumes.
--
-- * 'dvsrrsResponseStatus' - -- | The response status code.
describeVolumeStatusResponse ::
  -- | 'dvsrrsResponseStatus'
  Int ->
  DescribeVolumeStatusResponse
describeVolumeStatusResponse pResponseStatus_ =
  DescribeVolumeStatusResponse'
    { _dvsrrsNextToken =
        Nothing,
      _dvsrrsVolumeStatuses = Nothing,
      _dvsrrsResponseStatus = pResponseStatus_
    }

-- | The token to use to retrieve the next page of results. This value is @null@ when there are no more results to return.
dvsrrsNextToken :: Lens' DescribeVolumeStatusResponse (Maybe Text)
dvsrrsNextToken = lens _dvsrrsNextToken (\s a -> s {_dvsrrsNextToken = a})

-- | Information about the status of the volumes.
dvsrrsVolumeStatuses :: Lens' DescribeVolumeStatusResponse [VolumeStatusItem]
dvsrrsVolumeStatuses = lens _dvsrrsVolumeStatuses (\s a -> s {_dvsrrsVolumeStatuses = a}) . _Default . _Coerce

-- | -- | The response status code.
dvsrrsResponseStatus :: Lens' DescribeVolumeStatusResponse Int
dvsrrsResponseStatus = lens _dvsrrsResponseStatus (\s a -> s {_dvsrrsResponseStatus = a})
