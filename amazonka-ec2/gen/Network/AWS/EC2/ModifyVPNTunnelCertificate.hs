...
instance ToQuery ModifyVPNTunnelCertificate where
  toQuery ModifyVPNTunnelCertificate' {..} =
    mconcat
      [ "Action"
          =: ("ModifyVpnTunnelCertificate" :: ByteString),
...
data ModifyVPNTunnelCertificateResponse = ModifyVPNTunnelCertificateResponse'
  { _mvtcrrsVPNConnection ::
      !( Maybe
           VPNConnection
       ),
    _mvtcrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'mvtcrrsVPNConnection' - Undocumented member.
--
-- * 'mvtcrrsResponseStatus' - -- | The response status code.
modifyVPNTunnelCertificateResponse ::
  -- | 'mvtcrrsResponseStatus'
  Int ->
  ModifyVPNTunnelCertificateResponse
...
-- | Undocumented member.
mvtcrrsVPNConnection :: Lens' ModifyVPNTunnelCertificateResponse (Maybe VPNConnection)
mvtcrrsVPNConnection = lens _mvtcrrsVPNConnection (\s a -> s {_mvtcrrsVPNConnection = a})

-- | -- | The response status code.
mvtcrrsResponseStatus :: Lens' ModifyVPNTunnelCertificateResponse Int
mvtcrrsResponseStatus = lens _mvtcrrsResponseStatus (\s a -> s {_mvtcrrsResponseStatus = a})
