...
instance AWSPager GetAssociatedIPv6PoolCidrs where
  page rq rs
    | stop (rs ^. gaipcrrsNextToken) = Nothing
    | stop (rs ^. gaipcrrsIPv6CidrAssociations) = Nothing
    | otherwise =
      Just $ rq & gaipcNextToken .~ rs ^. gaipcrrsNextToken
...
instance ToQuery GetAssociatedIPv6PoolCidrs where
  toQuery GetAssociatedIPv6PoolCidrs' {..} =
    mconcat
      [ "Action"
          =: ("GetAssociatedIpv6PoolCidrs" :: ByteString),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gaipcrrsIPv6CidrAssociations' - Information about the IPv6 CIDR block associations.
--
-- * 'gaipcrrsNextToken' - The token to use to retrieve the next page of results. This value is @null@ when there are no more results to return.
--
-- * 'gaipcrrsResponseStatus' - -- | The response status code.
getAssociatedIPv6PoolCidrsResponse ::
  -- | 'gaipcrrsResponseStatus'
  Int ->
  GetAssociatedIPv6PoolCidrsResponse
getAssociatedIPv6PoolCidrsResponse pResponseStatus_ =
  GetAssociatedIPv6PoolCidrsResponse'
    { _gaipcrrsIPv6CidrAssociations =
        Nothing,
      _gaipcrrsNextToken = Nothing,
      _gaipcrrsResponseStatus =
        pResponseStatus_
    }

-- | Information about the IPv6 CIDR block associations.
gaipcrrsIPv6CidrAssociations :: Lens' GetAssociatedIPv6PoolCidrsResponse [IPv6CidrAssociation]
gaipcrrsIPv6CidrAssociations = lens _gaipcrrsIPv6CidrAssociations (\s a -> s {_gaipcrrsIPv6CidrAssociations = a}) . _Default . _Coerce

-- | The token to use to retrieve the next page of results. This value is @null@ when there are no more results to return.
gaipcrrsNextToken :: Lens' GetAssociatedIPv6PoolCidrsResponse (Maybe Text)
gaipcrrsNextToken = lens _gaipcrrsNextToken (\s a -> s {_gaipcrrsNextToken = a})

-- | -- | The response status code.
gaipcrrsResponseStatus :: Lens' GetAssociatedIPv6PoolCidrsResponse Int
gaipcrrsResponseStatus = lens _gaipcrrsResponseStatus (\s a -> s {_gaipcrrsResponseStatus = a})
