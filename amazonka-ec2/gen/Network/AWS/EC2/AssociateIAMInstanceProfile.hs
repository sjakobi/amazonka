...
module Network.AWS.EC2.AssociateIAMInstanceProfile
  ( -- * Creating a Request
    ...
    -- * Response Lenses
    aiaprrsIAMInstanceProfileAssociation,
    aiaprrsResponseStatus,
  )
where
...
data AssociateIAMInstanceProfileResponse = AssociateIAMInstanceProfileResponse'
  { _aiaprrsIAMInstanceProfileAssociation ::
      !( Maybe
           IAMInstanceProfileAssociation
       ),
    _aiaprrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'aiaprrsIAMInstanceProfileAssociation' - Information about the IAM instance profile association.
--
-- * 'aiaprrsResponseStatus' - -- | The response status code.
associateIAMInstanceProfileResponse ::
  -- | 'aiaprrsResponseStatus'
  Int ->
  AssociateIAMInstanceProfileResponse
...
aiaprrsIAMInstanceProfileAssociation :: Lens' AssociateIAMInstanceProfileResponse (Maybe IAMInstanceProfileAssociation)
aiaprrsIAMInstanceProfileAssociation = lens _aiaprrsIAMInstanceProfileAssociation (\s a -> s {_aiaprrsIAMInstanceProfileAssociation = a})

-- | -- | The response status code.
aiaprrsResponseStatus :: Lens' AssociateIAMInstanceProfileResponse Int
aiaprrsResponseStatus = lens _aiaprrsResponseStatus (\s a -> s {_aiaprrsResponseStatus = a})
