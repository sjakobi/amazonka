...
instance ToQuery ExportClientVPNClientConfiguration where
  toQuery ExportClientVPNClientConfiguration' {..} =
    mconcat
      [ "Action"
          =: ("ExportClientVpnClientConfiguration" :: ByteString),
...
data ExportClientVPNClientConfigurationResponse = ExportClientVPNClientConfigurationResponse'
  { _ecvccrrsClientConfiguration ::
      !( Maybe
           Text
       ),
    _ecvccrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ecvccrrsClientConfiguration' - The contents of the Client VPN endpoint configuration file.
--
-- * 'ecvccrrsResponseStatus' - -- | The response status code.
exportClientVPNClientConfigurationResponse ::
  -- | 'ecvccrrsResponseStatus'
  Int ->
  ExportClientVPNClientConfigurationResponse
...
ecvccrrsClientConfiguration :: Lens' ExportClientVPNClientConfigurationResponse (Maybe Text)
ecvccrrsClientConfiguration = lens _ecvccrrsClientConfiguration (\s a -> s {_ecvccrrsClientConfiguration = a})

-- | -- | The response status code.
ecvccrrsResponseStatus :: Lens' ExportClientVPNClientConfigurationResponse Int
ecvccrrsResponseStatus = lens _ecvccrrsResponseStatus (\s a -> s {_ecvccrrsResponseStatus = a})
