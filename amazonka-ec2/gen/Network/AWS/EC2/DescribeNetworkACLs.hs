...
instance AWSPager DescribeNetworkACLs where
  page rq rs
    | stop (rs ^. dnarrsNextToken) = Nothing
    | stop (rs ^. dnarrsNetworkACLs) = Nothing
    | otherwise =
      Just $ rq & dnaclNextToken .~ rs ^. dnarrsNextToken
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dnarrsNextToken' - The token to use to retrieve the next page of results. This value is @null@ when there are no more results to return.
--
-- * 'dnarrsNetworkACLs' - Information about one or more network ACLs.
--
-- * 'dnarrsResponseStatus' - -- | The response status code.
describeNetworkACLsResponse ::
  -- | 'dnarrsResponseStatus'
  Int ->
  DescribeNetworkACLsResponse
describeNetworkACLsResponse pResponseStatus_ =
  DescribeNetworkACLsResponse'
    { _dnarrsNextToken =
        Nothing,
      _dnarrsNetworkACLs = Nothing,
      _dnarrsResponseStatus = pResponseStatus_
    }

-- | The token to use to retrieve the next page of results. This value is @null@ when there are no more results to return.
dnarrsNextToken :: Lens' DescribeNetworkACLsResponse (Maybe Text)
dnarrsNextToken = lens _dnarrsNextToken (\s a -> s {_dnarrsNextToken = a})

-- | Information about one or more network ACLs.
dnarrsNetworkACLs :: Lens' DescribeNetworkACLsResponse [NetworkACL]
dnarrsNetworkACLs = lens _dnarrsNetworkACLs (\s a -> s {_dnarrsNetworkACLs = a}) . _Default . _Coerce

-- | -- | The response status code.
dnarrsResponseStatus :: Lens' DescribeNetworkACLsResponse Int
dnarrsResponseStatus = lens _dnarrsResponseStatus (\s a -> s {_dnarrsResponseStatus = a})
