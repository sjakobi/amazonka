...
instance ToQuery ModifyVPNTunnelOptions where
  toQuery ModifyVPNTunnelOptions' {..} =
    mconcat
      [ "Action"
          =: ("ModifyVpnTunnelOptions" :: ByteString),
...
data ModifyVPNTunnelOptionsResponse = ModifyVPNTunnelOptionsResponse'
  { _mvtorrsVPNConnection ::
      !( Maybe
           VPNConnection
       ),
    _mvtorrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'mvtorrsVPNConnection' - Undocumented member.
--
-- * 'mvtorrsResponseStatus' - -- | The response status code.
modifyVPNTunnelOptionsResponse ::
  -- | 'mvtorrsResponseStatus'
  Int ->
  ModifyVPNTunnelOptionsResponse
modifyVPNTunnelOptionsResponse pResponseStatus_ =
  ModifyVPNTunnelOptionsResponse'
    { _mvtorrsVPNConnection =
        Nothing,
      _mvtorrsResponseStatus = pResponseStatus_
    }

-- | Undocumented member.
mvtorrsVPNConnection :: Lens' ModifyVPNTunnelOptionsResponse (Maybe VPNConnection)
mvtorrsVPNConnection = lens _mvtorrsVPNConnection (\s a -> s {_mvtorrsVPNConnection = a})

-- | -- | The response status code.
mvtorrsResponseStatus :: Lens' ModifyVPNTunnelOptionsResponse Int
mvtorrsResponseStatus = lens _mvtorrsResponseStatus (\s a -> s {_mvtorrsResponseStatus = a})
