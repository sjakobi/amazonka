...
module Network.AWS.EC2.DeleteVPCEndpointServiceConfigurations
  ( -- * Creating a Request
    ...
    -- * Response Lenses
    dvpcescrrsUnsuccessful,
    dvpcescrrsResponseStatus,
  )
where
...
data DeleteVPCEndpointServiceConfigurationsResponse = DeleteVPCEndpointServiceConfigurationsResponse'
  { _dvpcescrrsUnsuccessful ::
      !( Maybe
           [UnsuccessfulItem]
       ),
    _dvpcescrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dvpcescrrsUnsuccessful' - Information about the service configurations that were not deleted, if applicable.
--
-- * 'dvpcescrrsResponseStatus' - -- | The response status code.
deleteVPCEndpointServiceConfigurationsResponse ::
  -- | 'dvpcescrrsResponseStatus'
  Int ->
  DeleteVPCEndpointServiceConfigurationsResponse
...
dvpcescrrsUnsuccessful :: Lens' DeleteVPCEndpointServiceConfigurationsResponse [UnsuccessfulItem]
dvpcescrrsUnsuccessful = lens _dvpcescrrsUnsuccessful (\s a -> s {_dvpcescrrsUnsuccessful = a}) . _Default . _Coerce

-- | -- | The response status code.
dvpcescrrsResponseStatus :: Lens' DeleteVPCEndpointServiceConfigurationsResponse Int
dvpcescrrsResponseStatus = lens _dvpcescrrsResponseStatus (\s a -> s {_dvpcescrrsResponseStatus = a})
