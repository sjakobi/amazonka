...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dclvrrsReturn' - Returns @true@ if the request succeeds; otherwise, it returns an error.
--
-- * 'dclvrrsResponseStatus' - -- | The response status code.
detachClassicLinkVPCResponse ::
  -- | 'dclvrrsResponseStatus'
  Int ->
  DetachClassicLinkVPCResponse
detachClassicLinkVPCResponse pResponseStatus_ =
  DetachClassicLinkVPCResponse'
    { _dclvrrsReturn =
        Nothing,
      _dclvrrsResponseStatus = pResponseStatus_
    }

-- | Returns @true@ if the request succeeds; otherwise, it returns an error.
dclvrrsReturn :: Lens' DetachClassicLinkVPCResponse (Maybe Bool)
dclvrrsReturn = lens _dclvrrsReturn (\s a -> s {_dclvrrsReturn = a})

-- | -- | The response status code.
dclvrrsResponseStatus :: Lens' DetachClassicLinkVPCResponse Int
dclvrrsResponseStatus = lens _dclvrrsResponseStatus (\s a -> s {_dclvrrsResponseStatus = a})
