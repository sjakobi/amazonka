...
--
-- * 'abcCidr' - The address range, in CIDR notation. This must be the exact range that you provisioned. You can't advertise only a portion of the provisioned range.
advertiseByoipCidr ::
  -- | 'abcCidr'
  Text ->
  AdvertiseByoipCidr
...
instance Hashable AdvertiseByoipCidr

instance NFData AdvertiseByoipCidr
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'abcrrsByoipCidr' - Information about the address range.
--
-- * 'abcrrsResponseStatus' - -- | The response status code.
advertiseByoipCidrResponse ::
  -- | 'abcrrsResponseStatus'
  Int ->
  AdvertiseByoipCidrResponse
advertiseByoipCidrResponse pResponseStatus_ =
  AdvertiseByoipCidrResponse'
    { _abcrrsByoipCidr =
        Nothing,
      _abcrrsResponseStatus = pResponseStatus_
    }

-- | Information about the address range.
abcrrsByoipCidr :: Lens' AdvertiseByoipCidrResponse (Maybe ByoipCidr)
abcrrsByoipCidr = lens _abcrrsByoipCidr (\s a -> s {_abcrrsByoipCidr = a})

-- | -- | The response status code.
abcrrsResponseStatus :: Lens' AdvertiseByoipCidrResponse Int
abcrrsResponseStatus = lens _abcrrsResponseStatus (\s a -> s {_abcrrsResponseStatus = a})

instance NFData AdvertiseByoipCidrResponse
