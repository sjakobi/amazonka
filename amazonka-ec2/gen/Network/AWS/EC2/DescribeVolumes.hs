...
describeVolumes_filters = Lens.lens (\DescribeVolumes' {filters} -> filters) (\s@DescribeVolumes' {} a -> s {filters = a} :: DescribeVolumes) Prelude.. Lens.mapping Prelude._Coerce
    ...
    Response.receiveXML
      ( \s h x ->
          DescribeVolumesResponse'
            ...
            Prelude.<*> ( x Prelude..@? "volumeSet" Prelude..!@ Prelude.mempty
...
instance Prelude.ToQuery DescribeVolumes where
  toQuery DescribeVolumes' {..} =
    Prelude.mconcat
      [ "Action"
          Prelude.=: ("DescribeVolumes" :: Prelude.ByteString),
        "Version"
