...
data AcceptVPCEndpointConnectionsResponse = AcceptVPCEndpointConnectionsResponse'
  { _avecrrsUnsuccessful ::
      !( Maybe
           [UnsuccessfulItem]
       ),
    _avecrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'avecrrsUnsuccessful' - Information about the interface endpoints that were not accepted, if applicable.
--
-- * 'avecrrsResponseStatus' - -- | The response status code.
acceptVPCEndpointConnectionsResponse ::
  -- | 'avecrrsResponseStatus'
  Int ->
  AcceptVPCEndpointConnectionsResponse
...
avecrrsUnsuccessful :: Lens' AcceptVPCEndpointConnectionsResponse [UnsuccessfulItem]
avecrrsUnsuccessful = lens _avecrrsUnsuccessful (\s a -> s {_avecrrsUnsuccessful = a}) . _Default . _Coerce

-- | -- | The response status code.
avecrrsResponseStatus :: Lens' AcceptVPCEndpointConnectionsResponse Int
avecrrsResponseStatus = lens _avecrrsResponseStatus (\s a -> s {_avecrrsResponseStatus = a})
