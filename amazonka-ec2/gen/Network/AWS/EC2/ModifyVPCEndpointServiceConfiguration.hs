...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'mvescrrsReturn' - Returns @true@ if the request succeeds; otherwise, it returns an error.
--
-- * 'mvescrrsResponseStatus' - -- | The response status code.
modifyVPCEndpointServiceConfigurationResponse ::
  -- | 'mvescrrsResponseStatus'
  Int ->
  ModifyVPCEndpointServiceConfigurationResponse
...
mvescrrsReturn :: Lens' ModifyVPCEndpointServiceConfigurationResponse (Maybe Bool)
mvescrrsReturn = lens _mvescrrsReturn (\s a -> s {_mvescrrsReturn = a})

-- | -- | The response status code.
mvescrrsResponseStatus :: Lens' ModifyVPCEndpointServiceConfigurationResponse Int
mvescrrsResponseStatus = lens _mvescrrsResponseStatus (\s a -> s {_mvescrrsResponseStatus = a})
