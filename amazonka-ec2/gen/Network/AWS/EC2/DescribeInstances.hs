...
--
-- * 'diInstanceIds' - The instance IDs. Default: Describes all your instances.
--
-- * 'diNextToken' - The token to request the next page of results.
--
...
diFilters = lens _diFilters (\s a -> s {_diFilters = a}) . _Default . _Coerce

instance AWSPager DescribeInstances where
  page rq rs
    | stop (rs ^. dirrsNextToken) = Nothing
    | stop (rs ^. dirrsReservations) = Nothing
    | otherwise =
  ...
  toQuery DescribeInstances' {..} =
    mconcat
      [ "Action" =: ("DescribeInstances" :: ByteString),
        ...
        toQuery
          (toQueryList "InstanceId" <$> _diInstanceIds),
        "NextToken" =: _diNextToken,
        "DryRun" =: _diDryRun,
        "MaxResults" =: _diMaxResults,
        toQuery (toQueryList "Filter" <$> _diFilters)
      ]
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dirrsNextToken' - The token to use to retrieve the next page of results. This value is @null@ when there are no more results to return.
--
-- * 'dirrsReservations' - Information about the reservations.
--
...
describeInstancesResponse pResponseStatus_ =
  DescribeInstancesResponse'
    { _dirrsNextToken =
        Nothing,
      _dirrsReservations = Nothing,
      _dirrsResponseStatus = pResponseStatus_
    }

-- | The token to use to retrieve the next page of results. This value is @null@ when there are no more results to return.
dirrsNextToken :: Lens' DescribeInstancesResponse (Maybe Text)
dirrsNextToken = lens _dirrsNextToken (\s a -> s {_dirrsNextToken = a})

-- | Information about the reservations.
dirrsReservations :: Lens' DescribeInstancesResponse [Reservation]
dirrsReservations = lens _dirrsReservations (\s a -> s {_dirrsReservations = a}) . _Default . _Coerce

-- | -- | The response status code.
dirrsResponseStatus :: Lens' DescribeInstancesResponse Int
