...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'bgirrsUnprocessedKeys' - A map of tables and their respective keys that were not processed with the current response. The @UnprocessedKeys@ value is in the same form as @RequestItems@ , so the value can be provided directly to a subsequent @BatchGetItem@ operation. For more information, see @RequestItems@ in the Request Parameters section. Each element consists of:     * @Keys@ - An array of primary key attribute values that define specific items in the table.     * @ProjectionExpression@ - One or more attributes to be retrieved from the table or index. By default, all attributes are returned. If a requested attribute is not found, it does not appear in the result.     * @ConsistentRead@ - The consistency of a read operation. If set to @true@ , then a strongly consistent read is used; otherwise, an eventually consistent read is used. If there are no unprocessed keys remaining, the response contains an empty @UnprocessedKeys@ map.
--
-- * 'bgirrsConsumedCapacity' - The read capacity units consumed by the entire @BatchGetItem@ operation. Each element consists of:     * @TableName@ - The table that consumed the provisioned throughput.     * @CapacityUnits@ - The total number of capacity units consumed.
--
-- * 'bgirrsResponses' - A map of table name to a list of items. Each object in @Responses@ consists of a table name, along with a map of attribute data consisting of the data type and attribute value.
--
...
batchGetItemResponse pResponseStatus_ =
  BatchGetItemResponse'
    { _bgirrsUnprocessedKeys =
        Nothing,
      _bgirrsConsumedCapacity = Nothing,
      _bgirrsResponses = Nothing,
      _bgirrsResponseStatus = pResponseStatus_
    }

-- | A map of tables and their respective keys that were not processed with the current response. The @UnprocessedKeys@ value is in the same form as @RequestItems@ , so the value can be provided directly to a subsequent @BatchGetItem@ operation. For more information, see @RequestItems@ in the Request Parameters section. Each element consists of:     * @Keys@ - An array of primary key attribute values that define specific items in the table.     * @ProjectionExpression@ - One or more attributes to be retrieved from the table or index. By default, all attributes are returned. If a requested attribute is not found, it does not appear in the result.     * @ConsistentRead@ - The consistency of a read operation. If set to @true@ , then a strongly consistent read is used; otherwise, an eventually consistent read is used. If there are no unprocessed keys remaining, the response contains an empty @UnprocessedKeys@ map.
bgirrsUnprocessedKeys :: Lens' BatchGetItemResponse (HashMap Text KeysAndAttributes)
bgirrsUnprocessedKeys = lens _bgirrsUnprocessedKeys (\s a -> s {_bgirrsUnprocessedKeys = a}) . _Default . _Map
...
bgirrsConsumedCapacity :: Lens' BatchGetItemResponse [ConsumedCapacity]
bgirrsConsumedCapacity = lens _bgirrsConsumedCapacity (\s a -> s {_bgirrsConsumedCapacity = a}) . _Default . _Coerce

-- | A map of table name to a list of items. Each object in @Responses@ consists of a table name, along with a map of attribute data consisting of the data type and attribute value.
bgirrsResponses :: Lens' BatchGetItemResponse (HashMap Text [HashMap Text AttributeValue])
bgirrsResponses = lens _bgirrsResponses (\s a -> s {_bgirrsResponses = a}) . _Default . _Map

-- | -- | The response status code.
bgirrsResponseStatus :: Lens' BatchGetItemResponse Int
