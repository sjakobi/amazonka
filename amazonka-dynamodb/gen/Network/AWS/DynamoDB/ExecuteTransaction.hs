    ...
    object
      ( catMaybes
          [ ("ClientRequestToken" .=) <$> _etClientRequestToken,
            Just
              ("TransactStatements" .= _etTransactStatements)
          ]
      )
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'etrrsResponses' - The response to a PartiQL transaction.
--
-- * 'etrrsResponseStatus' - -- | The response status code.
executeTransactionResponse ::
  -- | 'etrrsResponseStatus'
  Int ->
  ExecuteTransactionResponse
executeTransactionResponse pResponseStatus_ =
  ExecuteTransactionResponse'
    { _etrrsResponses =
        Nothing,
      _etrrsResponseStatus = pResponseStatus_
    }

-- | The response to a PartiQL transaction.
etrrsResponses :: Lens' ExecuteTransactionResponse (Maybe (NonEmpty ItemResponse))
etrrsResponses = lens _etrrsResponses (\s a -> s {_etrrsResponses = a}) . mapping _List1

-- | -- | The response status code.
etrrsResponseStatus :: Lens' ExecuteTransactionResponse Int
etrrsResponseStatus = lens _etrrsResponseStatus (\s a -> s {_etrrsResponseStatus = a})
