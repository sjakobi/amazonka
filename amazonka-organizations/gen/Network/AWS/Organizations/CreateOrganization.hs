...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'corrsOrganization' - A structure that contains details about the newly created organization.
--
...
corrsOrganization :: Lens' CreateOrganizationResponse (Maybe Organization)
corrsOrganization = lens _corrsOrganization (\s a -> s {_corrsOrganization = a})
