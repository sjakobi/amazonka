...
instance AWSPager ListOrganizationalUnitsForParent where
  page rq rs
    | stop (rs ^. loufprrsNextToken) = Nothing
    | stop (rs ^. loufprrsOrganizationalUnits) = Nothing
    | otherwise =
      Just $ rq & loufpNextToken .~ rs ^. loufprrsNextToken
...
data ListOrganizationalUnitsForParentResponse = ListOrganizationalUnitsForParentResponse'
  { _loufprrsNextToken ::
    ...
    _loufprrsOrganizationalUnits ::
      !( Maybe
           [OrganizationalUnit]
       ),
    _loufprrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'loufprrsNextToken' - If present, indicates that more output is available than is included in the current response. Use this value in the @NextToken@ request parameter in a subsequent call to the operation to get the next part of the output. You should repeat this until the @NextToken@ response element comes back as @null@ .
--
-- * 'loufprrsOrganizationalUnits' - A list of the OUs in the specified root or parent OU.
--
-- * 'loufprrsResponseStatus' - -- | The response status code.
listOrganizationalUnitsForParentResponse ::
  -- | 'loufprrsResponseStatus'
  Int ->
  ListOrganizationalUnitsForParentResponse
...
loufprrsNextToken :: Lens' ListOrganizationalUnitsForParentResponse (Maybe Text)
loufprrsNextToken = lens _loufprrsNextToken (\s a -> s {_loufprrsNextToken = a})

-- | A list of the OUs in the specified root or parent OU.
loufprrsOrganizationalUnits :: Lens' ListOrganizationalUnitsForParentResponse [OrganizationalUnit]
loufprrsOrganizationalUnits = lens _loufprrsOrganizationalUnits (\s a -> s {_loufprrsOrganizationalUnits = a}) . _Default . _Coerce

-- | -- | The response status code.
loufprrsResponseStatus :: Lens' ListOrganizationalUnitsForParentResponse Int
loufprrsResponseStatus = lens _loufprrsResponseStatus (\s a -> s {_loufprrsResponseStatus = a})
