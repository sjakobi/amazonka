...
instance AWSPager ListAccountsForParent where
  page rq rs
    | stop (rs ^. lafprrsNextToken) = Nothing
    | stop (rs ^. lafprrsAccounts) = Nothing
    | otherwise =
      Just $ rq & lafpNextToken .~ rs ^. lafprrsNextToken

instance AWSRequest ListAccountsForParent where
  type
    Rs ListAccountsForParent =
      ListAccountsForParentResponse
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lafprrsNextToken' - If present, indicates that more output is available than is included in the current response. Use this value in the @NextToken@ request parameter in a subsequent call to the operation to get the next part of the output. You should repeat this until the @NextToken@ response element comes back as @null@ .
--
-- * 'lafprrsAccounts' - A list of the accounts in the specified root or OU.
--
-- * 'lafprrsResponseStatus' - -- | The response status code.
listAccountsForParentResponse ::
  -- | 'lafprrsResponseStatus'
  Int ->
  ListAccountsForParentResponse
listAccountsForParentResponse pResponseStatus_ =
  ListAccountsForParentResponse'
    { _lafprrsNextToken =
        Nothing,
      _lafprrsAccounts = Nothing,
      _lafprrsResponseStatus = pResponseStatus_
    }

-- | If present, indicates that more output is available than is included in the current response. Use this value in the @NextToken@ request parameter in a subsequent call to the operation to get the next part of the output. You should repeat this until the @NextToken@ response element comes back as @null@ .
lafprrsNextToken :: Lens' ListAccountsForParentResponse (Maybe Text)
lafprrsNextToken = lens _lafprrsNextToken (\s a -> s {_lafprrsNextToken = a})

-- | A list of the accounts in the specified root or OU.
lafprrsAccounts :: Lens' ListAccountsForParentResponse [Account]
lafprrsAccounts = lens _lafprrsAccounts (\s a -> s {_lafprrsAccounts = a}) . _Default . _Coerce

-- | -- | The response status code.
lafprrsResponseStatus :: Lens' ListAccountsForParentResponse Int
lafprrsResponseStatus = lens _lafprrsResponseStatus (\s a -> s {_lafprrsResponseStatus = a})
