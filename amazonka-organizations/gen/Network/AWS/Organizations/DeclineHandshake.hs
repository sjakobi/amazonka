...
--
-- * 'dhHandshakeId' - The unique identifier (ID) of the handshake that you want to decline. You can get the ID from the 'ListHandshakesForAccount' operation. The <http://wikipedia.org/wiki/regex regex pattern> for handshake ID string requires "h-" followed by from 8 to 32 lowercase letters or digits.
declineHandshake ::
  -- | 'dhHandshakeId'
  Text ->
  DeclineHandshake
declineHandshake pHandshakeId_ =
  DeclineHandshake' {_dhHandshakeId = pHandshakeId_}
...
dhHandshakeId :: Lens' DeclineHandshake Text
