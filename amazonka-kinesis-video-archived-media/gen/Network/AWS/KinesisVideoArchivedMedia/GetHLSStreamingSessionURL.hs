...
module Network.AWS.KinesisVideoArchivedMedia.GetHLSStreamingSessionURL
  ( -- * Creating a Request
    ...
    -- * Response Lenses
    ghlsssurrsHLSStreamingSessionURL,
    ghlsssurrsResponseStatus,
  )
where
...
data GetHLSStreamingSessionURLResponse = GetHLSStreamingSessionURLResponse'
  { _ghlsssurrsHLSStreamingSessionURL ::
      !( Maybe
           Text
       ),
    _ghlsssurrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ghlsssurrsHLSStreamingSessionURL' - The URL (containing the session token) that a media player can use to retrieve the HLS master playlist.
--
-- * 'ghlsssurrsResponseStatus' - -- | The response status code.
getHLSStreamingSessionURLResponse ::
  -- | 'ghlsssurrsResponseStatus'
  Int ->
  GetHLSStreamingSessionURLResponse
...
ghlsssurrsHLSStreamingSessionURL :: Lens' GetHLSStreamingSessionURLResponse (Maybe Text)
ghlsssurrsHLSStreamingSessionURL = lens _ghlsssurrsHLSStreamingSessionURL (\s a -> s {_ghlsssurrsHLSStreamingSessionURL = a})

-- | -- | The response status code.
ghlsssurrsResponseStatus :: Lens' GetHLSStreamingSessionURLResponse Int
ghlsssurrsResponseStatus = lens _ghlsssurrsResponseStatus (\s a -> s {_ghlsssurrsResponseStatus = a})
