...
module Network.AWS.KinesisVideoArchivedMedia.GetDASHStreamingSessionURL
  ( -- * Creating a Request
    ...
    -- * Response Lenses
    gdashssurrsDASHStreamingSessionURL,
    gdashssurrsResponseStatus,
  )
where
...
data GetDASHStreamingSessionURLResponse = GetDASHStreamingSessionURLResponse'
  { _gdashssurrsDASHStreamingSessionURL ::
      !( Maybe
           Text
       ),
    _gdashssurrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gdashssurrsDASHStreamingSessionURL' - The URL (containing the session token) that a media player can use to retrieve the MPEG-DASH manifest.
--
-- * 'gdashssurrsResponseStatus' - -- | The response status code.
getDASHStreamingSessionURLResponse ::
  -- | 'gdashssurrsResponseStatus'
  Int ->
  GetDASHStreamingSessionURLResponse
...
gdashssurrsDASHStreamingSessionURL :: Lens' GetDASHStreamingSessionURLResponse (Maybe Text)
gdashssurrsDASHStreamingSessionURL = lens _gdashssurrsDASHStreamingSessionURL (\s a -> s {_gdashssurrsDASHStreamingSessionURL = a})

-- | -- | The response status code.
gdashssurrsResponseStatus :: Lens' GetDASHStreamingSessionURLResponse Int
gdashssurrsResponseStatus = lens _gdashssurrsResponseStatus (\s a -> s {_gdashssurrsResponseStatus = a})
