...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'actcrrsResult' - True if 'AddCommunicationToCase' succeeds. Otherwise, returns an error.
--
-- * 'actcrrsResponseStatus' - -- | The response status code.
addCommunicationToCaseResponse ::
  -- | 'actcrrsResponseStatus'
  Int ->
  AddCommunicationToCaseResponse
addCommunicationToCaseResponse pResponseStatus_ =
  AddCommunicationToCaseResponse'
    { _actcrrsResult =
        Nothing,
      _actcrrsResponseStatus = pResponseStatus_
    }

-- | True if 'AddCommunicationToCase' succeeds. Otherwise, returns an error.
actcrrsResult :: Lens' AddCommunicationToCaseResponse (Maybe Bool)
actcrrsResult = lens _actcrrsResult (\s a -> s {_actcrrsResult = a})

-- | -- | The response status code.
actcrrsResponseStatus :: Lens' AddCommunicationToCaseResponse Int
actcrrsResponseStatus = lens _actcrrsResponseStatus (\s a -> s {_actcrrsResponseStatus = a})
