...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'garrsTemporaryCredential' - A @TemporaryCredential@ object that contains the data needed to log in to the instance by RDP clients, such as the Microsoft Remote Desktop Connection.
--
...
garrsTemporaryCredential :: Lens' GrantAccessResponse (Maybe TemporaryCredential)
garrsTemporaryCredential = lens _garrsTemporaryCredential (\s a -> s {_garrsTemporaryCredential = a})

-- | -- | The response status code.
garrsResponseStatus :: Lens' GrantAccessResponse Int
