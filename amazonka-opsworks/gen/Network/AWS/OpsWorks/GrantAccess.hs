...
data GrantAccess = GrantAccess'
  { -- | The length of time (in minutes) that the grant is valid. When the grant
    ...
    validForInMinutes :: Prelude.Maybe Prelude.Nat,
...
grantAccess_validForInMinutes = Lens.lens (\GrantAccess' {validForInMinutes} -> validForInMinutes) (\s@GrantAccess' {} a -> s {validForInMinutes = a} :: GrantAccess) Prelude.. Lens.mapping Prelude._Nat
