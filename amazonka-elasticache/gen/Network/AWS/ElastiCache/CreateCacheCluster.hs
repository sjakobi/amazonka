...
data CreateCacheCluster = CreateCacheCluster'
  { _cccSecurityGroupIds ::
    ...
    _cccCacheSecurityGroupNames ::
      !(Maybe [Text]),
    _cccReplicationGroupId ::
      !(Maybe Text),
    _cccSnapshotWindow ::
      !(Maybe Text),
    _cccPreferredOutpostARNs ::
      !(Maybe [Text]),
    _cccOutpostMode ::
      !(Maybe OutpostMode),
    ...
    _cccCacheNodeType ::
      !(Maybe Text),
    _cccTags :: !(Maybe [Tag]),
    _cccNotificationTopicARN ::
      !(Maybe Text),
    _cccPort :: !(Maybe Int),
    _cccEngine :: !(Maybe Text),
    _cccPreferredOutpostARN ::
      !(Maybe Text),
...
--
-- * 'cccPreferredOutpostARNs' - The outpost ARNs in which the cache cluster is created.
--
-- * 'cccOutpostMode' - Specifies whether the nodes in the cluster are created in a single outpost or across multiple outposts.
--
...
--
-- * 'cccPreferredAvailabilityZone' - The EC2 Availability Zone in which the cluster is created. All nodes belonging to this cluster are placed in the preferred Availability Zone. If you want to create your nodes across multiple Availability Zones, use @PreferredAvailabilityZones@ . Default: System chosen Availability Zone.
--
-- * 'cccCacheSubnetGroupName' - The name of the subnet group to be used for the cluster. Use this parameter only when you are creating a cluster in an Amazon Virtual Private Cloud (Amazon VPC). /Important:/ If you're going to launch your cluster in an Amazon VPC, you need to create a subnet group before you start creating a cluster. For more information, see <https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/SubnetGroups.html Subnets and Subnet Groups> .
--
-- * 'cccEngineVersion' - The version number of the cache engine to be used for this cluster. To view the supported cache engine versions, use the DescribeCacheEngineVersions operation. __Important:__ You can upgrade to a newer engine version (see <https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/SelectEngine.html#VersionManagement Selecting a Cache Engine and Version> ), but you cannot downgrade to an earlier engine version. If you want to use an earlier engine version, you must delete the existing cluster or replication group and create it anew with the earlier engine version.
--
-- * 'cccPreferredMaintenanceWindow' - Specifies the weekly time range during which maintenance on the cluster is performed. It is specified as a range in the format ddd:hh24:mi-ddd:hh24:mi (24H Clock UTC). The minimum maintenance window is a 60 minute period. Valid values for @ddd@ are:
--
...
--
-- * 'cccPreferredOutpostARN' - The outpost ARN in which the cache cluster is created.
--
...
--
-- * 'cccCacheClusterId' - The node group (shard) identifier. This parameter is stored as a lowercase string. __Constraints:__      * A name must contain from 1 to 50 alphanumeric characters or hyphens.     * The first character must be a letter.     * A name cannot end with a hyphen or contain two consecutive hyphens.
createCacheCluster ::
  -- | 'cccCacheClusterId'
  Text ->
  CreateCacheCluster
createCacheCluster pCacheClusterId_ =
  CreateCacheCluster'
    { _cccSecurityGroupIds = Nothing,
      ...
      _cccPreferredOutpostARNs = Nothing,
      _cccOutpostMode = Nothing,
      ...
      _cccPort = Nothing,
      _cccEngine = Nothing,
      _cccPreferredOutpostARN = Nothing,
...
cccPreferredOutpostARNs :: Lens' CreateCacheCluster [Text]
cccPreferredOutpostARNs = lens _cccPreferredOutpostARNs (\s a -> s {_cccPreferredOutpostARNs = a}) . _Default . _Coerce

-- | Specifies whether the nodes in the cluster are created in a single outpost or across multiple outposts.
cccOutpostMode :: Lens' CreateCacheCluster (Maybe OutpostMode)
cccOutpostMode = lens _cccOutpostMode (\s a -> s {_cccOutpostMode = a})
...
-- | The name of the subnet group to be used for the cluster. Use this parameter only when you are creating a cluster in an Amazon Virtual Private Cloud (Amazon VPC). /Important:/ If you're going to launch your cluster in an Amazon VPC, you need to create a subnet group before you start creating a cluster. For more information, see <https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/SubnetGroups.html Subnets and Subnet Groups> .
...
-- | The version number of the cache engine to be used for this cluster. To view the supported cache engine versions, use the DescribeCacheEngineVersions operation. __Important:__ You can upgrade to a newer engine version (see <https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/SelectEngine.html#VersionManagement Selecting a Cache Engine and Version> ), but you cannot downgrade to an earlier engine version. If you want to use an earlier engine version, you must delete the existing cluster or replication group and create it anew with the earlier engine version.
...
cccPreferredOutpostARN :: Lens' CreateCacheCluster (Maybe Text)
cccPreferredOutpostARN = lens _cccPreferredOutpostARN (\s a -> s {_cccPreferredOutpostARN = a})
...
-- | The node group (shard) identifier. This parameter is stored as a lowercase string. __Constraints:__      * A name must contain from 1 to 50 alphanumeric characters or hyphens.     * The first character must be a letter.     * A name cannot end with a hyphen or contain two consecutive hyphens.
    ...
    mconcat
      [ "Action" =: ("CreateCacheCluster" :: ByteString),
        ...
        "PreferredOutpostArns"
          =: toQuery
            ( toQueryList "PreferredOutpostArn"
                <$> _cccPreferredOutpostARNs
            ),
        "OutpostMode" =: _cccOutpostMode,
        ...
        "Port" =: _cccPort,
        "Engine" =: _cccEngine,
        "PreferredOutpostArn" =: _cccPreferredOutpostARN,
        "SnapshotName" =: _cccSnapshotName,
        "AuthToken" =: _cccAuthToken,
        "AZMode" =: _cccAZMode,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'cccrrsCacheCluster' - Undocumented member.
--
-- * 'cccrrsResponseStatus' - -- | The response status code.
createCacheClusterResponse ::
  -- | 'cccrrsResponseStatus'
  Int ->
  CreateCacheClusterResponse
createCacheClusterResponse pResponseStatus_ =
  CreateCacheClusterResponse'
    { _cccrrsCacheCluster =
        Nothing,
      _cccrrsResponseStatus = pResponseStatus_
    }

-- | Undocumented member.
cccrrsCacheCluster :: Lens' CreateCacheClusterResponse (Maybe CacheCluster)
cccrrsCacheCluster = lens _cccrrsCacheCluster (\s a -> s {_cccrrsCacheCluster = a})

-- | -- | The response status code.
cccrrsResponseStatus :: Lens' CreateCacheClusterResponse Int
cccrrsResponseStatus = lens _cccrrsResponseStatus (\s a -> s {_cccrrsResponseStatus = a})
