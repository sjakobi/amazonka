...
instance AWSPager DescribeReservedCacheNodes where
  page rq rs
    | stop (rs ^. drcnrrsMarker) = Nothing
    | stop (rs ^. drcnrrsReservedCacheNodes) = Nothing
    | otherwise =
      Just $ rq & drcnMarker .~ rs ^. drcnrrsMarker
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'drcnrrsReservedCacheNodes' - A list of reserved cache nodes. Each element in the list contains detailed information about one node.
--
-- * 'drcnrrsMarker' - Provides an identifier to allow retrieval of paginated results.
--
-- * 'drcnrrsResponseStatus' - -- | The response status code.
describeReservedCacheNodesResponse ::
  -- | 'drcnrrsResponseStatus'
  Int ->
  DescribeReservedCacheNodesResponse
describeReservedCacheNodesResponse pResponseStatus_ =
  DescribeReservedCacheNodesResponse'
    { _drcnrrsReservedCacheNodes =
        Nothing,
      _drcnrrsMarker = Nothing,
      _drcnrrsResponseStatus =
        pResponseStatus_
    }

-- | A list of reserved cache nodes. Each element in the list contains detailed information about one node.
drcnrrsReservedCacheNodes :: Lens' DescribeReservedCacheNodesResponse [ReservedCacheNode]
drcnrrsReservedCacheNodes = lens _drcnrrsReservedCacheNodes (\s a -> s {_drcnrrsReservedCacheNodes = a}) . _Default . _Coerce

-- | Provides an identifier to allow retrieval of paginated results.
drcnrrsMarker :: Lens' DescribeReservedCacheNodesResponse (Maybe Text)
drcnrrsMarker = lens _drcnrrsMarker (\s a -> s {_drcnrrsMarker = a})

-- | -- | The response status code.
drcnrrsResponseStatus :: Lens' DescribeReservedCacheNodesResponse Int
drcnrrsResponseStatus = lens _drcnrrsResponseStatus (\s a -> s {_drcnrrsResponseStatus = a})
