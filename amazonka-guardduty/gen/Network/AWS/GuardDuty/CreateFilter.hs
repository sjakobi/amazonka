...
--
-- * 'cfDetectorId' - The ID of the detector belonging to the GuardDuty account that you want to create a filter for.
--
-- * 'cfName' - The name of the filter. Minimum length of 3. Maximum length of 64. Valid characters include alphanumeric characters, dot (.), underscore (_), and dash (-). Spaces are not allowed.
--
...
--
-- * 'cfrrsName' - The name of the successfully created filter.
createFilterResponse ::
  -- | 'cfrrsResponseStatus'
  Int ->
  -- | 'cfrrsName'
...
cfrrsResponseStatus :: Lens' CreateFilterResponse Int
cfrrsResponseStatus = lens _cfrrsResponseStatus (\s a -> s {_cfrrsResponseStatus = a})

-- | The name of the successfully created filter.
cfrrsName :: Lens' CreateFilterResponse Text
cfrrsName = lens _cfrrsName (\s a -> s {_cfrrsName = a})
