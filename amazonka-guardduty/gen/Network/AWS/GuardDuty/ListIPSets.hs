...
instance AWSPager ListIPSets where
  page rq rs
    | stop (rs ^. lisrrsNextToken) = Nothing
    | stop (rs ^. lisrrsIPSetIds) = Nothing
    | otherwise =
      Just $ rq & lisNextToken .~ rs ^. lisrrsNextToken
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lisrrsNextToken' - The pagination parameter to be used on the next list operation to retrieve more items.
--
-- * 'lisrrsResponseStatus' - -- | The response status code.
--
-- * 'lisrrsIPSetIds' - The IDs of the IPSet resources.
listIPSetsResponse ::
  -- | 'lisrrsResponseStatus'
  Int ->
  ListIPSetsResponse
listIPSetsResponse pResponseStatus_ =
  ListIPSetsResponse'
    { _lisrrsNextToken = Nothing,
      _lisrrsResponseStatus = pResponseStatus_,
      _lisrrsIPSetIds = mempty
    }

-- | The pagination parameter to be used on the next list operation to retrieve more items.
lisrrsNextToken :: Lens' ListIPSetsResponse (Maybe Text)
lisrrsNextToken = lens _lisrrsNextToken (\s a -> s {_lisrrsNextToken = a})
...
lisrrsIPSetIds :: Lens' ListIPSetsResponse [Text]
lisrrsIPSetIds = lens _lisrrsIPSetIds (\s a -> s {_lisrrsIPSetIds = a}) . _Coerce
