...
instance ToJSON DisableOrganizationAdminAccount where
  toJSON DisableOrganizationAdminAccount' {..} =
    object
      ( catMaybes
          [Just ("adminAccountId" .= _doaaAdminAccountId)]
      )
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'doaarrsResponseStatus' - -- | The response status code.
disableOrganizationAdminAccountResponse ::
  -- | 'doaarrsResponseStatus'
  Int ->
  DisableOrganizationAdminAccountResponse
...
doaarrsResponseStatus :: Lens' DisableOrganizationAdminAccountResponse Int
doaarrsResponseStatus = lens _doaarrsResponseStatus (\s a -> s {_doaarrsResponseStatus = a})
