...
data UpdateUserPoolDomainResponse = UpdateUserPoolDomainResponse'
  { _uupdrrsCloudFrontDomain ::
      !(Maybe Text),
    _uupdrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'uupdrrsCloudFrontDomain' - The Amazon CloudFront endpoint that Amazon Cognito set up when you added the custom domain to your user pool.
--
-- * 'uupdrrsResponseStatus' - -- | The response status code.
updateUserPoolDomainResponse ::
  -- | 'uupdrrsResponseStatus'
  Int ->
  UpdateUserPoolDomainResponse
updateUserPoolDomainResponse pResponseStatus_ =
  UpdateUserPoolDomainResponse'
    { _uupdrrsCloudFrontDomain =
        Nothing,
      _uupdrrsResponseStatus = pResponseStatus_
    }

-- | The Amazon CloudFront endpoint that Amazon Cognito set up when you added the custom domain to your user pool.
uupdrrsCloudFrontDomain :: Lens' UpdateUserPoolDomainResponse (Maybe Text)
uupdrrsCloudFrontDomain = lens _uupdrrsCloudFrontDomain (\s a -> s {_uupdrrsCloudFrontDomain = a})

-- | -- | The response status code.
uupdrrsResponseStatus :: Lens' UpdateUserPoolDomainResponse Int
uupdrrsResponseStatus = lens _uupdrrsResponseStatus (\s a -> s {_uupdrrsResponseStatus = a})
