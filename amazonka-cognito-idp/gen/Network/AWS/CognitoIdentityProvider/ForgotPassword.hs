...
data ForgotPasswordResponse = ForgotPasswordResponse'
  { _fprrsCodeDeliveryDetails ::
      !( Maybe
           CodeDeliveryDetailsType
       ),
    _fprrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'fprrsCodeDeliveryDetails' - The code delivery details returned by the server in response to the request to reset a password.
--
-- * 'fprrsResponseStatus' - -- | The response status code.
forgotPasswordResponse ::
  -- | 'fprrsResponseStatus'
  Int ->
  ForgotPasswordResponse
forgotPasswordResponse pResponseStatus_ =
  ForgotPasswordResponse'
    { _fprrsCodeDeliveryDetails =
        Nothing,
      _fprrsResponseStatus = pResponseStatus_
    }

-- | The code delivery details returned by the server in response to the request to reset a password.
fprrsCodeDeliveryDetails :: Lens' ForgotPasswordResponse (Maybe CodeDeliveryDetailsType)
fprrsCodeDeliveryDetails = lens _fprrsCodeDeliveryDetails (\s a -> s {_fprrsCodeDeliveryDetails = a})

-- | -- | The response status code.
fprrsResponseStatus :: Lens' ForgotPasswordResponse Int
fprrsResponseStatus = lens _fprrsResponseStatus (\s a -> s {_fprrsResponseStatus = a})
