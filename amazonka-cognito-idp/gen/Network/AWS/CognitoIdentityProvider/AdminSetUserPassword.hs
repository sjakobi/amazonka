...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'asuprrsResponseStatus' - -- | The response status code.
adminSetUserPasswordResponse ::
  -- | 'asuprrsResponseStatus'
  Int ->
  AdminSetUserPasswordResponse
...
asuprrsResponseStatus :: Lens' AdminSetUserPasswordResponse Int
asuprrsResponseStatus = lens _asuprrsResponseStatus (\s a -> s {_asuprrsResponseStatus = a})
