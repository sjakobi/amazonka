...
data AdminListDevicesResponse = AdminListDevicesResponse'
  { _aldrrsDevices ::
      !(Maybe [DeviceType]),
    _aldrrsPaginationToken ::
      !(Maybe Text),
    _aldrrsResponseStatus ::
      !Int
  }
  deriving (Eq, Show, Data, Typeable, Generic)
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'aldrrsDevices' - The devices in the list of devices response.
--
-- * 'aldrrsPaginationToken' - The pagination token.
--
-- * 'aldrrsResponseStatus' - -- | The response status code.
adminListDevicesResponse ::
  -- | 'aldrrsResponseStatus'
  Int ->
  AdminListDevicesResponse
adminListDevicesResponse pResponseStatus_ =
  AdminListDevicesResponse'
    { _aldrrsDevices = Nothing,
      _aldrrsPaginationToken = Nothing,
      _aldrrsResponseStatus = pResponseStatus_
    }

-- | The devices in the list of devices response.
aldrrsDevices :: Lens' AdminListDevicesResponse [DeviceType]
aldrrsDevices = lens _aldrrsDevices (\s a -> s {_aldrrsDevices = a}) . _Default . _Coerce

-- | The pagination token.
aldrrsPaginationToken :: Lens' AdminListDevicesResponse (Maybe Text)
aldrrsPaginationToken = lens _aldrrsPaginationToken (\s a -> s {_aldrrsPaginationToken = a})

-- | -- | The response status code.
aldrrsResponseStatus :: Lens' AdminListDevicesResponse Int
aldrrsResponseStatus = lens _aldrrsResponseStatus (\s a -> s {_aldrrsResponseStatus = a})
