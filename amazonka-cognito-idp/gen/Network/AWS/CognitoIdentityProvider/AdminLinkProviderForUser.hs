...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'alpfurrsResponseStatus' - -- | The response status code.
adminLinkProviderForUserResponse ::
  -- | 'alpfurrsResponseStatus'
  Int ->
  AdminLinkProviderForUserResponse
...
alpfurrsResponseStatus :: Lens' AdminLinkProviderForUserResponse Int
alpfurrsResponseStatus = lens _alpfurrsResponseStatus (\s a -> s {_alpfurrsResponseStatus = a})
