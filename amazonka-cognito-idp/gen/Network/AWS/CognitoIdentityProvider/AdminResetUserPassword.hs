...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'aruprrsResponseStatus' - -- | The response status code.
adminResetUserPasswordResponse ::
  -- | 'aruprrsResponseStatus'
  Int ->
  AdminResetUserPasswordResponse
...
aruprrsResponseStatus :: Lens' AdminResetUserPasswordResponse Int
aruprrsResponseStatus = lens _aruprrsResponseStatus (\s a -> s {_aruprrsResponseStatus = a})
