...
data CreateUserPoolDomainResponse = CreateUserPoolDomainResponse'
  { _cupdrrsCloudFrontDomain ::
      !(Maybe Text),
    _cupdrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'cupdrrsCloudFrontDomain' - The Amazon CloudFront endpoint that you use as the target of the alias that you set up with your Domain Name Service (DNS) provider.
--
-- * 'cupdrrsResponseStatus' - -- | The response status code.
createUserPoolDomainResponse ::
  -- | 'cupdrrsResponseStatus'
  Int ->
  CreateUserPoolDomainResponse
createUserPoolDomainResponse pResponseStatus_ =
  CreateUserPoolDomainResponse'
    { _cupdrrsCloudFrontDomain =
        Nothing,
      _cupdrrsResponseStatus = pResponseStatus_
    }

-- | The Amazon CloudFront endpoint that you use as the target of the alias that you set up with your Domain Name Service (DNS) provider.
cupdrrsCloudFrontDomain :: Lens' CreateUserPoolDomainResponse (Maybe Text)
cupdrrsCloudFrontDomain = lens _cupdrrsCloudFrontDomain (\s a -> s {_cupdrrsCloudFrontDomain = a})

-- | -- | The response status code.
cupdrrsResponseStatus :: Lens' CreateUserPoolDomainResponse Int
cupdrrsResponseStatus = lens _cupdrrsResponseStatus (\s a -> s {_cupdrrsResponseStatus = a})
