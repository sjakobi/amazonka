...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'audsrrsResponseStatus' - -- | The response status code.
adminUpdateDeviceStatusResponse ::
  -- | 'audsrrsResponseStatus'
  Int ->
  AdminUpdateDeviceStatusResponse
...
audsrrsResponseStatus :: Lens' AdminUpdateDeviceStatusResponse Int
audsrrsResponseStatus = lens _audsrrsResponseStatus (\s a -> s {_audsrrsResponseStatus = a})
