...
-- in the /AWS Batch User Guide/.
--
-- 'containerOverrides', 'submitJob_containerOverrides' - A list of container overrides in JSON format that specify the name of a
-- container in the specified job definition and the overrides it should
-- receive. You can override the default command for a container (that\'s
-- specified in the job definition or the Docker image) with a @command@
-- override. You can also override existing environment variables (that are
...
-- are allowed.
--
-- 'jobQueue', 'submitJob_jobQueue' - The job queue into which the job is submitted. You can specify either
-- the name or the Amazon Resource Name (ARN) of the queue.
--
...
submitJob_dependsOn = Lens.lens (\SubmitJob' {dependsOn} -> dependsOn) (\s@SubmitJob' {} a -> s {dependsOn = a} :: SubmitJob) Prelude.. Lens.mapping Prelude._Coerce
...
submitJob_tags = Lens.lens (\SubmitJob' {tags} -> tags) (\s@SubmitJob' {} a -> s {tags = a} :: SubmitJob) Prelude.. Lens.mapping Prelude._Map
...
submitJob_parameters = Lens.lens (\SubmitJob' {parameters} -> parameters) (\s@SubmitJob' {} a -> s {parameters = a} :: SubmitJob) Prelude.. Lens.mapping Prelude._Map
...
instance Prelude.AWSRequest SubmitJob where
...
instance Prelude.ToHeaders SubmitJob where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "Content-Type"
...
instance Prelude.ToJSON SubmitJob where
  toJSON SubmitJob' {..} =
    Prelude.object
      ( Prelude.catMaybes
          [ ("dependsOn" Prelude..=) Prelude.<$> dependsOn,
            ...
            ("nodeOverrides" Prelude..=)
              Prelude.<$> nodeOverrides,
...
instance Prelude.ToPath SubmitJob where
  toPath = Prelude.const "/v1/submitjob"

instance Prelude.ToQuery SubmitJob where
