...
-- | An object representing the attributes of a compute environment that can be updated. For more information, see <https://docs.aws.amazon.com/batch/latest/userguide/compute_environments.html Compute Environments> in the /AWS Batch User Guide/ .
--
--
--
-- /See:/ 'computeResourceUpdate' smart constructor.
data ComputeResourceUpdate = ComputeResourceUpdate'
  { _cruSecurityGroupIds ::
      !(Maybe [Text]),
    _cruMinvCPUs ::
      !(Maybe Int),
    _cruMaxvCPUs ::
...
cruSecurityGroupIds = lens _cruSecurityGroupIds (\s a -> s {_cruSecurityGroupIds = a}) . _Default . _Coerce
...
cruSubnets = lens _cruSubnets (\s a -> s {_cruSubnets = a}) . _Default . _Coerce
