...
data CreateDBCluster = CreateDBCluster'
  { _cdcBackupRetentionPeriod ::
    ...
    _cdcEnableGlobalWriteForwarding ::
      !(Maybe Bool),
    _cdcTags :: !(Maybe [Tag]),
    _cdcPort :: !(Maybe Int),
...
--
-- * 'cdcEnableGlobalWriteForwarding' - A value that indicates whether to enable this DB cluster to forward write operations to the primary cluster of an Aurora global database ('GlobalCluster' ). By default, write operations are not allowed on Aurora DB clusters that are secondary clusters in an Aurora global database. You can set this value only on Aurora DB clusters that are members of an Aurora global database. With this parameter enabled, a secondary cluster can forward writes to the current primary cluster and the resulting changes are replicated back to this cluster. For the primary DB cluster of an Aurora global database, this value is used immediately if the primary is demoted by the 'FailoverGlobalCluster' API operation, but it does nothing until then.
--
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'cdcrrsDBCluster' - Undocumented member.
--
...
-- | Undocumented member.
cdcrrsDBCluster :: Lens' CreateDBClusterResponse (Maybe DBCluster)
cdcrrsDBCluster = lens _cdcrrsDBCluster (\s a -> s {_cdcrrsDBCluster = a})

-- | -- | The response status code.
cdcrrsResponseStatus :: Lens' CreateDBClusterResponse Int
