...
data RestoreDBInstanceFromS3 = RestoreDBInstanceFromS3'
  { _rdifsBackupRetentionPeriod ::
      !(Maybe Int),
    _rdifsDeletionProtection ::
      !(Maybe Bool),
    _rdifsStorageEncrypted ::
      !(Maybe Bool),
    ...
    _rdifsUseDefaultProcessorFeatures ::
      !(Maybe Bool),
    _rdifsMonitoringInterval ::
      !(Maybe Int),
    _rdifsOptionGroupName ::
      !(Maybe Text),
    _rdifsMonitoringRoleARN ::
      !(Maybe Text),
    _rdifsDBSubnetGroupName ::
      !(Maybe Text),
    _rdifsMasterUserPassword ::
      !(Maybe Text),
    _rdifsMasterUsername ::
      !(Maybe Text),
    _rdifsMultiAZ ::
      !(Maybe Bool),
    _rdifsPubliclyAccessible ::
      !(Maybe Bool),
    ...
    _rdifsKMSKeyId ::
      !(Maybe Text),
    _rdifsDBParameterGroupName ::
      !(Maybe Text),
    _rdifsAvailabilityZone ::
      !(Maybe Text),
    _rdifsEngineVersion ::
      !(Maybe Text),
    ...
    _rdifsDBName ::
      !(Maybe Text),
    _rdifsCopyTagsToSnapshot ::
      !(Maybe Bool),
    _rdifsS3Prefix ::
      !(Maybe Text),
    _rdifsAllocatedStorage ::
      !(Maybe Int),
    _rdifsIOPS ::
      !(Maybe Int),
    _rdifsAutoMinorVersionUpgrade ::
      !(Maybe Bool),
    _rdifsDBInstanceIdentifier ::
      !Text,
    _rdifsDBInstanceClass ::
      !Text,
    _rdifsEngine :: !Text,
    _rdifsSourceEngine ::
      !Text,
    _rdifsSourceEngineVersion ::
      !Text,
    _rdifsS3BucketName ::
      !Text,
    _rdifsS3IngestionRoleARN ::
      !Text
  }
  deriving
    ( Eq,
...
--
-- * 'rdifsMaxAllocatedStorage' - The upper limit to which Amazon RDS can automatically scale the storage of the DB instance. For more information about this setting, including limitations that apply to it, see <https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_PIOPS.StorageTypes.html#USER_PIOPS.Autoscaling Managing capacity automatically with Amazon RDS storage autoscaling> in the /Amazon RDS User Guide/ .
--
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rdifsrrsDBInstance' - Undocumented member.
--
-- * 'rdifsrrsResponseStatus' - -- | The response status code.
restoreDBInstanceFromS3Response ::
  -- | 'rdifsrrsResponseStatus'
  Int ->
  RestoreDBInstanceFromS3Response
restoreDBInstanceFromS3Response pResponseStatus_ =
  RestoreDBInstanceFromS3Response'
    { _rdifsrrsDBInstance =
        Nothing,
      _rdifsrrsResponseStatus = pResponseStatus_
    }

-- | Undocumented member.
rdifsrrsDBInstance :: Lens' RestoreDBInstanceFromS3Response (Maybe DBInstance)
rdifsrrsDBInstance = lens _rdifsrrsDBInstance (\s a -> s {_rdifsrrsDBInstance = a})

-- | -- | The response status code.
rdifsrrsResponseStatus :: Lens' RestoreDBInstanceFromS3Response Int
rdifsrrsResponseStatus = lens _rdifsrrsResponseStatus (\s a -> s {_rdifsrrsResponseStatus = a})
