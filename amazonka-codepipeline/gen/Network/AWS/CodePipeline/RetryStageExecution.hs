...
data RetryStageExecutionResponse = RetryStageExecutionResponse'
  { _rserrsPipelineExecutionId ::
      !(Maybe Text),
    _rserrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rserrsPipelineExecutionId' - The ID of the current workflow execution in the failed stage.
--
-- * 'rserrsResponseStatus' - -- | The response status code.
retryStageExecutionResponse ::
  -- | 'rserrsResponseStatus'
  Int ->
  RetryStageExecutionResponse
retryStageExecutionResponse pResponseStatus_ =
  RetryStageExecutionResponse'
    { _rserrsPipelineExecutionId =
        Nothing,
      _rserrsResponseStatus = pResponseStatus_
    }

-- | The ID of the current workflow execution in the failed stage.
rserrsPipelineExecutionId :: Lens' RetryStageExecutionResponse (Maybe Text)
rserrsPipelineExecutionId = lens _rserrsPipelineExecutionId (\s a -> s {_rserrsPipelineExecutionId = a})

-- | -- | The response status code.
rserrsResponseStatus :: Lens' RetryStageExecutionResponse Int
rserrsResponseStatus = lens _rserrsResponseStatus (\s a -> s {_rserrsResponseStatus = a})
