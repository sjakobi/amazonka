...
data GetPipelineExecutionResponse = GetPipelineExecutionResponse'
  { _gperrsPipelineExecution ::
      !( Maybe
           PipelineExecution
       ),
    _gperrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gperrsPipelineExecution' - Represents information about the execution of a pipeline.
--
-- * 'gperrsResponseStatus' - -- | The response status code.
getPipelineExecutionResponse ::
  -- | 'gperrsResponseStatus'
  Int ->
  GetPipelineExecutionResponse
getPipelineExecutionResponse pResponseStatus_ =
  GetPipelineExecutionResponse'
    { _gperrsPipelineExecution =
        Nothing,
      _gperrsResponseStatus = pResponseStatus_
    }

-- | Represents information about the execution of a pipeline.
gperrsPipelineExecution :: Lens' GetPipelineExecutionResponse (Maybe PipelineExecution)
gperrsPipelineExecution = lens _gperrsPipelineExecution (\s a -> s {_gperrsPipelineExecution = a})

-- | -- | The response status code.
gperrsResponseStatus :: Lens' GetPipelineExecutionResponse Int
gperrsResponseStatus = lens _gperrsResponseStatus (\s a -> s {_gperrsResponseStatus = a})
