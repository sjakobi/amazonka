...
--
-- Changes the password of the IAM user who is calling this operation. This operation can be performed using the AWS CLI, the AWS API, or the __My Security Credentials__ page in the AWS Management Console. The AWS account root user password is not affected by this operation.
--
--
-- Use 'UpdateLoginProfile' to use the AWS CLI, the AWS API, or the __Users__ page in the IAM console to change the password for any IAM user. For more information about modifying passwords, see <https://docs.aws.amazon.com/IAM/latest/UserGuide/Using_ManagingLogins.html Managing passwords> in the /IAM User Guide/ .
module Network.AWS.IAM.ChangePassword
  ( -- * Creating a Request
    changePassword,
    ChangePassword,
