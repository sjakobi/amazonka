...
--
--     * __management account__ – When you specify the management account, the resulting report lists all AWS services, because the management account is not limited by SCPs. For each service, the report includes data for only the management account.
--
...
instance ToQuery GenerateOrganizationsAccessReport where
  toQuery GenerateOrganizationsAccessReport' {..} =
    mconcat
      [ "Action"
          =: ("GenerateOrganizationsAccessReport" :: ByteString),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'goarrorsJobId' - The job identifier that you can use in the 'GetOrganizationsAccessReport' operation.
--
-- * 'goarrorsResponseStatus' - -- | The response status code.
generateOrganizationsAccessReportResponse ::
  -- | 'goarrorsResponseStatus'
  Int ->
  GenerateOrganizationsAccessReportResponse
...
goarrorsJobId :: Lens' GenerateOrganizationsAccessReportResponse (Maybe Text)
goarrorsJobId = lens _goarrorsJobId (\s a -> s {_goarrorsJobId = a})

-- | -- | The response status code.
goarrorsResponseStatus :: Lens' GenerateOrganizationsAccessReportResponse Int
goarrorsResponseStatus = lens _goarrorsResponseStatus (\s a -> s {_goarrorsResponseStatus = a})
