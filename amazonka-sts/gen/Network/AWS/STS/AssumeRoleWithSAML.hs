  ...
  toQuery AssumeRoleWithSAML' {..} =
    mconcat
      [ "Action" =: ("AssumeRoleWithSAML" :: ByteString),
        ...
        "PolicyArns"
          =: toQuery
            (toQueryList "member" <$> _arwsamlPolicyARNs),
...
data AssumeRoleWithSAMLResponse = AssumeRoleWithSAMLResponse'
  { _arwsamlrrsNameQualifier ::
      !(Maybe Text),
    _arwsamlrrsAudience ::
      !(Maybe Text),
    _arwsamlrrsSubjectType ::
      !(Maybe Text),
    _arwsamlrrsSubject ::
      !(Maybe Text),
    _arwsamlrrsIssuer ::
      !(Maybe Text),
    _arwsamlrrsCredentials ::
      !(Maybe AuthEnv),
    _arwsamlrrsAssumedRoleUser ::
      !( Maybe
           AssumedRoleUser
       ),
    _arwsamlrrsPackedPolicySize ::
      !(Maybe Nat),
    _arwsamlrrsResponseStatus ::
      !Int
  }
...
--
-- * 'arwsamlrrsAudience' - The value of the @Recipient@ attribute of the @SubjectConfirmationData@ element of the SAML assertion.
--
-- * 'arwsamlrrsSubjectType' - The format of the name ID, as defined by the @Format@ attribute in the @NameID@ element of the SAML assertion. Typical examples of the format are @transient@ or @persistent@ .  If the format includes the prefix @urn:oasis:names:tc:SAML:2.0:nameid-format@ , that prefix is removed. For example, @urn:oasis:names:tc:SAML:2.0:nameid-format:transient@ is returned as @transient@ . If the format includes any other prefix, the format is returned with no modifications.
--
-- * 'arwsamlrrsSubject' - The value of the @NameID@ element in the @Subject@ element of the SAML assertion.
--
-- * 'arwsamlrrsIssuer' - The value of the @Issuer@ element of the SAML assertion.
--
-- * 'arwsamlrrsCredentials' - The temporary security credentials, which include an access key ID, a secret access key, and a security (or session) token.
--
-- * 'arwsamlrrsAssumedRoleUser' - The identifiers for the temporary security credentials that the operation returns.
--
-- * 'arwsamlrrsPackedPolicySize' - A percentage value that indicates the packed size of the session policies and session tags combined passed in the request. The request fails if the packed size is greater than 100 percent, which means the policies and tags exceeded the allowed space.
--
-- * 'arwsamlrrsResponseStatus' - -- | The response status code.
assumeRoleWithSAMLResponse ::
  -- | 'arwsamlrrsResponseStatus'
  Int ->
  AssumeRoleWithSAMLResponse
assumeRoleWithSAMLResponse pResponseStatus_ =
  AssumeRoleWithSAMLResponse'
    { _arwsamlrrsNameQualifier =
        Nothing,
      _arwsamlrrsAudience = Nothing,
      _arwsamlrrsSubjectType = Nothing,
      _arwsamlrrsSubject = Nothing,
      _arwsamlrrsIssuer = Nothing,
      _arwsamlrrsCredentials = Nothing,
      _arwsamlrrsAssumedRoleUser = Nothing,
      _arwsamlrrsPackedPolicySize = Nothing,
      _arwsamlrrsResponseStatus = pResponseStatus_
    }

-- | A hash value based on the concatenation of the @Issuer@ response value, the AWS account ID, and the friendly name (the last part of the ARN) of the SAML provider in IAM. The combination of @NameQualifier@ and @Subject@ can be used to uniquely identify a federated user.  The following pseudocode shows how the hash value is calculated: @BASE64 ( SHA1 ( "https://example.com/saml" + "123456789012" + "/MySAMLIdP" ) )@
arwsamlrrsNameQualifier :: Lens' AssumeRoleWithSAMLResponse (Maybe Text)
arwsamlrrsNameQualifier = lens _arwsamlrrsNameQualifier (\s a -> s {_arwsamlrrsNameQualifier = a})

-- | The value of the @Recipient@ attribute of the @SubjectConfirmationData@ element of the SAML assertion.
arwsamlrrsAudience :: Lens' AssumeRoleWithSAMLResponse (Maybe Text)
arwsamlrrsAudience = lens _arwsamlrrsAudience (\s a -> s {_arwsamlrrsAudience = a})

-- | The format of the name ID, as defined by the @Format@ attribute in the @NameID@ element of the SAML assertion. Typical examples of the format are @transient@ or @persistent@ .  If the format includes the prefix @urn:oasis:names:tc:SAML:2.0:nameid-format@ , that prefix is removed. For example, @urn:oasis:names:tc:SAML:2.0:nameid-format:transient@ is returned as @transient@ . If the format includes any other prefix, the format is returned with no modifications.
arwsamlrrsSubjectType :: Lens' AssumeRoleWithSAMLResponse (Maybe Text)
arwsamlrrsSubjectType = lens _arwsamlrrsSubjectType (\s a -> s {_arwsamlrrsSubjectType = a})

-- | The value of the @NameID@ element in the @Subject@ element of the SAML assertion.
arwsamlrrsSubject :: Lens' AssumeRoleWithSAMLResponse (Maybe Text)
arwsamlrrsSubject = lens _arwsamlrrsSubject (\s a -> s {_arwsamlrrsSubject = a})

-- | The value of the @Issuer@ element of the SAML assertion.
arwsamlrrsIssuer :: Lens' AssumeRoleWithSAMLResponse (Maybe Text)
arwsamlrrsIssuer = lens _arwsamlrrsIssuer (\s a -> s {_arwsamlrrsIssuer = a})

-- | The temporary security credentials, which include an access key ID, a secret access key, and a security (or session) token.
arwsamlrrsCredentials :: Lens' AssumeRoleWithSAMLResponse (Maybe AuthEnv)
arwsamlrrsCredentials = lens _arwsamlrrsCredentials (\s a -> s {_arwsamlrrsCredentials = a})

-- | The identifiers for the temporary security credentials that the operation returns.
arwsamlrrsAssumedRoleUser :: Lens' AssumeRoleWithSAMLResponse (Maybe AssumedRoleUser)
arwsamlrrsAssumedRoleUser = lens _arwsamlrrsAssumedRoleUser (\s a -> s {_arwsamlrrsAssumedRoleUser = a})

-- | A percentage value that indicates the packed size of the session policies and session tags combined passed in the request. The request fails if the packed size is greater than 100 percent, which means the policies and tags exceeded the allowed space.
arwsamlrrsPackedPolicySize :: Lens' AssumeRoleWithSAMLResponse (Maybe Natural)
arwsamlrrsPackedPolicySize = lens _arwsamlrrsPackedPolicySize (\s a -> s {_arwsamlrrsPackedPolicySize = a}) . mapping _Nat

-- | -- | The response status code.
arwsamlrrsResponseStatus :: Lens' AssumeRoleWithSAMLResponse Int
arwsamlrrsResponseStatus = lens _arwsamlrrsResponseStatus (\s a -> s {_arwsamlrrsResponseStatus = a})
