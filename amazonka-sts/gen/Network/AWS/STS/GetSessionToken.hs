...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gstrrsCredentials' - The temporary security credentials, which include an access key ID, a secret access key, and a security (or session) token.
--
...
gstrrsCredentials :: Lens' GetSessionTokenResponse (Maybe AuthEnv)
gstrrsCredentials = lens _gstrrsCredentials (\s a -> s {_gstrrsCredentials = a})

-- | -- | The response status code.
gstrrsResponseStatus :: Lens' GetSessionTokenResponse Int
