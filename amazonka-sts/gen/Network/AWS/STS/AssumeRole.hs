...
data AssumeRole = AssumeRole'
  { _arTokenCode ::
    ...
    _arTransitiveTagKeys :: !(Maybe [Text]),
  ...
  toQuery AssumeRole' {..} =
    mconcat
      [ "Action" =: ("AssumeRole" :: ByteString),
        ...
        "PolicyArns"
          =: toQuery (toQueryList "member" <$> _arPolicyARNs),
        "TransitiveTagKeys"
          =: toQuery
            (toQueryList "member" <$> _arTransitiveTagKeys),
...
data AssumeRoleResponse = AssumeRoleResponse'
  { _arrrsCredentials ::
      !(Maybe AuthEnv),
    _arrrsAssumedRoleUser ::
      !(Maybe AssumedRoleUser),
    _arrrsPackedPolicySize ::
      !(Maybe Nat),
    _arrrsResponseStatus :: !Int
  }
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'arrrsCredentials' - The temporary security credentials, which include an access key ID, a secret access key, and a security (or session) token.
--
-- * 'arrrsAssumedRoleUser' - The Amazon Resource Name (ARN) and the assumed role ID, which are identifiers that you can use to refer to the resulting temporary security credentials. For example, you can reference these credentials as a principal in a resource-based policy by using the ARN or assumed role ID. The ARN and ID include the @RoleSessionName@ that you specified when you called @AssumeRole@ .
--
-- * 'arrrsPackedPolicySize' - A percentage value that indicates the packed size of the session policies and session tags combined passed in the request. The request fails if the packed size is greater than 100 percent, which means the policies and tags exceeded the allowed space.
--
-- * 'arrrsResponseStatus' - -- | The response status code.
assumeRoleResponse ::
  -- | 'arrrsResponseStatus'
  Int ->
  AssumeRoleResponse
assumeRoleResponse pResponseStatus_ =
  AssumeRoleResponse'
    { _arrrsCredentials = Nothing,
      _arrrsAssumedRoleUser = Nothing,
      _arrrsPackedPolicySize = Nothing,
      _arrrsResponseStatus = pResponseStatus_
    }

-- | The temporary security credentials, which include an access key ID, a secret access key, and a security (or session) token.
arrrsCredentials :: Lens' AssumeRoleResponse (Maybe AuthEnv)
arrrsCredentials = lens _arrrsCredentials (\s a -> s {_arrrsCredentials = a})

-- | The Amazon Resource Name (ARN) and the assumed role ID, which are identifiers that you can use to refer to the resulting temporary security credentials. For example, you can reference these credentials as a principal in a resource-based policy by using the ARN or assumed role ID. The ARN and ID include the @RoleSessionName@ that you specified when you called @AssumeRole@ .
arrrsAssumedRoleUser :: Lens' AssumeRoleResponse (Maybe AssumedRoleUser)
arrrsAssumedRoleUser = lens _arrrsAssumedRoleUser (\s a -> s {_arrrsAssumedRoleUser = a})

-- | A percentage value that indicates the packed size of the session policies and session tags combined passed in the request. The request fails if the packed size is greater than 100 percent, which means the policies and tags exceeded the allowed space.
arrrsPackedPolicySize :: Lens' AssumeRoleResponse (Maybe Natural)
arrrsPackedPolicySize = lens _arrrsPackedPolicySize (\s a -> s {_arrrsPackedPolicySize = a}) . mapping _Nat

-- | -- | The response status code.
arrrsResponseStatus :: Lens' AssumeRoleResponse Int
arrrsResponseStatus = lens _arrrsResponseStatus (\s a -> s {_arrrsResponseStatus = a})
