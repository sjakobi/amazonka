  ...
  toQuery GetFederationToken' {..} =
    mconcat
      [ "Action" =: ("GetFederationToken" :: ByteString),
        ...
        "Tags"
          =: toQuery (toQueryList "member" <$> _gftTags),
        "PolicyArns"
          =: toQuery (toQueryList "member" <$> _gftPolicyARNs),
        "Policy" =: _gftPolicy,
...
data GetFederationTokenResponse = GetFederationTokenResponse'
  { _gftrrsCredentials ::
      !(Maybe AuthEnv),
    _gftrrsFederatedUser ::
      !( Maybe
           FederatedUser
       ),
    _gftrrsPackedPolicySize ::
      !(Maybe Nat),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gftrrsCredentials' - The temporary security credentials, which include an access key ID, a secret access key, and a security (or session) token.
--
-- * 'gftrrsFederatedUser' - Identifiers for the federated user associated with the credentials (such as @arn:aws:sts::123456789012:federated-user/Bob@ or @123456789012:Bob@ ). You can use the federated user's ARN in your resource-based policies, such as an Amazon S3 bucket policy.
--
-- * 'gftrrsPackedPolicySize' - A percentage value that indicates the packed size of the session policies and session tags combined passed in the request. The request fails if the packed size is greater than 100 percent, which means the policies and tags exceeded the allowed space.
--
...
getFederationTokenResponse pResponseStatus_ =
  GetFederationTokenResponse'
    { _gftrrsCredentials =
        Nothing,
      _gftrrsFederatedUser = Nothing,
      _gftrrsPackedPolicySize = Nothing,
      _gftrrsResponseStatus = pResponseStatus_
    }

-- | The temporary security credentials, which include an access key ID, a secret access key, and a security (or session) token.
gftrrsCredentials :: Lens' GetFederationTokenResponse (Maybe AuthEnv)
gftrrsCredentials = lens _gftrrsCredentials (\s a -> s {_gftrrsCredentials = a})

-- | Identifiers for the federated user associated with the credentials (such as @arn:aws:sts::123456789012:federated-user/Bob@ or @123456789012:Bob@ ). You can use the federated user's ARN in your resource-based policies, such as an Amazon S3 bucket policy.
gftrrsFederatedUser :: Lens' GetFederationTokenResponse (Maybe FederatedUser)
gftrrsFederatedUser = lens _gftrrsFederatedUser (\s a -> s {_gftrrsFederatedUser = a})

-- | A percentage value that indicates the packed size of the session policies and session tags combined passed in the request. The request fails if the packed size is greater than 100 percent, which means the policies and tags exceeded the allowed space.
gftrrsPackedPolicySize :: Lens' GetFederationTokenResponse (Maybe Natural)
gftrrsPackedPolicySize = lens _gftrrsPackedPolicySize (\s a -> s {_gftrrsPackedPolicySize = a}) . mapping _Nat
