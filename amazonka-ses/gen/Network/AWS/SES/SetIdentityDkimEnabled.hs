...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'siderrsResponseStatus' - -- | The response status code.
setIdentityDkimEnabledResponse ::
  -- | 'siderrsResponseStatus'
  Int ->
  SetIdentityDkimEnabledResponse
...
siderrsResponseStatus :: Lens' SetIdentityDkimEnabledResponse Int
siderrsResponseStatus = lens _siderrsResponseStatus (\s a -> s {_siderrsResponseStatus = a})
