...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'sifferrsResponseStatus' - -- | The response status code.
setIdentityFeedbackForwardingEnabledResponse ::
  -- | 'sifferrsResponseStatus'
  Int ->
  SetIdentityFeedbackForwardingEnabledResponse
...
sifferrsResponseStatus :: Lens' SetIdentityFeedbackForwardingEnabledResponse Int
sifferrsResponseStatus = lens _sifferrsResponseStatus (\s a -> s {_sifferrsResponseStatus = a})
