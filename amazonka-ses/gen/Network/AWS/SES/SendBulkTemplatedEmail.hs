...
instance Hashable SendBulkTemplatedEmail

instance NFData SendBulkTemplatedEmail
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'sbterrsResponseStatus' - -- | The response status code.
--
-- * 'sbterrsStatus' - The unique message identifier returned from the @SendBulkTemplatedEmail@ action.
sendBulkTemplatedEmailResponse ::
  -- | 'sbterrsResponseStatus'
  Int ->
  SendBulkTemplatedEmailResponse
sendBulkTemplatedEmailResponse pResponseStatus_ =
  SendBulkTemplatedEmailResponse'
    { _sbterrsResponseStatus =
        pResponseStatus_,
      _sbterrsStatus = mempty
    }

-- | -- | The response status code.
sbterrsResponseStatus :: Lens' SendBulkTemplatedEmailResponse Int
sbterrsResponseStatus = lens _sbterrsResponseStatus (\s a -> s {_sbterrsResponseStatus = a})

-- | The unique message identifier returned from the @SendBulkTemplatedEmail@ action.
sbterrsStatus :: Lens' SendBulkTemplatedEmailResponse [BulkEmailDestinationStatus]
sbterrsStatus = lens _sbterrsStatus (\s a -> s {_sbterrsStatus = a}) . _Coerce

instance NFData SendBulkTemplatedEmailResponse
