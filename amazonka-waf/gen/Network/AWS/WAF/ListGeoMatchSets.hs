...
listGeoMatchSets_limit = Lens.lens (\ListGeoMatchSets' {limit} -> limit) (\s@ListGeoMatchSets' {} a -> s {limit = a} :: ListGeoMatchSets) Prelude.. Lens.mapping Prelude._Nat
  ...
  page rq rs
    ...
    | Prelude.otherwise =
      Prelude.Just Prelude.$
        rq
          Lens.& listGeoMatchSets_nextMarker
          Lens..~ rs
          Lens.^? listGeoMatchSetsResponse_nextMarker
            Prelude.. Lens._Just
...
instance Prelude.ToHeaders ListGeoMatchSets where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "X-Amz-Target"
              Prelude.=# ( "AWSWAF_20150824.ListGeoMatchSets" ::
                             Prelude.ByteString
                         ),
            "Content-Type"
