...
instance ToHeaders UpdateRuleGroup where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AWSWAF_20150824.UpdateRuleGroup" :: ByteString),
            "Content-Type"
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'urgrrsChangeToken' - The @ChangeToken@ that you used to submit the @UpdateRuleGroup@ request. You can also use this value to query the status of the request. For more information, see 'GetChangeTokenStatus' .
--
...
urgrrsChangeToken :: Lens' UpdateRuleGroupResponse (Maybe Text)
urgrrsChangeToken = lens _urgrrsChangeToken (\s a -> s {_urgrrsChangeToken = a})

-- | -- | The response status code.
urgrrsResponseStatus :: Lens' UpdateRuleGroupResponse Int
