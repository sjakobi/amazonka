...
instance AWSPager ListRegexMatchSets where
  page rq rs
    | stop (rs ^. lrmsrrsNextMarker) = Nothing
    | stop (rs ^. lrmsrrsRegexMatchSets) = Nothing
    | otherwise =
      Just $ rq & lrmsNextMarker .~ rs ^. lrmsrrsNextMarker
...
data ListRegexMatchSetsResponse = ListRegexMatchSetsResponse'
  { _lrmsrrsNextMarker ::
      !(Maybe Text),
    _lrmsrrsRegexMatchSets ::
      !( Maybe
           [RegexMatchSetSummary]
       ),
    _lrmsrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lrmsrrsNextMarker' - If you have more @RegexMatchSet@ objects than the number that you specified for @Limit@ in the request, the response includes a @NextMarker@ value. To list more @RegexMatchSet@ objects, submit another @ListRegexMatchSets@ request, and specify the @NextMarker@ value from the response in the @NextMarker@ value in the next request.
--
-- * 'lrmsrrsRegexMatchSets' - An array of 'RegexMatchSetSummary' objects.
--
-- * 'lrmsrrsResponseStatus' - -- | The response status code.
listRegexMatchSetsResponse ::
  -- | 'lrmsrrsResponseStatus'
  Int ->
  ListRegexMatchSetsResponse
listRegexMatchSetsResponse pResponseStatus_ =
  ListRegexMatchSetsResponse'
    { _lrmsrrsNextMarker =
        Nothing,
      _lrmsrrsRegexMatchSets = Nothing,
      _lrmsrrsResponseStatus = pResponseStatus_
    }

-- | If you have more @RegexMatchSet@ objects than the number that you specified for @Limit@ in the request, the response includes a @NextMarker@ value. To list more @RegexMatchSet@ objects, submit another @ListRegexMatchSets@ request, and specify the @NextMarker@ value from the response in the @NextMarker@ value in the next request.
lrmsrrsNextMarker :: Lens' ListRegexMatchSetsResponse (Maybe Text)
lrmsrrsNextMarker = lens _lrmsrrsNextMarker (\s a -> s {_lrmsrrsNextMarker = a})

-- | An array of 'RegexMatchSetSummary' objects.
lrmsrrsRegexMatchSets :: Lens' ListRegexMatchSetsResponse [RegexMatchSetSummary]
lrmsrrsRegexMatchSets = lens _lrmsrrsRegexMatchSets (\s a -> s {_lrmsrrsRegexMatchSets = a}) . _Default . _Coerce

-- | -- | The response status code.
lrmsrrsResponseStatus :: Lens' ListRegexMatchSetsResponse Int
lrmsrrsResponseStatus = lens _lrmsrrsResponseStatus (\s a -> s {_lrmsrrsResponseStatus = a})
