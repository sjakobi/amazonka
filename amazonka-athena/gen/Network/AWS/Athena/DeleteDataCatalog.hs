...
deleteDataCatalog ::
  ...
  Text ->
  DeleteDataCatalog
deleteDataCatalog pName_ =
  DeleteDataCatalog' {_ddcName = pName_}
...
instance AWSRequest DeleteDataCatalog where
  ...
  response =
    receiveEmpty
      ( \s h x ->
          DeleteDataCatalogResponse' <$> (pure (fromEnum s))
      )
...
ddcrrsResponseStatus :: Lens' DeleteDataCatalogResponse Int
