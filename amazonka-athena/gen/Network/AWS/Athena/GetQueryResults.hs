...
instance AWSPager GetQueryResults where
  page rq rs
    | stop (rs ^. gqrrrsNextToken) = Nothing
    | stop (rs ^? gqrrrsResultSet . _Just . rsRows) =
      Nothing
    | otherwise =
      Just $ rq & gqrNextToken .~ rs ^. gqrrrsNextToken
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gqrrrsNextToken' - A token generated by the Athena service that specifies where to continue pagination if a previous request was truncated. To obtain the next set of pages, pass in the @NextToken@ from the response object of the previous page call.
--
-- * 'gqrrrsResultSet' - The results of the query execution.
--
-- * 'gqrrrsUpdateCount' - The number of rows inserted with a CREATE TABLE AS SELECT statement.
--
...
getQueryResultsResponse pResponseStatus_ =
  GetQueryResultsResponse'
    { _gqrrrsNextToken =
        Nothing,
      _gqrrrsResultSet = Nothing,
      _gqrrrsUpdateCount = Nothing,
      _gqrrrsResponseStatus = pResponseStatus_
    }

-- | A token generated by the Athena service that specifies where to continue pagination if a previous request was truncated. To obtain the next set of pages, pass in the @NextToken@ from the response object of the previous page call.
gqrrrsNextToken :: Lens' GetQueryResultsResponse (Maybe Text)
gqrrrsNextToken = lens _gqrrrsNextToken (\s a -> s {_gqrrrsNextToken = a})

-- | The results of the query execution.
gqrrrsResultSet :: Lens' GetQueryResultsResponse (Maybe ResultSet)
gqrrrsResultSet = lens _gqrrrsResultSet (\s a -> s {_gqrrrsResultSet = a})

-- | The number of rows inserted with a CREATE TABLE AS SELECT statement.
gqrrrsUpdateCount :: Lens' GetQueryResultsResponse (Maybe Integer)
gqrrrsUpdateCount = lens _gqrrrsUpdateCount (\s a -> s {_gqrrrsUpdateCount = a})
