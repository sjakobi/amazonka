...
--
-- * 'wgsEngineVersion' - The engine version setting for all queries on the workgroup. Queries on the @AmazonAthenaPreviewFunctionality@ workgroup run on the preview engine regardless of this setting.
--
...
wgsEngineVersion :: Lens' WorkGroupSummary (Maybe EngineVersion)
wgsEngineVersion = lens _wgsEngineVersion (\s a -> s {_wgsEngineVersion = a})
