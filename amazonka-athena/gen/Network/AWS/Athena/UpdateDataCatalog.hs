...
--
-- * 'udcType' - Specifies the type of data catalog to update. Specify @LAMBDA@ for a federated catalog or @HIVE@ for an external hive metastore.
updateDataCatalog ::
  -- | 'udcName'
  Text ->
  -- | 'udcType'
...
instance AWSRequest UpdateDataCatalog where
  ...
  response =
    receiveEmpty
      ( \s h x ->
          UpdateDataCatalogResponse' <$> (pure (fromEnum s))
      )
...
udcrrsResponseStatus :: Lens' UpdateDataCatalogResponse Int
