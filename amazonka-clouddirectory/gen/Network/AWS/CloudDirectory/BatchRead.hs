...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'brrrsResponses' - A list of all the responses for each batch read.
--
-- * 'brrrsResponseStatus' - -- | The response status code.
batchReadResponse ::
  -- | 'brrrsResponseStatus'
  Int ->
  BatchReadResponse
batchReadResponse pResponseStatus_ =
  BatchReadResponse'
    { _brrrsResponses = Nothing,
      _brrrsResponseStatus = pResponseStatus_
    }

-- | A list of all the responses for each batch read.
brrrsResponses :: Lens' BatchReadResponse [BatchReadOperationResponse]
brrrsResponses = lens _brrrsResponses (\s a -> s {_brrrsResponses = a}) . _Default . _Coerce

-- | -- | The response status code.
brrrsResponseStatus :: Lens' BatchReadResponse Int
brrrsResponseStatus = lens _brrrsResponseStatus (\s a -> s {_brrrsResponseStatus = a})
