...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'aftorrsResponseStatus' - -- | The response status code.
addFacetToObjectResponse ::
  -- | 'aftorrsResponseStatus'
  Int ->
  AddFacetToObjectResponse
...
aftorrsResponseStatus :: Lens' AddFacetToObjectResponse Int
aftorrsResponseStatus = lens _aftorrsResponseStatus (\s a -> s {_aftorrsResponseStatus = a})
