...
listAliases_maxResults = Lens.lens (\ListAliases' {maxResults} -> maxResults) (\s@ListAliases' {} a -> s {maxResults = a} :: ListAliases) Prelude.. Lens.mapping Prelude._Nat
...
instance Prelude.ToHeaders ListAliases where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "X-Amz-Target"
              Prelude.=# ( "WorkMailService.ListAliases" ::
                             Prelude.ByteString
                         ),
            "Content-Type"
