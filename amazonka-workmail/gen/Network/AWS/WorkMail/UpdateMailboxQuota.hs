...
instance AWSRequest UpdateMailboxQuota where
  ...
  response =
    receiveEmpty
      ( \s h x ->
          UpdateMailboxQuotaResponse' <$> (pure (fromEnum s))
      )
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'umqrrsResponseStatus' - -- | The response status code.
updateMailboxQuotaResponse ::
  -- | 'umqrrsResponseStatus'
  Int ->
  UpdateMailboxQuotaResponse
...
umqrrsResponseStatus :: Lens' UpdateMailboxQuotaResponse Int
umqrrsResponseStatus = lens _umqrrsResponseStatus (\s a -> s {_umqrrsResponseStatus = a})
