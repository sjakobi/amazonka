...
data TaskDefinition = TaskDefinition'
  { _tdStatus ::
    ...
    _tdCompatibilities ::
      !(Maybe [Compatibility]),
    _tdRegisteredAt :: !(Maybe POSIX),
    _tdInferenceAccelerators ::
      !(Maybe [InferenceAccelerator]),
    _tdPlacementConstraints ::
      !( Maybe
           [TaskDefinitionPlacementConstraint]
       ),
    _tdProxyConfiguration ::
      !(Maybe ProxyConfiguration),
    _tdDeregisteredAt :: !(Maybe POSIX),
...
--
-- * 'tdRegisteredAt' - The Unix timestamp for when the task definition was registered.
--
...
--
-- * 'tdDeregisteredAt' - The Unix timestamp for when the task definition was deregistered.
--
...
--
-- * 'tdRegisteredBy' - The principal that registered the task definition.
--
...
tdRegisteredAt :: Lens' TaskDefinition (Maybe UTCTime)
tdRegisteredAt = lens _tdRegisteredAt (\s a -> s {_tdRegisteredAt = a}) . mapping _Time
...
tdDeregisteredAt :: Lens' TaskDefinition (Maybe UTCTime)
tdDeregisteredAt = lens _tdDeregisteredAt (\s a -> s {_tdDeregisteredAt = a}) . mapping _Time
...
tdRegisteredBy :: Lens' TaskDefinition (Maybe Text)
tdRegisteredBy = lens _tdRegisteredBy (\s a -> s {_tdRegisteredBy = a})
