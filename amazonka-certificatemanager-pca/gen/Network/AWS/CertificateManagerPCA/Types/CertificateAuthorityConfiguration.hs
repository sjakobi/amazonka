...
--
-- * 'cacCsrExtensions' - Specifies information to be added to the extension section of the certificate signing request (CSR).
--
...
cacCsrExtensions :: Lens' CertificateAuthorityConfiguration (Maybe CsrExtensions)
cacCsrExtensions = lens _cacCsrExtensions (\s a -> s {_cacCsrExtensions = a})
...
instance ToJSON CertificateAuthorityConfiguration where
  toJSON CertificateAuthorityConfiguration' {..} =
    object
      ( catMaybes
          [ ("CsrExtensions" .=) <$> _cacCsrExtensions,
