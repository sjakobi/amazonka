{-# LANGUAGE DeriveDataTypeable #-}
{-# LANGUAGE DeriveGeneric #-}
{-# LANGUAGE LambdaCase #-}
{-# LANGUAGE OverloadedStrings #-}
{-# LANGUAGE PatternSynonyms #-}
{-# LANGUAGE StrictData #-}
{-# LANGUAGE NoImplicitPrelude #-}
{-# OPTIONS_GHC -fno-warn-unused-imports #-}

-- Derived from AWS service descriptions, licensed under Apache 2.0.

-- |
-- Module      : Network.AWS.MediaConvert.Types.AvcIntraSlowPal
-- Copyright   : (c) 2013-2021 Brendan Hay
-- License     : Mozilla Public License, v. 2.0.
-- Maintainer  : Brendan Hay <brendan.g.hay+amazonka@gmail.com>
-- Stability   : auto-generated
-- Portability : non-portable (GHC extensions)
module Network.AWS.MediaConvert.Types.AvcIntraSlowPal
  ( AvcIntraSlowPal
      ( ..,
        AvcIntraSlowPalDISABLED,
        AvcIntraSlowPalENABLED
      ),
  )
where

import qualified Network.AWS.Prelude as Prelude

-- | Ignore this setting unless your input frame rate is 23.976 or 24 frames
-- per second (fps). Enable slow PAL to create a 25 fps output. When you
-- enable slow PAL, MediaConvert relabels the video frames to 25 fps and
-- resamples your audio to keep it synchronized with the video. Note that
-- enabling this setting will slightly reduce the duration of your video.
-- Required settings: You must also set Framerate to 25. In your JSON job
-- specification, set (framerateControl) to (SPECIFIED),
-- (framerateNumerator) to 25 and (framerateDenominator) to 1.
newtype AvcIntraSlowPal = AvcIntraSlowPal'
  { fromAvcIntraSlowPal ::
      Prelude.Text
  }
  deriving
    ( Prelude.Eq,
      Prelude.Ord,
      Prelude.Read,
      Prelude.Show,
      Prelude.Data,
      Prelude.Typeable,
      Prelude.Generic
    )

pattern AvcIntraSlowPalDISABLED :: AvcIntraSlowPal
pattern AvcIntraSlowPalDISABLED = AvcIntraSlowPal' "DISABLED"

pattern AvcIntraSlowPalENABLED :: AvcIntraSlowPal
pattern AvcIntraSlowPalENABLED = AvcIntraSlowPal' "ENABLED"

{-# COMPLETE
  AvcIntraSlowPalDISABLED,
  AvcIntraSlowPalENABLED,
  AvcIntraSlowPal'
  #-}

instance Prelude.FromText AvcIntraSlowPal where
  parser = AvcIntraSlowPal' Prelude.<$> Prelude.takeText

instance Prelude.ToText AvcIntraSlowPal where
  toText (AvcIntraSlowPal' x) = x

instance Prelude.Hashable AvcIntraSlowPal

instance Prelude.NFData AvcIntraSlowPal

instance Prelude.ToByteString AvcIntraSlowPal

instance Prelude.ToQuery AvcIntraSlowPal

instance Prelude.ToHeader AvcIntraSlowPal

instance Prelude.ToJSON AvcIntraSlowPal where
  toJSON = Prelude.toJSONText

instance Prelude.FromJSON AvcIntraSlowPal where
  parseJSON = Prelude.parseJSONText "AvcIntraSlowPal"
