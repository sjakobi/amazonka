...
data TrackSourceSettings = TrackSourceSettings'
  { -- | Use this setting to select a single captions track from a source. Track
    ...
    -- per selector.
    trackNumber :: Prelude.Maybe Prelude.Nat
  }
...
trackSourceSettings_trackNumber = Lens.lens (\TrackSourceSettings' {trackNumber} -> trackNumber) (\s@TrackSourceSettings' {} a -> s {trackNumber = a} :: TrackSourceSettings) Prelude.. Lens.mapping Prelude._Nat
