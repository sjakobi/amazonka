...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'pwrprrsResponseStatus' - -- | The response status code.
putWorkflowRunPropertiesResponse ::
  -- | 'pwrprrsResponseStatus'
  Int ->
  PutWorkflowRunPropertiesResponse
...
pwrprrsResponseStatus :: Lens' PutWorkflowRunPropertiesResponse Int
pwrprrsResponseStatus = lens _pwrprrsResponseStatus (\s a -> s {_pwrprrsResponseStatus = a})
