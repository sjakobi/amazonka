...
instance AWSPager ListNotebookExecutions where
  page rq rs
    | stop (rs ^. lnerrsMarker) = Nothing
    | stop (rs ^. lnerrsNotebookExecutions) = Nothing
    | otherwise =
      Just $ rq & lneMarker .~ rs ^. lnerrsMarker
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lnerrsNotebookExecutions' - A list of notebook executions.
--
-- * 'lnerrsMarker' - A pagination token that a subsequent @ListNotebookExecutions@ can use to determine the next set of results to retrieve.
--
-- * 'lnerrsResponseStatus' - -- | The response status code.
listNotebookExecutionsResponse ::
  -- | 'lnerrsResponseStatus'
  Int ->
  ListNotebookExecutionsResponse
listNotebookExecutionsResponse pResponseStatus_ =
  ListNotebookExecutionsResponse'
    { _lnerrsNotebookExecutions =
        Nothing,
      _lnerrsMarker = Nothing,
      _lnerrsResponseStatus = pResponseStatus_
    }

-- | A list of notebook executions.
lnerrsNotebookExecutions :: Lens' ListNotebookExecutionsResponse [NotebookExecutionSummary]
lnerrsNotebookExecutions = lens _lnerrsNotebookExecutions (\s a -> s {_lnerrsNotebookExecutions = a}) . _Default . _Coerce

-- | A pagination token that a subsequent @ListNotebookExecutions@ can use to determine the next set of results to retrieve.
lnerrsMarker :: Lens' ListNotebookExecutionsResponse (Maybe Text)
lnerrsMarker = lens _lnerrsMarker (\s a -> s {_lnerrsMarker = a})

-- | -- | The response status code.
lnerrsResponseStatus :: Lens' ListNotebookExecutionsResponse Int
lnerrsResponseStatus = lens _lnerrsResponseStatus (\s a -> s {_lnerrsResponseStatus = a})
