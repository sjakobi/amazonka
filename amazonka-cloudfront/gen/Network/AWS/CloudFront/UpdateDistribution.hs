...
instance ToPath UpdateDistribution where
  toPath UpdateDistribution' {..} =
    mconcat
      ["/2020-05-31/distribution/", toBS _udId, "/config"]
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'udrrsETag' - The current version of the configuration. For example: @E2QWRUHAPOMQZL@ .
--
-- * 'udrrsDistribution' - The distribution's information.
--
...
updateDistributionResponse pResponseStatus_ =
  UpdateDistributionResponse'
    { _udrrsETag = Nothing,
      _udrrsDistribution = Nothing,
      _udrrsResponseStatus = pResponseStatus_
    }

-- | The current version of the configuration. For example: @E2QWRUHAPOMQZL@ .
udrrsETag :: Lens' UpdateDistributionResponse (Maybe Text)
udrrsETag = lens _udrrsETag (\s a -> s {_udrrsETag = a})

-- | The distribution's information.
udrrsDistribution :: Lens' UpdateDistributionResponse (Maybe Distribution)
udrrsDistribution = lens _udrrsDistribution (\s a -> s {_udrrsDistribution = a})
