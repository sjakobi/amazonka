...
instance AWSPager ListClusters where
  page rq rs
    ...
    | stop (rs ^. lcrrsClusterListEntries) = Nothing
    | otherwise =
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lcrrsNextToken' - HTTP requests are stateless. If you use the automatically generated @NextToken@ value in your next @ClusterListEntry@ call, your list of returned clusters will start from this point in the array.
--
-- * 'lcrrsClusterListEntries' - Each @ClusterListEntry@ object contains a cluster's state, a cluster's ID, and other important status information.
--
...
listClustersResponse pResponseStatus_ =
  ListClustersResponse'
    { _lcrrsNextToken = Nothing,
      _lcrrsClusterListEntries = Nothing,
...
lcrrsClusterListEntries :: Lens' ListClustersResponse [ClusterListEntry]
lcrrsClusterListEntries = lens _lcrrsClusterListEntries (\s a -> s {_lcrrsClusterListEntries = a}) . _Default . _Coerce
