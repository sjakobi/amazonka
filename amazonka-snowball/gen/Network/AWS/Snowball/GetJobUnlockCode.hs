...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gjucrrsUnlockCode' - The @UnlockCode@ value for the specified job. The @UnlockCode@ value can be accessed for up to 90 days after the job has been created.
--
-- * 'gjucrrsResponseStatus' - -- | The response status code.
getJobUnlockCodeResponse ::
  -- | 'gjucrrsResponseStatus'
  Int ->
  GetJobUnlockCodeResponse
getJobUnlockCodeResponse pResponseStatus_ =
  GetJobUnlockCodeResponse'
    { _gjucrrsUnlockCode =
        Nothing,
      _gjucrrsResponseStatus = pResponseStatus_
    }

-- | The @UnlockCode@ value for the specified job. The @UnlockCode@ value can be accessed for up to 90 days after the job has been created.
gjucrrsUnlockCode :: Lens' GetJobUnlockCodeResponse (Maybe Text)
gjucrrsUnlockCode = lens _gjucrrsUnlockCode (\s a -> s {_gjucrrsUnlockCode = a})

-- | -- | The response status code.
gjucrrsResponseStatus :: Lens' GetJobUnlockCodeResponse Int
gjucrrsResponseStatus = lens _gjucrrsResponseStatus (\s a -> s {_gjucrrsResponseStatus = a})
