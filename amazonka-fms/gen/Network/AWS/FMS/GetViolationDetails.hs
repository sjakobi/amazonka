...
data GetViolationDetailsResponse = GetViolationDetailsResponse'
  { _gvdrrsViolationDetail ::
      !( Maybe
           ViolationDetail
       ),
    _gvdrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gvdrrsViolationDetail' - Violation detail for a resource.
--
-- * 'gvdrrsResponseStatus' - -- | The response status code.
getViolationDetailsResponse ::
  -- | 'gvdrrsResponseStatus'
  Int ->
  GetViolationDetailsResponse
getViolationDetailsResponse pResponseStatus_ =
  GetViolationDetailsResponse'
    { _gvdrrsViolationDetail =
        Nothing,
      _gvdrrsResponseStatus = pResponseStatus_
    }

-- | Violation detail for a resource.
gvdrrsViolationDetail :: Lens' GetViolationDetailsResponse (Maybe ViolationDetail)
gvdrrsViolationDetail = lens _gvdrrsViolationDetail (\s a -> s {_gvdrrsViolationDetail = a})

-- | -- | The response status code.
gvdrrsResponseStatus :: Lens' GetViolationDetailsResponse Int
gvdrrsResponseStatus = lens _gvdrrsResponseStatus (\s a -> s {_gvdrrsResponseStatus = a})
