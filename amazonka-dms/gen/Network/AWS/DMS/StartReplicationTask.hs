...
data StartReplicationTaskResponse = StartReplicationTaskResponse'
  { _srtrrsReplicationTask ::
      !( Maybe
           ReplicationTask
       ),
    _srtrrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'srtrrsReplicationTask' - The replication task started.
--
-- * 'srtrrsResponseStatus' - -- | The response status code.
startReplicationTaskResponse ::
  -- | 'srtrrsResponseStatus'
  Int ->
  StartReplicationTaskResponse
startReplicationTaskResponse pResponseStatus_ =
  StartReplicationTaskResponse'
    { _srtrrsReplicationTask =
        Nothing,
      _srtrrsResponseStatus = pResponseStatus_
    }

-- | The replication task started.
srtrrsReplicationTask :: Lens' StartReplicationTaskResponse (Maybe ReplicationTask)
srtrrsReplicationTask = lens _srtrrsReplicationTask (\s a -> s {_srtrrsReplicationTask = a})

-- | -- | The response status code.
srtrrsResponseStatus :: Lens' StartReplicationTaskResponse Int
srtrrsResponseStatus = lens _srtrrsResponseStatus (\s a -> s {_srtrrsResponseStatus = a})
