...
instance
  ...
  response =
    receiveJSON
      ( \s h x ->
          DescribeReplicationTaskIndividualAssessmentsResponse'
            <$> ( x .?> "ReplicationTaskIndividualAssessments"
                    .!@ mempty
                )
              <*> (x .?> "Marker")
              <*> (pure (fromEnum s))
...
data DescribeReplicationTaskIndividualAssessmentsResponse = DescribeReplicationTaskIndividualAssessmentsResponse'
  { _drtiarrsReplicationTaskIndividualAssessments ::
      !( Maybe
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'drtiarrsReplicationTaskIndividualAssessments' - One or more individual assessments as specified by @Filters@ .
--
-- * 'drtiarrsMarker' - A pagination token returned for you to pass to a subsequent request. If you pass this token as the @Marker@ value in a subsequent request, the response includes only records beyond the marker, up to the value specified in the request by @MaxRecords@ .
--
-- * 'drtiarrsResponseStatus' - -- | The response status code.
describeReplicationTaskIndividualAssessmentsResponse ::
  -- | 'drtiarrsResponseStatus'
  Int ->
  DescribeReplicationTaskIndividualAssessmentsResponse
...
drtiarrsReplicationTaskIndividualAssessments :: Lens' DescribeReplicationTaskIndividualAssessmentsResponse [ReplicationTaskIndividualAssessment]
drtiarrsReplicationTaskIndividualAssessments = lens _drtiarrsReplicationTaskIndividualAssessments (\s a -> s {_drtiarrsReplicationTaskIndividualAssessments = a}) . _Default . _Coerce

-- | A pagination token returned for you to pass to a subsequent request. If you pass this token as the @Marker@ value in a subsequent request, the response includes only records beyond the marker, up to the value specified in the request by @MaxRecords@ .
drtiarrsMarker :: Lens' DescribeReplicationTaskIndividualAssessmentsResponse (Maybe Text)
drtiarrsMarker = lens _drtiarrsMarker (\s a -> s {_drtiarrsMarker = a})

-- | -- | The response status code.
drtiarrsResponseStatus :: Lens' DescribeReplicationTaskIndividualAssessmentsResponse Int
drtiarrsResponseStatus = lens _drtiarrsResponseStatus (\s a -> s {_drtiarrsResponseStatus = a})
